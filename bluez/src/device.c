...
static gboolean store_device_info_cb(gpointer user_data)
{
	...
	if (device->rpa_exist) {
		...
		g_key_file_remove_key(key_file, "General", "IdentityResolvingKey",
								NULL);
	}
#endif
...
#else
	if (!device->le_state.bonded) {
		DBG("Device is not bonded. Before entering to gatt_db_clear");
		gatt_db_clear(device->db);
		DBG("After entering to gatt_db_clear");
	}
#endif
}
			...
			const char *peer, GKeyFile *key_file)
{
	...
	str = g_key_file_get_string(key_file, "General", "IdentityResolvingKey",
								NULL);
...
#ifdef __TIZEN_PATCH__
	if (device->service_changed_timeout) {
		g_source_remove(device->service_changed_timeout);
		device->service_changed_timeout = 0;
	} else {
...
#ifdef __TIZEN_PATCH__
static gboolean notify_service_changed(gpointer user_data)
{
	struct btd_device *device = user_data;

	DBG("Notify service changed");
							...
							void *user_data)
{
	...
	device->service_changed_timeout = g_timeout_add(1000,
			notify_service_changed, device);
#endif
}
