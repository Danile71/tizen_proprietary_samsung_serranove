...
#if defined TIZEN_EXT && defined TIZEN_RTC_TIMER
#define ROAMING_RENEW_TIMEOUT 4
#endif
...
#if defined TIZEN_EXT && defined TIZEN_RTC_TIMER
	if (dhcp_client->roaming == TRUE) {
		/* For roaming case,
		 * set renew timeout 4 seconds.
		 * and restart dhcp again if renew failed.
		 *   Ex. fail case) using same ssid but different gateway.
		 */
		dhcp_client->roaming = FALSE;
		dhcp_client->lease_seconds = 4;
		dhcp_client->rtc_timeout = rtc_timeout_add_seconds(
				ROAMING_RENEW_TIMEOUT,
				start_renew_timeout,
				dhcp_client,
				NULL);
	} else {
...
void g_dhcp_client_start_renew(GDHCPClient *dhcp_client)
{
	...
	/* If lease_seconds <= 60, it's going through dhcp rebound.
	 * that's why we don't have to set roaming flag for renew.
	 */
	if (dhcp_client->lease_seconds > 60)
		dhcp_client->roaming = TRUE;
