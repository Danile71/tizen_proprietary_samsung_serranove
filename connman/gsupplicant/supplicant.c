...
static struct strvalmap keymgmt_map[] = {
	...
	{ "wapi-psk",	G_SUPPLICANT_KEYMGMT_WAPI_PSK	},
	{ "wapi-cert",	G_SUPPLICANT_KEYMGMT_WAPI_CERT	},
#endif
#if defined TIZEN_EXT
	{ "cckm",	G_SUPPLICANT_KEYMGMT_WPA_CCKM	},
#endif
	{ }
};
...
#endif
#if defined TIZEN_EXT && defined TIZEN_WLAN_CHINA_WAPI
	case G_SUPPLICANT_SECURITY_WAPI_PSK:
		return "wapi_psk";
	case G_SUPPLICANT_SECURITY_WAPI_CERT:
		return "wapi_cert";
#endif
	}
...
#if defined TIZEN_EXT && TIZEN_WLAN_CHINA_WAPI
#define WLAN_EID_WAPI	68
#define WLAN_WAPI_CERT	1
#define WLAN_WAPI_PSK	2
#endif
	...
	for (ie_end = ie + ie_len; ie < ie_end && ie + ie[1] + 1 <= ie_end;
							ie += ie[1] + 2) {
#if defined TIZEN_EXT && TIZEN_WLAN_CHINA_WAPI
		if (ie[0] == WLAN_EID_WAPI && ie[9] == WLAN_WAPI_PSK) {
			SUPPLICANT_DBG("IE: match WAPI_PSK");
			bss->security = G_SUPPLICANT_SECURITY_WAPI_PSK;
			return;
		} else if (ie[0] == WLAN_EID_WAPI && ie[9] == WLAN_WAPI_CERT) {
			SUPPLICANT_DBG("IE: match WAPI_CERT");
			bss->security = G_SUPPLICANT_SECURITY_WAPI_CERT;
			return;
		}
#endif
...
static void bss_compute_security(struct g_supplicant_bss *bss)
{
...
#if defined TIZEN_EXT && TIZEN_WLAN_CHINA_WAPI
	if (bss->security == G_SUPPLICANT_SECURITY_WAPI_PSK ||
			bss->security == G_SUPPLICANT_SECURITY_WAPI_CERT)
		return;
#endif
	...
	if (bss->keymgmt &
		...
		bss->ieee8021x = TRUE;
	if (bss->keymgmt & G_SUPPLICANT_KEYMGMT_WPA_FT_EAP)
		bss->ft_ieee8021x = TRUE;
	if (bss->keymgmt & G_SUPPLICANT_KEYMGMT_WPA_CCKM)
		bss->cckm = TRUE;
#else
	if (bss->keymgmt &
			(G_SUPPLICANT_KEYMGMT_WPA_EAP |
...
#if defined TIZEN_EXT && defined TIZEN_WLAN_CHINA_WAPI
static void add_network_security_wapi_cert(DBusMessageIter *dict,
						GSupplicantSSID *ssid)
{
	if (ssid->ca_cert_path) {
		SUPPLICANT_DBG("wapi_as_cert %s", ssid->ca_cert_path);
		supplicant_dbus_dict_append_basic(dict, "wapi_as_cert",
				DBUS_TYPE_STRING,
				&ssid->ca_cert_path);
	} else
		SUPPLICANT_DBG("wapi_as_cert is NULL");

	if (ssid->client_cert_path) {
		SUPPLICANT_DBG("wapi_user_cert_path %s", ssid->client_cert_path);
		supplicant_dbus_dict_append_basic(dict, "wapi_user_cert",
				DBUS_TYPE_STRING,
				&ssid->client_cert_path);
	} else
		SUPPLICANT_DBG("wapi_user_cert_path is NULL");
}
#endif
...
static void add_network_security(DBusMessageIter *dict, GSupplicantSSID *ssid)
{
	...
	case G_SUPPLICANT_SECURITY_IEEE8021X:
#if defined TIZEN_EXT
		if (ssid->eap_keymgmt == G_SUPPLICANT_EAP_KEYMGMT_FT) {
			key_mgmt = "FT-EAP";
			SUPPLICANT_DBG("TEST keymgmt : FT-EAP");
		} else if (ssid->eap_keymgmt == G_SUPPLICANT_EAP_KEYMGMT_CCKM) {
			key_mgmt = "CCKM";
			SUPPLICANT_DBG("TEST keymgmt : CCKM");
		} else {
			key_mgmt = "WPA-EAP";
			SUPPLICANT_DBG("TEST keymgmt : EAP");
		}
#else
		key_mgmt = "WPA-EAP";
#endif
	...
	case G_SUPPLICANT_SECURITY_WAPI_CERT:
		key_mgmt = "WAPI-CERT";
		add_network_security_wapi_cert(dict, ssid);
		add_network_security_ciphers(dict, ssid);
		add_network_security_proto(dict, ssid);
		break;
#endif
