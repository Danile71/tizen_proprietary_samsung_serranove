...
#if defined TIZEN_EXT
#define SERVICE_KEY_KEYMGMT_TYPE       "KeymgmtType"
#endif
...
#if defined TIZEN_EXT
	g_free(config_service->keymgmt_type);
#endif
...
#if defined TIZEN_EXT
	str = g_key_file_get_string(keyfile, group, SERVICE_KEY_KEYMGMT_TYPE, NULL);
	if (str != NULL) {
...
#if TIZEN_EXT
		if (g_str_has_prefix(ident, "wifi_") == TRUE) {
			if (event->mask & IN_CREATE) {
				...
				__connman_service_set_storage_reload(service, TRUE);

				DBG("set service storage reload %s", ident);
			}

			continue;
		} else if (g_str_has_suffix(ident, ".config") == FALSE)
			continue;
#else
...
static void provision_service(gpointer key, gpointer value, gpointer user_data)
{
	...
	if (config->keymgmt_type != NULL)
		__connman_service_set_string(service, "KeymgmtType", config->keymgmt_type);
