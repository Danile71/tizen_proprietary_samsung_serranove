...
#if defined TIZEN_EXT
	g_free(network->wifi.keymgmt_type);
#endif
...
void connman_network_set_index(struct connman_network *network, int index)
{
	...
	struct connman_ipconfig *ipconfig_v6 = NULL;
#else
	struct connman_ipconfig *ipconfig;
#endif
	...
	{
		//IPv6 index set
		ipconfig_v6 = __connman_service_get_ip6config(service);
		if(ipconfig_v6 == NULL)
			goto done;

		__connman_ipconfig_set_index(ipconfig_v6, index);
	}
#else
	...
	network->index = index;
#if defined TIZEN_EXT
	DBG("index %d service %p ip4config %p ip6config %p", network->index,
			service, ipconfig, ipconfig_v6);
#endif
}
						...
						connman_bool_t associating)
{
	...
	if (associating == TRUE) {
		...
		__connman_service_ipconfig_indicate_state(service,
					...
					CONNMAN_IPCONFIG_TYPE_IPV6);
#else
		struct connman_ipconfig *ipconfig_v4 = NULL;
		struct connman_ipconfig *ipconfig_v6 = NULL;
		enum connman_ipconfig_method method_v4;
		enum connman_ipconfig_method method_v6;

		ipconfig_v4 = __connman_service_get_ip4config(service);
		ipconfig_v6 = __connman_service_get_ip6config(service);

		method_v4 = __connman_ipconfig_get_method(ipconfig_v4);
		method_v6 = __connman_ipconfig_get_method(ipconfig_v6);

		if (method_v4 > CONNMAN_IPCONFIG_METHOD_OFF){
			__connman_service_ipconfig_indicate_state(service,
						CONNMAN_SERVICE_STATE_ASSOCIATION,
						CONNMAN_IPCONFIG_TYPE_IPV4);
		}

		if (method_v6 > CONNMAN_IPCONFIG_METHOD_OFF) {
			__connman_service_ipconfig_indicate_state(service,
						CONNMAN_SERVICE_STATE_ASSOCIATION,
						CONNMAN_IPCONFIG_TYPE_IPV6);
		}
...
static void set_ip_configuration_roam(struct connman_network *network)
{
	struct connman_ipconfig *ipconfig_ipv4, *ipconfig_ipv6;
	enum connman_ipconfig_method ipv4_method, ipv6_method;
	struct connman_service *service;
	...
	if (__connman_ipconfig_get_local(ipconfig_ipv4) == NULL &&
			__connman_ipconfig_get_local(ipconfig_ipv6) == NULL) {
		DBG("IP not assigned yet");
		return;
	}
	...
	case CONNMAN_IPCONFIG_METHOD_MANUAL:
		return;
	case CONNMAN_IPCONFIG_METHOD_DHCP:
		__connman_dhcp_start_renew(network, dhcp_callback);
		return;
	}
}
#endif
...
#if defined TIZEN_EXT
	if (network->type == CONNMAN_NETWORK_TYPE_WIFI &&
			network->connected == TRUE && connected == TRUE) {
		DBG("Wi-Fi roaming and IP configuration if necessary");
		set_ip_configuration_roam(network);
	}
#endif
...
#if defined TIZEN_EXT
	} else if (g_str_equal(key, "WiFi.KeymgmtType") == TRUE) {
		g_free(network->wifi.keymgmt_type);
		network->wifi.keymgmt_type = g_strdup(value);
#endif
	} else {
		return -EINVAL;
	}
							...
							const char *key)
{
	...
	if (g_str_equal(key, "Path") == TRUE)
		...
		return network->wifi.keymgmt_type;
#endif
