...
#define TIME_METHOD "alarm_set_time_with_propagation_delay"
		...
		msg = dbus_message_new_method_call(TIME_BUS_NAME, TIME_PATH, TIME_INTERFACE, TIME_METHOD);
		dbus_message_append_args(msg, DBUS_TYPE_UINT32, &(cur.tv_sec), DBUS_TYPE_UINT32, &(cur.tv_nsec),
			DBUS_TYPE_UINT32, &(req.tv_sec), DBUS_TYPE_UINT32, &(req.tv_nsec), DBUS_TYPE_INVALID);
