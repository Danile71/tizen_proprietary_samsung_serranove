...
static const char *security2string(enum connman_service_security security)
{
	...
	case CONNMAN_SERVICE_SECURITY_WAPI_CERT:
		return "wapi_cert";
#endif
	case CONNMAN_SERVICE_SECURITY_FT_PSK:
		return "ft_psk";
#endif
	}
...
static const char *error2string(enum connman_service_error error)
{
	...
	case CONNMAN_SERVICE_ERROR_INVALID_KEY:
		return "invalid-key";
#if defined TIZEN_EXT
	case CONNMAN_SERVICE_ERROR_INTERNET_FAILED:
...
#if defined TIZEN_EXT
	else if (g_strcmp0(error, "internet-unreachable") == 0)
		return CONNMAN_SERVICE_ERROR_INTERNET_FAILED;
#endif
...
#if 0
#define IPTIME "iptime"
#define IPTIME_5G "iptime5G"
#endif
...
static int service_load(struct connman_service *service)
{
	...
	{
		gboolean b_ssid_exist = FALSE;
		guint cnt_bssid = 0;
		unsigned char *bssid;

		gchar *ssid = NULL;
		gchar *bssid_str = NULL;
		gchar **saved_bssid_list = NULL;

		ssid = g_strdup(service->name);
		bssid = connman_network_get_bssid(service->network);
		bssid_str = g_strdup_printf("%02x:%02x:%02x:%02x:%02x:%02x",
				bssid[0], bssid[1], bssid[2],
				bssid[3], bssid[4], bssid[5]);

		DBG("ssid (%s) bssid(%s)", ssid, bssid_str);

		if(g_strcmp0(IPTIME, ssid) == 0 || g_strcmp0(IPTIME_5G, ssid) == 0){
			int idx = 0;

			saved_bssid_list = g_key_file_get_string_list(keyfile, service->identifier, "BSSID", &cnt_bssid, NULL);

			if(cnt_bssid <= 0){
				DBG("no stored bssid");
				b_ssid_exist = TRUE;
			}
			else if(cnt_bssid > 0 && cnt_bssid < 100){

				for(idx = 0; idx < cnt_bssid; idx++){
					if(g_strcmp0(saved_bssid_list[idx], bssid_str) == 0){
						DBG("same bssid already exist");
						b_ssid_exist = TRUE;
						break;
					}
				}
				...
				DBG("stored bssid over (%d)", cnt_bssid);
				b_ssid_exist = TRUE;
			}

			DBG("service auto connect support (%d)", b_ssid_exist);
			service->autoconnect = b_ssid_exist;


			if(saved_bssid_list != NULL)
				g_strfreev(saved_bssid_list);

			g_free(ssid);
			g_free(bssid_str);

			DBG("done to compare the bssid");
		}
	}
#endif
	...
	if (service->type == CONNMAN_SERVICE_TYPE_WIFI &&
		...
		str = g_key_file_get_string(keyfile,
				service->identifier, "KeymgmtType", NULL);
		if (str != NULL) {
			g_free(service->keymgmt_type);
			service->keymgmt_type = str;
		}
	}
#endif
done:
...
static int service_save(struct connman_service *service)
{
	...
	case CONNMAN_SERVICE_TYPE_WIFI:
...
#if 0
		{
			...
			if(g_strcmp0(IPTIME, ssid) == 0 || g_strcmp0(IPTIME_5G, ssid) == 0){
				...
				gchar **saved_clone = NULL;
				saved_bssid_list = g_key_file_get_string_list(keyfile, service->identifier, "BSSID", &cnt_bssid, NULL);
				DBG("ssid list len(%d)", cnt_bssid);

				saved_clone = g_new(gchar *, cnt_bssid+1);

				if(saved_bssid_list){
					for(idx = 0; idx < cnt_bssid; idx++){
						saved_clone[idx] = saved_bssid_list[idx];
						if(g_strcmp0(saved_clone[idx], bssid_str) == 0){
							DBG("same bssid already exist");
							b_ssid_exist = TRUE;
							break;
						}
				...
				if(!b_ssid_exist){
					guint arr_len = 0;
					DBG("add the new bssid into config");

					saved_clone[idx] = bssid_str;
					arr_len = idx + 1;
					g_key_file_set_string_list(keyfile, service->identifier, "BSSID", (const gchar **)saved_clone, arr_len);
				}
	...
	if (service->type == CONNMAN_SERVICE_TYPE_WIFI &&
		...
		if (service->keymgmt_type != NULL && strlen(service->keymgmt_type) > 0)
			g_key_file_set_string(keyfile, service->identifier,
					"KeymgmtType", service->keymgmt_type);
		else
			g_key_file_remove_key(keyfile, service->identifier,
					"KeymgmtType", NULL);
	}
...
#if defined TIZEN_EXT
	if(g_strcmp0(nameserver, "::") == 0){
		DBG("invalid IPv6 name server");
		return -EINVAL;
	}
#endif
					...
					struct connman_network *network)
{
	...
	if (str != NULL && g_str_equal(str, "ieee8021x") == TRUE) {
		...
		str = connman_network_get_string(network,
							"WiFi.KeymgmtType");
		if (str != NULL)
			connman_dbus_dict_append_basic(dict, "KeymgmtType",
					DBUS_TYPE_STRING, &str);
	}
}
#endif
					...
					DBusMessage *msg, void *user_data)
{
	...
	if (g_str_equal(name, "AutoConnect") == TRUE) {
		...
		service_save(service);
	} else if (g_str_equal(name, "Error") == TRUE) {
		const char *s_error;
		enum connman_service_error err = CONNMAN_SERVICE_ERROR_UNKNOWN;

		DBG("set service error");
		if (type != DBUS_TYPE_STRING)
			return __connman_error_invalid_arguments(msg);

		dbus_message_iter_get_basic(&value, &s_error);
		DBG("service %p, error(%s)", service, s_error);

		err = string2error(s_error);

		set_error(service, err);
...
#if defined TIZEN_EXT
	if(service->error == CONNMAN_SERVICE_ERROR_INTERNET_FAILED)
		return TRUE;
#endif
...
{
#if defined TIZEN_EXT
	struct connman_service *candidate_service = NULL;
#endif
...
static void __connman_service_cleanup_network_8021x(struct connman_service *service)
{
	...
	connman_network_set_string(service->network, "WiFi.KeymgmtType", NULL);
}
#endif
...
#if defined TIZEN_EXT
	else if (g_str_equal(key, "KeymgmtType") == TRUE) {
		g_free(service->keymgmt_type);
		service->keymgmt_type = g_strdup(value);
	}
#endif
}
...
static enum connman_service_security convert_wifi_security(const char *security)
{
	if (security == NULL)
		...
		return CONNMAN_SERVICE_SECURITY_FT_PSK;
