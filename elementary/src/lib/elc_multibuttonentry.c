...
_item_del(Elm_Multibuttonentry_Item_Data *item)
{
   ...
   if (!eina_list_count(sd->items))
     _visual_guide_text_set(obj);
   eo_it = eina_list_data_get(eina_list_last(sd->items));
   if (eo_it)
     {
        ELM_MULTIBUTTONENTRY_ITEM_DATA_GET(eo_it, curr_it);
        elm_layout_signal_emit(VIEW(curr_it), "elm,state,hide", "elm");
     }
}
                 ...
                 void *data)
{
   ...
   eo_obj_item = eina_list_data_get(eina_list_last(sd->items));
   if (eo_obj_item)
     {
        ELM_MULTIBUTTONENTRY_ITEM_DATA_GET(eo_obj_item, last_it);
        elm_layout_signal_emit(VIEW(last_it), "elm,state,show", "elm");
     }
   ...
   eo_obj_item = eina_list_data_get(eina_list_last(sd->items));
   ELM_MULTIBUTTONENTRY_ITEM_DATA_GET(eo_obj_item, curr_it);
   elm_layout_signal_emit(VIEW(curr_it), "elm,state,hide", "elm");
