...
static Evas_Object* _create_bg_image(Eo* eo_item);
static void _remove_bg_image(Eo* eo_item);
                                   ...
                                   const char *source EINA_UNUSED)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "_on_item_title_transition_finished: %p", obj);
                                                  ...
                                                  const char *label)
{
   ...
   else if ((!strcmp("subtitle", part)) || (!strcmp(part, SUBTITLE_PART)))
     {
...
_elm_naviframe_item_eo_base_destructor(Eo *eo_item, Elm_Naviframe_Item_Data *it)
{
   ...
   ecore_idler_del(nit->animator);

   _remove_bg_image(eo_item);
...
static void
_remove_bg_image(Eo* eo_item)
{
   Evas_Object* bg_image = NULL;

   ELM_NAVIFRAME_ITEM_DATA_GET(eo_item, it);

   if (edje_object_part_exists(elm_layout_edje_get(VIEW(it)), "mesh_bg_changeable"))
     {
        bg_image = elm_object_part_content_get(VIEW(it), "mesh_bg_changeable");
        if (bg_image)
          {
             gxt_shared_image_buffer_deinitialize(it->handler);
             elm_object_part_content_unset(VIEW(it), "mesh_bg_changeable");
             evas_object_image_native_surface_set(bg_image, NULL);
...
_create_bg_image(Eo* eo_item)
{
   ...
   if (eelist)
     {
       ...
       EINA_LIST_FOREACH(eelist, list, ee)
         {
             changeable = ecore_evas_data_get(ee, "changeable_ui");
             if (changeable) break;
         }
   ...
   if (changeable)
     {
       unsigned char *fixed = (unsigned char *)ecore_evas_data_get(ecore_evas_ecore_evas_get(evas_object_evas_get(WIDGET(it))), "fixed_theme");
       ...
       if (edje_object_part_exists(elm_layout_edje_get(VIEW(it)), "mesh_bg_changeable"))
        {
           ...
           if (bg_image)
             {
                image_width = tbm_surface_get_width(it->tbm_surface);
                image_height = tbm_surface_get_height(it->tbm_surface);
                evas_object_image_data_update_add(bg_image, 0, 0,  image_width, image_height);
             }
           else
             {
                ...
                image_width = tbm_surface_get_width(tbm_surface);
                image_height = tbm_surface_get_height(tbm_surface);

                bg_image = evas_object_image_filled_add(evas_object_evas_get(WIDGET(it)));
                if (!bg_image)
                  {
                     gxt_shared_image_buffer_deinitialize(handler);
                     return NULL;
                ...
                evas_object_resize(bg_image, image_width, image_height);
                evas_object_image_size_set(bg_image, image_width, image_height);
                evas_object_show(bg_image);

                if (fixed)
                  {
                     evas_object_image_data_set(bg_image, fixed);
                     evas_object_image_data_update_add(bg_image, 0, 0, image_width, image_height);
                  }
                else
                  {
                         ...
                         evas_object_image_native_surface_set(bg_image, &surface);
                         evas_object_image_data_update_add(bg_image, 0, 0, image_width, image_height);
                  }

                it->tbm_surface = tbm_surface;

                elm_object_part_content_set(VIEW(it), "mesh_bg_changeable", bg_image);
             }
       	  }
        elm_object_signal_emit(VIEW(it), "elm,state,changeable,bg,on", "elm");
     }
   else
     {
        elm_object_signal_emit(VIEW(it), "elm,state,changeable,bg,off", "elm");
     }
...
static void
_color_class_changed_cb(void *data,
                        Evas_Object *obj EINA_UNUSED,
                        const char *emission EINA_UNUSED,
                        const char *source EINA_UNUSED)
{
          ...
          const char *item_style)
{
   ...
   elm_layout_signal_callback_add(VIEW(it), "color_class,set", "B011", _color_class_changed_cb, eo_item);
   ...
   if (_elm_config->atspi_mode)
     {
         Evas_Object *part = (Evas_Object*)_edje_object_part_object_get2(VIEW(it), TITLE_ACCESS_PART);
...
static void
_custom_op(Elm_Transit_Effect *effect, Elm_Transit *transit EINA_UNUSED, double progress)
{
   if (!effect) return;
...
static void
_tizen_push_effect(Elm_Naviframe_Item_Data *it)
{
   Elm_Transit *transit;
...
static void
_tizen_pop_effect(Elm_Naviframe_Item_Data *it)
{
   Elm_Transit *transit;
   Elm_Transit_Effect *effect_context;
   double v[] = {0.25, 0.46, 0.45, 1.0};
   ...
   if (top_item)
     {
        ...
        ecore_idler_del(item->animator);
        item->animator = ecore_idler_add(_push_transition_cb, item);
     }
   else
     {
...
elm_naviframe_add(Evas_Object *parent)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "elm_naviframe_add: %p", parent);
...
_elm_naviframe_item_push(Eo *obj, Elm_Naviframe_Data *sd EINA_UNUSED, const char *title_label, Evas_Object *prev_btn, Evas_Object *next_btn, Evas_Object *content, const char *item_style)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "_elm_naviframe_item_push: %p", obj);
...
_elm_naviframe_item_pop(Eo *obj, Elm_Naviframe_Data *sd)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "_elm_naviframe_item_pop: %p", obj);
   ...
   if (prev_it)
     {
        ...
        ecore_idler_del(it->animator);
        it->animator = ecore_idler_add(_pop_transition_cb, it);
...
_elm_naviframe_item_pop_to(Eo *eo_it, Elm_Naviframe_Item_Data *it)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "_elm_naviframe_item_pop_to: %p", eo_it);

   if (eo_it == elm_naviframe_top_item_get(WIDGET(it)))
     {
        traceEnd(TTRACE_TAG_VIEW);
        return;
     }
