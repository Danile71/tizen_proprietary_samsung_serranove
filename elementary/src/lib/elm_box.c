...
//TIZEN_ONLY
static void
_elm_box_remove_banded_item_data(Evas_Object* box_obj, Evas_Object *data)
{
   Elm_Box_Banded_Item_Data *opt;
   Eina_List *l;
   ELM_BOX_DATA_GET(box_obj, sd_box);

   EINA_LIST_FOREACH(sd_box->children, l, opt)
     {
        Evas_Object *obj = opt->child_obj;
        if (obj == data)
          {
             ELM_SAFE_FREE(opt->banded_bg, evas_object_del);
             sd_box->children = eina_list_remove(sd_box->children, opt);
             free(opt);
          }
     }
}
...
static void
_elm_box_remove_banded_item_data_all(Evas_Object* box_obj)
{
   Elm_Box_Banded_Item_Data *opt;
   Eina_List *l;
   ELM_BOX_DATA_GET(box_obj, sd_box);
...
static void
_banded_item_color_set(Evas_Object *obj, int index)
{
   int color;
   color = 255 * (1 - 0.04 * index);

   if (obj)
     evas_object_color_set(obj, color, color, color, color);
}

EOLIAN static void
_apply_color(Evas_Object *obj)
{
   ELM_BOX_DATA_GET(obj, sd);
   Eina_List* pList = sd->children;
   Elm_Box_Banded_Item_Data *it;
   Eina_List* tempList;
   Elm_Box_Banded_Item_Data *top_item = NULL;
   int color_set_index = 0;
   Evas_Coord item_x, item_y, item_w, item_h;
   evas_object_geometry_get(sd->scroller , NULL, &sd->svy, NULL, &sd->svh);
   if (pList)
     {
        Eina_Bool setColor = EINA_FALSE;
        EINA_LIST_FOREACH(pList, tempList, it)
          {
             evas_object_geometry_get(it->child_obj , &item_x, &item_y, &item_w, &item_h);
             if (!setColor && ((item_y + item_h >= sd->svy)  && (item_y + item_h < sd->svy + sd->svh)))
               setColor = EINA_TRUE;
             else if (!setColor &&  (item_y <= sd->svy) && ((item_y + item_h) >= sd->svh))
               setColor = EINA_TRUE;

             if (setColor)
               {
                  _banded_item_color_set(it->banded_bg, color_set_index);
                  color_set_index++;
                  if(color_set_index > MAX_ITEMS_PER_VIEWPORT)
                    break;
               }
          }
        sd->top_item = top_item;
     }

}

static void
_scroller_resize_cb(void *data, Evas *e EINA_UNUSED, Evas_Object *obj EINA_UNUSED, void *event_info EINA_UNUSED)
{
   _apply_color(data);
}

static void
_item_resize_cb(void *data, Evas *e EINA_UNUSED, Evas_Object *obj EINA_UNUSED, void *event_info EINA_UNUSED)
{
   _apply_color(data);
}

static void
_set_scroller_size(Evas_Object *obj)
{
   ELM_BOX_DATA_GET(obj, sd);
   evas_object_geometry_get(sd->scroller , NULL, &sd->svy, NULL, &sd->svh);
}
...
static void
_apply_item_color(Evas_Object *obj)
{
   ELM_BOX_DATA_GET(obj, sd);
   Eina_List* pList = sd->children;
   Elm_Box_Banded_Item_Data *it;
        ...
        EINA_LIST_FOREACH(pList, tempList, it)
          {
             ...
             if (!setColor)
               {
                  if((item_y + item_h >= sd->svy)  && (item_y + item_h < sd->svy + sd->svh))
                    {
                       setColor = EINA_TRUE;
                       top_item = it;
                    }
                  else if((item_y <= sd->svy) && ((item_y + item_h) >= sd->svh))
                    {
                       setColor = EINA_TRUE;
                       top_item = it;
                    }
               }
             if ( setColor && top_item && top_item != sd->top_item)
               {
                  _banded_item_color_set(it->banded_bg, color_set_index);
                  color_set_index++;
                  if(color_set_index > MAX_ITEMS_PER_VIEWPORT)
...
static void
_item_move_cb(void *data, Evas *e EINA_UNUSED, Evas_Object *obj EINA_UNUSED, void *event_info EINA_UNUSED)
{
   Evas_Object *box = data;
   _apply_item_color(box);
}

static Elm_Box_Banded_Item_Data*
_get_item(Evas_Object *box, Evas_Object *box_item)
{
   Elm_Box_Banded_Item_Data *opt;
   ELM_BOX_DATA_GET(box, sd_box);
   Eina_List *l;
   EINA_LIST_FOREACH(sd_box->children, l, opt)
     {
        Evas_Object *obj = opt->child_obj;
...
static void
_banded_item_initialize(Evas_Object *obj, Evas_Object *subobj, Evas_Object *box_item, Elm_Box_Pack_Type type)
{
   ELM_WIDGET_DATA_GET_OR_RETURN(subobj, sub_wd);
   ELM_BOX_DATA_GET(obj, sd_box);
   ELM_WIDGET_DATA_GET_OR_RETURN(obj, wd);
   _set_scroller_size(obj);
   char buf[256];
   const char *bg_area = NULL;
   bg_area = edje_object_data_get(sub_wd->resize_obj, "banded_bg_area");
   if (!bg_area) return;

   Elm_Box_Banded_Item_Data *relative_it;
   Elm_Box_Banded_Item_Data* it = malloc(sizeof(Elm_Box_Banded_Item_Data));
   if (!it) return;
   it->child_obj = subobj;


        Evas_Object* banded_bg = evas_object_rectangle_add(evas_object_evas_get(subobj));
        it->banded_bg = banded_bg;

        if (type == ELM_BOX_PACK_START)
          sd_box->children = eina_list_prepend(sd_box->children, it);
        else if (type == ELM_BOX_PACK_END)
          sd_box->children = eina_list_append(sd_box->children, it);
        else if (type == ELM_BOX_PACK_BEFORE && box_item)
          {
             relative_it = _get_item(obj, box_item);
             sd_box->children = eina_list_prepend_relative(sd_box->children, it, relative_it);
          }
        else if (type == ELM_BOX_PACK_AFTER && box_item)
          {
             relative_it = _get_item(obj, box_item);
             sd_box->children = eina_list_append_relative(sd_box->children, it, relative_it);
          }

        edje_object_part_swallow(sub_wd->resize_obj, bg_area, banded_bg);
        evas_object_color_set(banded_bg, 0, 0, 0, 0);
        snprintf(buf, sizeof(buf), "elm,state,%s,visible", bg_area);
        edje_object_signal_emit(subobj, buf, "elm");

   if (sd_box->scroller)
     evas_object_event_callback_add(sd_box->scroller, EVAS_CALLBACK_RESIZE, _scroller_resize_cb, obj);

   evas_object_event_callback_add(subobj, EVAS_CALLBACK_MOVE, _item_move_cb, obj);
   evas_object_event_callback_add(subobj, EVAS_CALLBACK_RESIZE, _item_resize_cb, obj);
}
//TIZEN_ONLY end
...
//TIZEN_ONLY
   ...
   if (sd->is_banded_style == EINA_TRUE)
     {
        _set_scroller_size(data);
        _apply_item_color(data);
     }
}
...
//TIZEN_ONLY
   ...
   if (sd->is_banded_style == EINA_TRUE)
     {
        _elm_box_remove_banded_item_data(obj, child);
     }
   _sizing_eval(obj);
...
//TIZEN_ONLY
   Evas_Object *banded_scroller;
   ELM_BOX_DATA_GET(obj, sd_box);
   sd_box->is_banded_style = EINA_FALSE;
   sd_box->children = NULL;
...
//TIZEN_ONLY
   if (sd->is_banded_style == EINA_TRUE)
     {
        _banded_item_initialize(obj, subobj, NULL, ELM_BOX_PACK_START);
        _apply_color(obj);
     }
}
...
//TIZEN_ONLY
   if (sd->is_banded_style == EINA_TRUE)
     {
        _banded_item_initialize(obj, subobj, NULL, ELM_BOX_PACK_END);
        _apply_color(obj);
     }
...
//TIZEN_ONLY
   if (sd->is_banded_style == EINA_TRUE)
     {
        _banded_item_initialize(obj, subobj, before, ELM_BOX_PACK_BEFORE);
        _apply_color(obj);
     }
}
...
//TIZEN_ONLY
   if (sd->is_banded_style == EINA_TRUE)
     {
        _banded_item_initialize(obj, subobj, after, ELM_BOX_PACK_AFTER);
        _apply_color(obj);
     }
}
...
//TIZEN_ONLY
   if (sd->is_banded_style == EINA_TRUE)
     _elm_box_remove_banded_item_data_all(obj);
}
...
//TIZEN_ONLY
   if (sd->is_banded_style == EINA_TRUE)
     {
     _elm_box_remove_banded_item_data(obj, subobj);
        _apply_color(obj);
     }
}
