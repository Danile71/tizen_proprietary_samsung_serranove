...
#define ELM_BUTTON_MAX_BUFFER_SIZE 256
...
_elm_button_elm_widget_theme_apply(Eo *obj, Elm_Button_Data *_pd EINA_UNUSED)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "_elm_button_elm_widget_theme_apply: %p", obj);
...
#ifdef TIZEN_VECTOR_UX
   if (!strcmp(evas_object_type_get(sobj), "vectors"))
     return EINA_TRUE;
#endif
...
_elm_button_elm_container_content_set(Eo *obj, Elm_Button_Data *_pd EINA_UNUSED, const char *part, Evas_Object *content)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "_elm_button_elm_container_content_set: %p", obj);
...
#ifdef TIZEN_VECTOR_UX
   if (!strcmp(evas_object_type_get(content), "vectors"))
     {
        traceEnd(TTRACE_TAG_VIEW);
        return EINA_TRUE;
     }
...
_elm_button_evas_object_smart_add(Eo *obj, Elm_Button_Data *_pd EINA_UNUSED)
{
   ...
   char *style = NULL;
   char spinner_style_buffer[ELM_BUTTON_MAX_BUFFER_SIZE] = {0,};
   char *datetime_spinner_increase_style = NULL;
   char *datetime_spinner_text_style = NULL;
   char *datetime_spinner_decrease_style = NULL;

   traceBegin(TTRACE_TAG_VIEW, "_elm_button_evas_object_smart_add: %p", obj);
   ...
   button = elm_object_parent_widget_get(obj);

   style = evas_object_data_get(button, "UXT_BUTTON_ADD");
   datetime_spinner_increase_style = evas_object_data_get(button, "ELM_BUTTON_FOR_DATETIME_SPINNER_INCREASE");
   datetime_spinner_text_style = evas_object_data_get(button, "ELM_BUTTON_FOR_DATETIME_SPINNER_TEXT");
   datetime_spinner_decrease_style = evas_object_data_get(button, "ELM_BUTTON_FOR_DATETIME_SPINNER_DECREASE");

   if (!style)
     style = elm_widget_style_get(obj);

   if (datetime_spinner_increase_style)
     sprintf(spinner_style_buffer, "spinner/increase/%s", datetime_spinner_increase_style);
   else if (datetime_spinner_text_style)
     sprintf(spinner_style_buffer, "spinner/%s", datetime_spinner_text_style);
   else if (datetime_spinner_decrease_style)
     sprintf(spinner_style_buffer, "spinner/decrease/%s", datetime_spinner_decrease_style);

   if (!datetime_spinner_increase_style && !datetime_spinner_text_style && !datetime_spinner_decrease_style)
     elm_layout_theme_set(obj, "button", "base", style);
   else
     elm_layout_theme_set(obj, "button", "base", spinner_style_buffer);
...
elm_button_add(Evas_Object *parent)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "elm_button_add: %p", parent);
