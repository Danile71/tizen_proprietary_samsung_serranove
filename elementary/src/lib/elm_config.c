...
#ifndef ELM_FEATURE_CHANGEABLE_UI
   // TIZEN_ONLY(20150813) call elm_win_access only when access_mode is changed
...
_elm_config_save(void)
{
   ...
   if (_elm_config->accel)
     {
        accel = eina_stringshare_add(_elm_config->accel);
   ...
   if (accel)
     {
        eina_stringshare_replace(&(_elm_config->accel), accel);
        ELM_SAFE_FREE(accel, eina_stringshare_del);
     }
