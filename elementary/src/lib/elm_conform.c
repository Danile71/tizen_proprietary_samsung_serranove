   ...
   if (part_type & ELM_CONFORMANT_CLIPBOARD_PART)
     {
        ...
        if ((!_conformant_part_geometry_get_from_env
               ("ILLUME_CB", &sx, &sy, &sw, &sh)) && (xwin))
          {
             ...
             //TIZEN_ONLY(20150630): Get clipboard geometry from xwin.
...
#define MSG_ID_INDICATOR_HIDE 0X10007
   ...
   if (msg_domain == MSG_DOMAIN_CONTROL_INDICATOR)
     {
        ...
        if (msg_id == MSG_ID_INDICATOR_HIDE)
          {
             if (msg_data_size != (int)sizeof(Indicator_Data_Animation)) return;

             Indicator_Data_Animation *anim_data = msg_data;
             DBG("[INDICATOR]Receive indicator hide:(app xwin=%x, msg data=%x) ", anim_data->xwin, elm_win_xwindow_get(sd->win));
             if (anim_data->xwin == elm_win_xwindow_get(sd->win))
...
_clipboard_state_change(Evas_Object *obj, Ecore_X_Event_Window_Property *ev)
{
   ...
   if (state == ECORE_X_ILLUME_CLIPBOARD_STATE_OFF)
     {
        //TIZEN_ONLY(20150630): Support clipboard on/off signal.
        elm_layout_signal_emit(obj, "elm,state,clipboard,off", "elm");
        //
        evas_object_size_hint_min_set(sd->clipboard, -1, 0);
        evas_object_size_hint_max_set(sd->clipboard, -1, 0);
   ...
   else if (state == ECORE_X_ILLUME_CLIPBOARD_STATE_ON)
     {
        ...
        _autoscroll_objects_update(obj);
        //TIZEN_ONLY(20150630): Support clipboard on/off signal.
        elm_layout_signal_emit(obj, "elm,state,clipboard,on", "elm");
        //
...
elm_conformant_add(Evas_Object *parent)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "elm_conformant_add: %p", parent);
...
_elm_conformant_eo_base_constructor(Eo *obj, Elm_Conformant_Data *sd)
{
   ...
   edje_object_signal_callback_add
     (wd->resize_obj, "elm,state,clipboard,disable", "",
     _on_clipboard_off, obj);
   edje_object_signal_callback_add
     (wd->resize_obj, "elm,state,clipboard,enable", "",
