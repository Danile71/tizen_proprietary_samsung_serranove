...
_reload_format(Evas_Object *obj)
{
   ...
   //Tizen Only: This should be removed when the logic of recursive parsing for _expanded_fmt_str_get() is applied.
   if (sd->user_format)
   ...
   else
     {
        ...
        char uk_UA_fmt[] = "%a, %d-%b-%Y %H:%M:%S %z";

        locale_tmp = setlocale(LC_TIME, NULL);
        if (locale_tmp)
          {
             strncpy(locale, locale_tmp, BUFFER_SIZE);

             p = strchr(locale, '.');
             if (p) *p = 0;

             if (!strcmp(locale, "uk_UA"))
               strncpy(dt_fmt, uk_UA_fmt, ELM_DATETIME_MAX_FORMAT_LEN);
             else
               _expand_format(dt_fmt);
          }
        else
...
_elm_datetime_evas_object_smart_add(Eo *obj, Elm_Datetime_Data *priv)
{
   Datetime_Field *field;
   int idx;
   Evas_Object *date_time;
   char *custom_style = NULL;
   ...
   date_time = elm_object_parent_widget_get(obj);
   custom_style = (char *)evas_object_data_get(date_time, "UXT_DATE_TIME_STYLE_ADD");

   if (!custom_style)
     custom_style = elm_widget_style_get(obj);

   if (!elm_layout_theme_set(obj, "datetime", "base", custom_style))
...
elm_datetime_add(Evas_Object *parent)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "elm_datetime_add: parent(%p)", parent);
