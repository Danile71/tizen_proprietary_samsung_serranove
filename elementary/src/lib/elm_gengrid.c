...
static void _item_del_pre_fx_process(Elm_Gen_Item *it);
...
_elm_gengrid_pan_evas_object_smart_calculate(Eo *obj EINA_UNUSED, Elm_Gengrid_Pan_Data *psd)
{
   ...
   if (psd->wsd->fx_mode)
     {
        if (_elm_gengrid_fx_capture(sd->obj, 1))
          _elm_gengrid_fx_play(sd->obj);
     }
#endif
}
...
//TIZEN_ONLY(20150825) : Added job for fixing index updation on item deletion.
   if (sd->pos_update_job)
     {
        ecore_job_del(sd->pos_update_job);
        sd->pos_update_job = NULL;
     }
   if (sd->items && !sd->pos_update_job)
...
_elm_gengrid_item_elm_widget_item_del_pre(Eo *eo_it EINA_UNUSED, Elm_Gen_Item *it)
{
   ...
   if ((sd->fx_mode) && (!sd->gengrid_clearing))
     {
        evas_object_repeat_events_set(sd->hit_rect, EINA_FALSE);
...
_item_del_post_fx_process(Elm_Gen_Item *it)
{
   ...
   if (it->itc->func.del)
     it->itc->func.del((void *)WIDGET_ITEM_DATA_GET(EO_OBJ(it)), WIDGET(it));
   if ((EO_OBJ(it) == sd->bring_in_it) && (sd->bring_in == EINA_TRUE))
     {
        sd->bring_in_it = NULL;
        sd->bring_in = EINA_FALSE;
     }

   if ((EO_OBJ(it) == sd->show_it) && (sd->show_region == EINA_TRUE))
     {
        sd->show_it = NULL;
        sd->show_region = EINA_FALSE;
     }
...
{
   Elm_Gengrid_Data *sd = GG_IT(it)->wsd;
   _elm_gengrid_fx_capture(sd->obj, 0);
...
//   _highlight_timer_disable(it);
   ...
   it->generation = sd->generation - 1; /* This means that the
                                                     * item is deleted */
   if (it->walking > 0) return;

   if (it->selected)
     sd->selected =
       eina_list_remove(sd->selected, it);
...
#if GENGRID_FX_SUPPORT
EOLIAN static void
_elm_gengrid_fx_mode_set(Eo *obj EINA_UNUSED, Elm_Gengrid_Data *sd, Eina_Bool mode)
{
   sd->fx_mode = mode;
}

EOLIAN static Eina_Bool
_elm_gengrid_fx_mode_get(Eo *obj EINA_UNUSED, Elm_Gengrid_Data *sd)
{
   return sd->fx_mode;
}
...
{
   ELM_GENGRID_ITEM_DATA_GET(item, it);
   ...
   pi->proxy = evas_object_image_filled_add
      (evas_object_evas_get(GG_IT(it)->wsd->obj));
   if (!pi->proxy)
     {
        free(pi);
        return NULL;
   ...
   pi->it = it;
   pi->num = it->item->num;
   evas_object_geometry_get(VIEW(it), &ox, &oy, &ow, &oh);
   pi->x = ox;
   pi->y = oy;
   pi->w = ow;
   pi->h =oh;
        ...
        if (it->realized)
          {
             ...
             pi = _elm_gengrid_proxy_item_new(EO_OBJ(it));
             if (!pi) continue;
             if (!level)
...
{
   if ((!pi) || (!pi->it) || (!pi->it->item)) return NULL;
...
_elm_gengrid_fx_items_make(Evas_Object *obj)
{
   ...
   EINA_LIST_FOREACH(sd->capture_after_items, l, pi)
     {
        fi = _elm_gengrid_fx_item_find(EO_OBJ(pi->it));
...
static void
_item_fx_op(Elm_Transit_Effect *data, Elm_Transit *transit EINA_UNUSED, double progress EINA_UNUSED)
{
   Elm_Gen_FX_Item *fi = data;
   Elm_Gengrid_Data *sd = GG_IT(fi->it)->wsd;
...
static void
_item_fx_done(Elm_Transit_Effect *data, Elm_Transit *transit EINA_UNUSED)
{
...
static void
_item_fx_del_cb(void *data, Elm_Transit *transit EINA_UNUSED)
{
   Elm_Gen_FX_Item *fx_done_it = data;
   Elm_Gen_Item *it = NULL;
   Proxy_Item *pi = NULL;
   ...
   if (!eina_list_count(sd->fx_items))
     {
        ...
        sd->pan_changed = EINA_TRUE;
        //FIX Me: this line never got hit so commenting.
