              ...
              void *info)
{
   ...
   if ((gesture->state != ELM_GESTURE_STATE_UNDEFINED) &&
     ...
     {
        Func_Data *cb_info;
        switch(gesture->g_type)
          {
             case ELM_GESTURE_ZOOM:
               {
                  Elm_Gesture_Zoom_Info *p = (Elm_Gesture_Zoom_Info *) info;
                  INF("ELM_GESTURE_ZOOM GestureState:%d, [%d,%d],radius:[%d],zoom:[%lf],momentum:[%lf]", gesture->state, p->x, p->y, p->radius, p->zoom, p->momentum);
               }
             break;
             case ELM_GESTURE_MOMENTUM:
               {
                  Elm_Gesture_Momentum_Info*p = (Elm_Gesture_Momentum_Info *) info;
                  INF("ELM_GESTURE_MOMENTUM GestureState:%d, 1[%d,%d], 2[%d,%d], time[%d,%d], m[%d,%d]", gesture->state, p->x1, p->y1, p->x2, p->y2, p->tx, p->ty, p->mx, p->my);
               }
             break;
             case ELM_GESTURE_N_TAPS:
             case ELM_GESTURE_N_DOUBLE_TAPS:
               {
                  Elm_Gesture_Taps_Info*p = (Elm_Gesture_Taps_Info *) info;
                  INF("ELM_GESTURE_N_TAPS GestureState:%d, [%d,%d],fingerCount:[%d]", gesture->state, p->x, p->y, p->n);
               }
             break;
             case ELM_GESTURE_N_LONG_TAPS:
               {
                  Elm_Gesture_Taps_Info*p = (Elm_Gesture_Taps_Info *) info;
                  INF("ELM_GESTURE_N_LONG_TAPS GestureState:%d, [%d,%d],fingerCount:[%d]", gesture->state, p->x, p->y, p->n);
               }
             break;
             case ELM_GESTURE_N_FLICKS:
               {
                  Elm_Gesture_Line_Info*p = (Elm_Gesture_Line_Info *) info;
                  INF("ELM_GESTURE_N_FLICKS GestureState:%d, 1:[%d,%d], 2:[%d,%d], time:[%u,%u], momentum:[%d,%d], fingerCount:[%d]", gesture->state, p->momentum.x1, p->momentum.y1, p->momentum.x2, p->momentum.y2,
                        p->momentum.tx, p->momentum.ty, p->momentum.mx, p->momentum.my, p->momentum.n);
               }
             break;
             case ELM_GESTURE_N_LINES:
               {
                  Elm_Gesture_Line_Info*p = (Elm_Gesture_Line_Info *) info;
                  INF("ELM_GESTURE_N_LINES GestureState:%d, 1:[%d,%d], 2:[%d,%d], time:[%u,%u], momentum:[%d,%d], fingerCount:[%d]", gesture->state, p->momentum.x1, p->momentum.y1, p->momentum.x2, p->momentum.y2,
                        p->momentum.tx, p->momentum.ty, p->momentum.mx, p->momentum.my, p->momentum.n);
               }
             break;
          }
        ...
        Evas_Coord w)
{
   ...
   if ((abs(xx1-xx2) > w) || abs(yy1-yy2) > w)
     return EINA_FALSE;
...
EAPI void
_elm_gesture_layer_zoom_threshold_set(Evas_Object *obj, Evas_Coord zoom_threshold)
{
   ELM_GESTURE_LAYER_CHECK(obj);
   ELM_GESTURE_LAYER_DATA_GET(obj, sd);
  ...
  sd->zoom_threshold = zoom_threshold;
}

EAPI Evas_Coord
_elm_gesture_layer_zoom_threshold_get(const Evas_Object *obj)
{
   ELM_GESTURE_LAYER_CHECK(obj) 0.0;
   ELM_GESTURE_LAYER_DATA_GET(obj, sd);
