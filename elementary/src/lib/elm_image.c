   ...
   if (sd->edje)
     {
        ...
        if (!ok)
          {
             SECURE_ERR("failed to open edje file '%s', group '%s': %s", file, key,
   ...
   else
     {
        ...
        if (!ok)
          {
             SECURE_ERR("failed to open image file '%s', key '%s': %s", file, key,
...
#ifdef ELM_FEATURE_CHANGEABLE_UI
        _elm_widget_color_overlay_set(obj, sd->img);
        _elm_widget_font_overlay_set(obj, sd->img);
#endif
     }
        ...
        if (f)
          {
             if (!edje_object_mmap_set(sd->img, f, group))
               {
                  SECURE_ERR("failed to set edje file '%s', group '%s': %s", file, group,
                      edje_load_error_str(edje_object_load_error_get(sd->img)));
...
elm_image_add(Evas_Object *parent)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "elm_image_add: %p", parent);
                                   ...
                                   const char *file, const Eina_File *f, const char *key, Eina_Bool *ret)
{
   ...
   if (evas_object_image_load_error_get(sd->img) != EVAS_LOAD_ERROR_NONE)
     {
        SECURE_ERR("Things are going bad for '%s' (%p)", file, sd->img);
...
{
   //TIZEN_ONLY(20150826)
   //Now There is some issue about preload with elm_image on upstream.
   //We should apply changes to here when the issue is fixed on upstream
   if (sd->edje || !sd->img) return;

   if (disable && sd->preloading)
    {
       sd->preloading = EINA_FALSE;
       evas_object_image_preload(sd->img, TRUE);
             if (sd->show) evas_object_show(sd->img);
             ELM_SAFE_FREE(sd->prev_img, evas_object_del);
          }
   else if(!disable)
    {
       sd->preloading = EINA_TRUE;
       evas_object_image_preload(sd->img, FALSE);
    }
