...
_elm_interface_scrollable_wanted_region_set(Eo *obj, Elm_Scrollable_Smart_Interface_Data *sid, Evas_Coord x EINA_UNUSED, Evas_Coord y EINA_UNUSED)
{
   ...
   eo_do(obj, elm_interface_scrollable_content_region_set(wx, wy, ww, wh));
}
//
                       ...
                       if ((_elm_config->thumbscroll_friction > 0.0) &&
                           (vel > _elm_config->thumbscroll_momentum_threshold))
                         {
                            ...
                            if (sid->cb_func.scroll_position_get)
                              {
                                //for __elm_scroll_scroll_position_get_cb_set (UXT)
                                Evas_Coord pos_x, pos_y;
                            ...
                            if (sid->is_page_flick && _paging_is_enabled(sid))
                              {
                                 ...
                                 eo_do(sid->obj, elm_interface_scrollable_content_pos_get(&pos_x, &pos_y));
                           ...
                           void *event_info EINA_UNUSED)
{
   ...
   eo_do(sid->pan_obj, elm_obj_pan_content_size_get(&w, &h));
   eo_do((Eo *)sid->obj, elm_interface_scrollable_content_viewport_geometry_get(NULL, NULL, &vw, &vh));
...
EOLIAN static void
_elm_interface_scrollable_auto_scroll_set(Eo *obj EINA_UNUSED, Elm_Scrollable_Smart_Interface_Data *sid, Eina_Bool enable)
{
	sid->is_auto_scroll = enable;
}

EOLIAN static Eina_Bool
_elm_interface_scrollable_auto_scroll_get(Eo *obj EINA_UNUSED, Elm_Scrollable_Smart_Interface_Data *sid)
{
	return sid->is_auto_scroll;
}

EOLIAN static void
_elm_interface_scrollable_page_flick_set(Eo *obj EINA_UNUSED, Elm_Scrollable_Smart_Interface_Data *sid, Eina_Bool enable)
{
	sid->is_page_flick = enable;
}

EOLIAN static Eina_Bool
_elm_interface_scrollable_page_flick_get(Eo *obj EINA_UNUSED, Elm_Scrollable_Smart_Interface_Data *sid)
{
	return sid->is_page_flick;
}

EOLIAN static void
_elm_interface_scrollable_rotary_event_set(Eo *obj EINA_UNUSED, Elm_Scrollable_Smart_Interface_Data *sid, Eina_Bool enable)
{
	sid->is_rotary_event_enable = enable;
}

EOLIAN static Eina_Bool
_elm_interface_scrollable_rotary_event_get(Eo *obj EINA_UNUSED, Elm_Scrollable_Smart_Interface_Data *sid)
{
	return sid->is_rotary_event_enable;
}

EOLIAN static void
_elm_interface_scrollable_origin_set(Eo *obj EINA_UNUSED, Elm_Scrollable_Smart_Interface_Data *sid, Eina_Bool origin_h, Eina_Bool origin_v)
{
   Evas_Coord x, y;

   sid->origin_h = origin_h;
   sid->origin_v = origin_v;
...
EOLIAN static void
_elm_interface_scrollable_origin_get(Eo *obj EINA_UNUSED, Elm_Scrollable_Smart_Interface_Data *sid, Eina_Bool *origin_h, Eina_Bool *origin_v)
{
   if (origin_h) *origin_h = sid->origin_h;
   if (origin_v) *origin_v = sid->origin_v;
}


//for UXT
EAPI void
__elm_interface_scrollable_auto_scroll_set(Eo *obj, Elm_Scrollable_Smart_Interface_Data *sid, Eina_Bool enable)
{
	eo_do(obj, elm_interface_scrollable_auto_scroll_set(enable));
}

EAPI Eina_Bool
__elm_interface_scrollable_auto_scroll_get(Eo *obj, Elm_Scrollable_Smart_Interface_Data *sid)
{
	Eina_Bool enable = EINA_FALSE;
	eo_do(obj, enable = elm_interface_scrollable_auto_scroll_get());

	return enable;
}

EAPI void
__elm_interface_scrollable_page_flick_set(Eo *obj, Elm_Scrollable_Smart_Interface_Data *sid, Eina_Bool enable)
{
	eo_do(obj, elm_interface_scrollable_page_flick_set(enable));
}

EAPI Eina_Bool
__elm_interface_scrollable_page_flick_get(Eo *obj, Elm_Scrollable_Smart_Interface_Data *sid)
{
	Eina_Bool enable = EINA_FALSE;
	eo_do(obj, enable = elm_interface_scrollable_page_flick_get());

	return enable;
}

EAPI void
__elm_interface_scrollable_rotary_event_set(Eo *obj, Elm_Scrollable_Smart_Interface_Data *sid, Eina_Bool enable)
{
	eo_do(obj, elm_interface_scrollable_rotary_event_set(enable));
}

EAPI Eina_Bool
__elm_interface_scrollable_rotary_event_get(Eo *obj, Elm_Scrollable_Smart_Interface_Data *sid)
{
	Eina_Bool enable = EINA_FALSE;
	eo_do(obj, enable = elm_interface_scrollable_rotary_event_get());

	return enable;
}

EAPI void
__elm_scroll_scroll_position_get_cb_set(Eo *obj, Elm_Scrollable_Smart_Interface_Data *sid, void (*scroll_position_get)(Evas_Object *obj, Evas_Coord *x, Evas_Coord *y))
{
   sid->cb_func.scroll_position_get = scroll_position_get;
}

EAPI void
__elm_interface_scrollable_origin_set(Eo *obj, Elm_Scrollable_Smart_Interface_Data *sid, Eina_Bool origin_h, Eina_Bool origin_v)
{
	eo_do(obj, elm_interface_scrollable_origin_set(origin_h, origin_v));
}

EAPI void
__elm_interface_scrollable_origin_get(Eo *obj, Elm_Scrollable_Smart_Interface_Data *sid, Eina_Bool *origin_h, Eina_Bool *origin_v)
{
	eo_do(obj, elm_interface_scrollable_origin_get(origin_h, origin_v));
}


EAPI Elm_Scrollable_Smart_Interface_Data*
__elm_scrollable_smart_interface_data_get(Eo *obj)
{
	Elm_Scrollable_Smart_Interface_Data *ptr = (!eo_isa(obj, MY_SCROLLABLE_INTERFACE) ? NULL : eo_data_scope_get(obj, MY_SCROLLABLE_INTERFACE));
	if (!ptr)
	  { 													   \
		 return NULL;
	  }
