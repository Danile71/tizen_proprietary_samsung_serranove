...
_elm_layout_theme_set(Eo *obj, Elm_Layout_Smart_Data *sd, const char *klass, const char *group, const char *style)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "_elm_layout_theme_set: obj(%p), type(%s), klass(%s), group(%s), style(%s)", obj, elm_widget_type_get(obj), klass, group, style);

   eina_stringshare_replace(&(sd->klass), klass);
   eina_stringshare_replace(&(sd->group), group);
   eina_stringshare_replace(&(wd->style), style);

   ret = _elm_layout_theme_internal(obj, sd);
...
elm_layout_file_set(Eo *obj, const char *file, const char *group)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "elm_layout_file_set: (%p) file(%s) (%s)", obj, file, group);

#ifdef ELM_FEATURE_CHANGEABLE_UI
   ELM_WIDGET_DATA_GET_OR_RETURN(obj, wd, EINA_FALSE);

   _elm_widget_color_overlay_set(obj, wd->resize_obj);
   _elm_widget_font_overlay_set(obj, wd->resize_obj);
#endif

   ret = eo_do((Eo *) obj, efl_file_set(file, group));
