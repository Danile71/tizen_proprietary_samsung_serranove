         ...
         char **argv)
{
   ...
   // TIZEN_ONLY(20150919): Initialize fontconfig when elementary is initialized.
   evas_font_init();
   //
...
elm_exit(void)
{
   traceBegin(TTRACE_TAG_VIEW, "elm_exit");
                     ...
                     const char  *style)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "elm_object_style_set: (%p) (%s)", obj, style);

   ret = elm_widget_style_set(obj, style);
