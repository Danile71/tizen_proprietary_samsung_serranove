...
{
   Evas_Coord x, y, w, h;
   //TIZEN_ONLY (20150903): Use focus auto scroll mode
   Eina_Bool auto_scroll;

   elm_widget_show_region_get(content_obj, &x, &y, &w, &h);
   //if (eo_do(data, elm_interface_scrollable_auto_scroll_get()))
   //   eo_do(data, elm_interface_scrollable_content_region_show(x, y, w, h));

   eo_do(data, auto_scroll = elm_interface_scrollable_auto_scroll_get());
        ...
        switch (_elm_config->focus_autoscroll_mode)
          {
           ...
           case ELM_FOCUS_AUTOSCROLL_MODE_BRING_IN:
              eo_do(data, elm_interface_scrollable_region_bring_in(x, y, w, h));
              break;
           default:
              break;
          }
...
EAPI void
elm_scroller_origin_reverse_set(Evas_Object *obj, Eina_Bool origin_x, Eina_Bool origin_y)
{
}

EAPI void
elm_scroller_origin_reverse_get(Evas_Object *obj, Eina_Bool *origin_x, Eina_Bool *origin_y)
{
}
