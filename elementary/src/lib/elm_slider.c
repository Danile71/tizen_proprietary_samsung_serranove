...
_indicator_set(Evas_Object *obj)
{
   ...
   if (sd->indicator_format_func)
   ...
   else if (sd->indicator)
     {
        ...
        //TIZEN ONLY(20150814):when format is %1.0f, and sd->value is in between
        //-0.0 to -0.5 then, snprintf(buf, sizeof(buf), "%1.0f",sd->value) will write -0 on buf instead of 0.
