                        ...
                        const char *source EINA_UNUSED)
{
    ...
    if (edje)
     {
        edje_object_signal_emit(edje, "elm,state,changeable_ui,on", "elm");
     }
}
   ...
   if (sd->shrink_mode == ELM_TOOLBAR_SHRINK_MENU)
     {
        ...
        if (sd->more_item)
           evas_object_geometry_get(sd->VIEW(more_item), NULL, NULL, &more_w, &more_h);
        ...
        if (((sd->vertical) && (ih > vh)) ||
            ((!sd->vertical) && (iw > vw)) || more)
          {
             ...
             if(sd->more_item) {
                ...
                menu = elm_toolbar_item_menu_get(EO_OBJ(sd->more_item));
             }
             EINA_INLIST_FOREACH(sd->items, it)
               {
                  if (!it->prio.visible)
                    {
                       if (it->separator)
                       ...
                       else
                         {
                            ...
                            if(menu)
                               menu_it = elm_menu_item_add(menu, NULL, it->icon_str, it->label, _elm_toolbar_item_menu_cb, it);
...
_resizing_eval_item(Elm_Toolbar_Item_Data *it)
{
   ...
   evas_object_geometry_get(obj, &x, &y, NULL, &h);
   traceBegin(TTRACE_TAG_VIEW, "_resizing_eval_item: %p h(%d)", it, h);
   evas_object_move(sd->more, x, y + h);
...
_item_select(Elm_Toolbar_Item_Data *it)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "_item_select: %p", it);
                  ...
                  if (sd->more_item == it)
                  ...
                  else
                    {
                       if (it->in_box != sd->bx)
                         {
                            if (sd->more_item) {
                               elm_layout_signal_emit(sd->VIEW(more_item), "elm,state,selected", "elm");
                               elm_widget_signal_emit(sd->more_item->icon, "elm,state,selected", "elm");
                            }
                         }
                       else
                         {
                            if (sd->more_item) {
                               elm_layout_signal_emit(sd->VIEW(more_item), "elm,state,unselected", "elm");
                               elm_widget_signal_emit(sd->more_item->icon, "elm,state,unselected", "elm");
                            }
                         }
                       elm_layout_signal_emit
                         (sd->more, "elm,state,close", "elm");
                 ...
                 Eina_Bool change_style)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "_item_theme_hook: %p", obj);
...
_sizing_eval(Evas_Object *obj)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "_sizing_eval: %p", obj);
...
_elm_toolbar_elm_widget_theme_apply(Eo *obj, Elm_Toolbar_Data *sd)
{
   ...
   edje_object_signal_callback_add(wd->resize_obj, "color_class,set", "B0514", _color_class_changed_cb, wd->resize_obj);
       ...
       EINA_LIST_FOREACH(eelist, list, ee)
         {
             ...
             if (changeable)
                {
                   edje_object_signal_emit(wd->resize_obj, "elm,state,changeable_ui,on", "elm");
                   break;
                }
         }
   ...
   EINA_INLIST_FOREACH(sd->items, it)
     _item_theme_hook(obj, it, scale, sd->icon_size, EINA_TRUE);

   if (sd->more_item)
     _item_theme_hook(obj, sd->more_item, scale, sd->icon_size, EINA_TRUE);
                       ...
                       Evas_Object *content)
{
   ...
   scale = elm_widget_scale_get(obj);
   _item_theme_hook(obj, item, scale, sd->icon_size, EINA_TRUE);
}
...
_elm_toolbar_item_eo_base_destructor(Eo *eo_item, Elm_Toolbar_Item_Data *item)
{
   ...
   evas_object_box_remove(sd->bx,VIEW(item));
          ...
          const void *data)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "_item_new: (%p) icon(%s) label(%s)", obj, icon, label);
...
_elm_toolbar_evas_object_smart_add(Eo *obj, Elm_Toolbar_Data *priv)
{
   ...
   toolbar = elm_object_parent_widget_get(obj);
   
   style = evas_object_data_get(toolbar, "UXT_TOOLBAR_ADD");
   ...
   elm_widget_theme_object_set(obj, edje, "toolbar", "base", style);
   ...
   edje_object_signal_callback_add(edje, "color_class,set", "B0514", _color_class_changed_cb, edje);
...
elm_toolbar_add(Evas_Object *parent)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "elm_toolbar_add: %p", parent);
...
_elm_toolbar_item_append(Eo *obj, Elm_Toolbar_Data *sd, const char *icon, const char *label, Evas_Smart_Cb func, const void *data)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "_elm_toolbar_item_append: %p", obj);
   ...
   _item_theme_hook(obj, it, scale, sd->icon_size, EINA_FALSE);
   _sizing_eval(obj);
   sd->item_count++;
...
_elm_toolbar_item_prepend(Eo *obj, Elm_Toolbar_Data *sd, const char *icon, const char *label, Evas_Smart_Cb func, const void *data)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "_elm_toolbar_item_prepend: %p", obj);
...
_elm_toolbar_item_insert_before(Eo *obj, Elm_Toolbar_Data *sd, Elm_Object_Item *eo_before, const char *icon, const char *label, Evas_Smart_Cb func, const void *data)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "_elm_toolbar_item_before: %p", obj);
...
_elm_toolbar_item_insert_after(Eo *obj, Elm_Toolbar_Data *sd, Elm_Object_Item *eo_after, const char *icon, const char *label, Evas_Smart_Cb func, const void *data)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "_elm_toolbar_item_after: %p", obj);
...
_elm_toolbar_shrink_mode_set(Eo *obj, Elm_Toolbar_Data *sd, Elm_Toolbar_Shrink_Mode shrink_mode)
{
   ...
   traceBegin(TTRACE_TAG_VIEW, "_elm_toolbar_shrink_mode_set: (%p) mode(%d)", obj, shrink_mode);
   style = evas_object_data_get(obj, "UXT_TOOLBAR_ADD");
