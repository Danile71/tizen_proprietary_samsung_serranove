   ...
   if ((ch_withdrawn) || (ch_iconified))
     {
        if (sd->withdrawn)
          ...
          {
             static Eina_Bool isFirstWindowShow = EINA_FALSE;
             if (!isFirstWindowShow)
               {
                  _elm_win_paragraph_direction_set();
                  isFirstWindowShow = EINA_TRUE;
               }
                        ...
                        void *event)
{
   ...
   /////////////////////////////////////////////////////////////////////////
#ifdef UI_ANALYZER_ENABLE
   if (e->message_type == ECORE_X_ATOM_SDB_SERVER_CONNECT)
   {
   ...
   else if (e->message_type == ECORE_X_ATOM_SDB_SERVER_DISCONNECT)
      {
        if (((unsigned)e->data.l[0] == 1) &&
            ((unsigned)e->data.l[1] == sd->x.xwin))
          _ui_analyzer_mod_shutdown(data);
      }
#endif
