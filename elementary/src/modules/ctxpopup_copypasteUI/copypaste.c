...
#ifdef HAVE_ELEMENTARY_APPSVC
static char * _remove_tags(const char *str);
static Eina_Bool
_quick_memo_text_send()
{
        ...
        if (selection)
          {
             ...
             if (str)
               {
                  appsvc_add_data(b, "http://tizen.org/appcontrol/data/text", str);
               }
          }
     }
...
static void
_quick_memo(void *data, Evas_Object *obj, void *event_info EINA_UNUSED)
{
   if (!_quick_memo_text_send())
     {
        DMSG("Could not send selected text to Memo service");
     }
   _ctxpopup_hide(obj);
...
#ifdef HAVE_ELEMENTARY_APPSVC
                        if (ext_mod->quick_memo_enabled)
                          {
                             added_item = elm_ctxpopup_item_append(ext_mod->popup, dgettext("elementary", "IDS_MEMO_HEADER_QUICK_MEMO"),
                                                         icon, _quick_memo, obj);
                          }
#endif
...
{
   if (!obj || !ext_mod || !ext_mod->popup || !ext_mod->popup_showing || !evas_object_visible_get(ext_mod->popup) ||
       (obj != ext_mod->caller))
     return;
