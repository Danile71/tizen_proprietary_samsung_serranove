...
static OMX_ERRORTYPE gst_omx_port_deallocate_buffers_unlocked (GstOMXPort * port);
...
gst_omx_core_acquire (const gchar * filename)
{
  ...
  G_LOCK (core_handles);
  if (!core_handles)
    core_handles = g_hash_table_new_full (g_str_hash, g_str_equal, g_free, NULL);
  ...
  if (!core) {
    ...
    if (g_str_has_suffix (filename, "vc/lib/libopenmaxil.so")) {
      ...
      bcm_host_filename = g_build_filename (bcm_host_path, "libbcm_host.so", NULL);

      bcm_host_module = g_module_open (bcm_host_filename, G_MODULE_BIND_LAZY | G_MODULE_BIND_LOCAL);
      ...
      if (!bcm_host_module) {
        ...
        bcm_host_module = g_module_open ("libbcm_host.so", G_MODULE_BIND_LAZY | G_MODULE_BIND_LOCAL);
        if (!bcm_host_module) {
      ...
      if (!g_module_symbol (bcm_host_module, "bcm_host_init", (gpointer *) & bcm_host_init)) {
    ...
    core->module = g_module_open (filename, G_MODULE_BIND_LAZY | G_MODULE_BIND_LOCAL);
    if (!core->module)
      goto load_failed;

    if (!g_module_symbol (core->module, "OMX_Init", (gpointer *) & core->init))
      goto symbol_error;
    if (!g_module_symbol (core->module, "OMX_Deinit", (gpointer *) & core->deinit))
      goto symbol_error;
    if (!g_module_symbol (core->module, "OMX_GetHandle", (gpointer *) & core->get_handle))
      goto symbol_error;
    if (!g_module_symbol (core->module, "OMX_FreeHandle", (gpointer *) & core->free_handle))
      goto symbol_error;
    if (!g_module_symbol (core->module, "OMX_SetupTunnel", (gpointer *) & core->setup_tunnel))
      goto symbol_error;
  ...
  while ((msg = g_queue_pop_head (&comp->messages))) {
    ...
    switch (msg->type) {
      case GST_OMX_MESSAGE_STATE_SET:{
        GST_INFO_OBJECT (comp->parent, "%s state change to %s finished", comp->name, gst_omx_state_to_string (msg->content.state_set.state));
      ...
      case GST_OMX_MESSAGE_FLUSH:{
        ...
        GST_DEBUG_OBJECT (comp->parent, "%s port %u flushed", comp->name, port->index);

        if (port->flushing) {
          port->flushed = TRUE;
        } else {
          GST_ERROR_OBJECT (comp->parent, "%s port %u was not flushing", comp->name, port->index);
        }
      ...
      case GST_OMX_MESSAGE_ERROR:{
        ...
        GST_ERROR_OBJECT (comp->parent, "%s got error: %s (0x%08x)", comp->name, gst_omx_error_to_string (error), error);
      ...
      case GST_OMX_MESSAGE_PORT_ENABLE:{
        ...
        GST_DEBUG_OBJECT (comp->parent, "%s port %u %s", comp->name, port->index, (enable ? "enabled" : "disabled"));
      ...
      case GST_OMX_MESSAGE_PORT_SETTINGS_CHANGED:{
        ...
        if(index == OMX_IndexConfigCommonOutputCrop)
        {
            GST_DEBUG_OBJECT (comp->parent, "%s settings changed (port 1) OutputCrop", comp->name);

            comp->crop_changed = TRUE;
            index = 1; //set as outport index
        ...
        GST_DEBUG_OBJECT (comp->parent, "%s settings changed (port %lu)", comp->name, index);
        ...
        for (k = outports; k; k = k->next) {
          ...
          if (!found)
            comp->pending_reconfigure_outports = g_list_prepend (comp->pending_reconfigure_outports, k->data);
        }
      ...
      case GST_OMX_MESSAGE_BUFFER_FLAG:{
        ...
        GST_DEBUG_OBJECT (comp->parent, "%s port %u got buffer flags 0x%08lx", comp->name, port->index, flags);
        if ((flags & OMX_BUFFERFLAG_EOS) && port->port_def.eDir == OMX_DirOutput)
          port->eos = TRUE;
      ...
      case GST_OMX_MESSAGE_BUFFER_DONE:{
        ...
        if (msg->content.buffer_done.empty) {
          ...
          GST_LOG_OBJECT (comp->parent, "%s port %u emptied buffer %p (%p)", comp->name, port->index, buf, buf->omx_buf->pBuffer);
          ...
          GST_LOG_OBJECT (comp->parent, "%s port %u filled buffer %p (%p)", comp->name, port->index, buf, buf->omx_buf->pBuffer);

          if ((buf->omx_buf->nFlags & OMX_BUFFERFLAG_EOS) && port->port_def.eDir == OMX_DirOutput)
            port->eos = TRUE;
        }
        buf->used = FALSE;
    ...
    OMX_U32 nData1, OMX_U32 nData2, OMX_PTR pEventData)
{
  ...
  switch (eEvent) {
    case OMX_EventCmdComplete:
    {
      ...
      GST_DEBUG_OBJECT (comp->parent, "%s %s command complete (%d)", comp->name, gst_omx_command_to_string (cmd), cmd);
      ...
      switch (cmd) {
        case OMX_CommandStateSet:{
          ...
          GST_DEBUG_OBJECT (comp->parent, "%s state change to %s finished", comp->name, gst_omx_state_to_string (msg->content.state_set.state));
        ...
        case OMX_CommandFlush:{
          ...
          GST_DEBUG_OBJECT (comp->parent, "%s port %lu flushed", comp->name, msg->content.flush.port);
        ...
        case OMX_CommandPortDisable:{
          ...
          GST_DEBUG_OBJECT (comp->parent, "%s port %lu %s", comp->name, msg->content.port_enable.port, (msg->content.port_enable.enable ? "enabled" : "disabled"));
    ...
    case OMX_EventError:
    {
      ...
      GST_ERROR_OBJECT (comp->parent, "%s got error: %s (0x%08x)", comp->name, gst_omx_error_to_string (msg->content.error.error), msg->content.error.error);
    ...
    case OMX_EventPortSettingsChanged:
    {
      if (nData2 == 0 || nData2 == OMX_IndexParamPortDefinition) {
        ...
        if (!(comp->hacks & GST_OMX_HACK_EVENT_PORT_SETTINGS_CHANGED_NDATA_PARAMETER_SWAP)) {
          index = nData1;
        } else {
          index = nData2;
        }

        if (index == 0 && (comp->hacks & GST_OMX_HACK_EVENT_PORT_SETTINGS_CHANGED_PORT_0_TO_1))
          index = 1;

        msg->type = GST_OMX_MESSAGE_PORT_SETTINGS_CHANGED;
        msg->content.port_settings_changed.port = index;
        GST_DEBUG_OBJECT (comp->parent, "%s settings changed (port index: %lu)", comp->name, msg->content.port_settings_changed.port);

        gst_omx_component_send_message (comp, msg);
      } else if (nData2 == OMX_IndexConfigCommonOutputCrop) {
        GST_WARNING_OBJECT (comp->parent, "OMX_IndexConfigCommonOutputCrop : Set.");
        GstOMXMessage *msg = g_slice_new (GstOMXMessage);

        msg->type = GST_OMX_MESSAGE_PORT_SETTINGS_CHANGED;
        //FIXME : If it is ok ?
        msg->content.port_settings_changed.port = OMX_IndexConfigCommonOutputCrop;
        gst_omx_component_send_message (comp, msg);
      } else {
        GST_WARNING_OBJECT (comp->parent, "Unhandled index 0x%x", nData2);
      }
      break;
    }
    case OMX_EventBufferFlag:{
      ...
      GST_DEBUG_OBJECT (comp->parent, "%s port %lu got buffer flags 0x%08lx", comp->name, msg->content.buffer_flag.port, msg->content.buffer_flag.flags);
    ...
    default:
      GST_DEBUG_OBJECT (comp->parent, "%s unknown event 0x%08x", comp->name, eEvent);
      break;
  }
    ...
    OMX_BUFFERHEADERTYPE * pBuffer)
{
  ...
  GST_DEBUG ("EmptyBufferDone is invoked");
  ...
  GST_LOG_OBJECT (comp->parent, "%s port %u emptied buffer %p (%p)", comp->name, buf->port->index, buf, buf->omx_buf->pBuffer);
    ...
    OMX_BUFFERHEADERTYPE * pBuffer)
{
  ...
  GST_DEBUG ("FillBufferDone is invoked");
#ifdef USE_OMX_TARGET_SPRD
  // Last buffer header just before EOS is returned as NULL in SPRD.
  // Should be rechecked why is it so.
  if (!pBuffer) {
    GST_ERROR ("Omx Buffer header shouldn't be NULL");
    return OMX_ErrorNone;
  }
#endif
  ...
  pBuffer->output_width = buf->port->cropped_width;
  pBuffer->output_height = buf->port->cropped_heigth;
#else
  pBuffer->output_width = buf->port->port_def.format.video.nFrameWidth;
  pBuffer->output_height = buf->port->port_def.format.video.nFrameHeight;
#endif
  ...
  GST_LOG_OBJECT (comp->parent, "%s port %u filled buffer %p (%p)", comp->name, buf->port->index, buf, buf->omx_buf->pBuffer);
    ...
    const gchar * component_name, const gchar * component_role, guint64 hacks)
{
  ...
  err = core->get_handle (&comp->handle, (OMX_STRING) component_name, comp, &callbacks);
  if (err != OMX_ErrorNone) {
    GST_ERROR_OBJECT (parent, "Failed to get component handle '%s' from core '%s': 0x%08x", component_name, core_name, err);
    gst_omx_core_release (core);
    g_slice_free (GstOMXComponent, comp);
    return NULL;
  }
  GST_DEBUG_OBJECT (parent, "Successfully got component handle %p (%s) from core '%s'", comp->handle, component_name, core_name);
  ...
  if (component_role && !(hacks & GST_OMX_HACK_NO_COMPONENT_ROLE)) {
    ...
    err = gst_omx_component_set_parameter (comp, OMX_IndexParamStandardComponentRole, &param);

    GST_DEBUG_OBJECT (parent, "Setting component role to '%s': %s (0x%08x)", component_role, gst_omx_error_to_string (err), err);
  ...
  if (strncmp(component_name+4, "SEC", 3) == 0)
  {
    core->component_vendor = GST_OMX_VENDOR_SLSI_SEC;

    if(strstr(component_name, "secure") != NULL) {
        GST_WARNING_OBJECT (parent, "[tz] This is secure omx");
        core->secure = TRUE;
    }
    GST_DEBUG_OBJECT (parent, "SEC %s component is loading", component_name);
  }
  else if (strncmp(component_name+4, "Exynos", 6) == 0)
  {
    core->component_vendor = GST_OMX_VENDOR_SLSI_EXYNOS;

    if(strstr(component_name, "secure") != NULL) {
        GST_INFO_OBJECT (parent, "[tz] This is secure omx");
        core->secure = TRUE;
    }
    GST_DEBUG_OBJECT (parent, "EXYNOS %s component is loading", component_name);
  }
  else if (strncmp(component_name+4, "qcom", 4) == 0)
  {
    core->component_vendor = GST_OMX_VENDOR_QCT;

    if(strstr(component_name, "secure") != NULL) {
        GST_WARNING_OBJECT (parent, "[tz] This is secure omx");
        core->secure = TRUE;
    }
    GST_DEBUG_OBJECT (parent, "QCT %s component is loading", component_name);
  }
  else if (strncmp(component_name+4, "SPRD", 4) == 0)
  {
    core->component_vendor = GST_OMX_VENDOR_SPRD;
    GST_DEBUG_OBJECT (parent, "SPRD %s component is loading", component_name);
  }
  else
  {
    core->component_vendor = GST_OMX_VENDOR_DEFAULT;
    GST_DEBUG_OBJECT (parent, "Default %s component is loading", component_name);
  }
...
gst_omx_component_set_state (GstOMXComponent * comp, OMX_STATETYPE state)
{
  ...
  if ((err = comp->last_error) != OMX_ErrorNone && state > old_state) {
    GST_ERROR_OBJECT (comp->parent, "Component %s in error state: %s (0x%08x)", comp->name, gst_omx_error_to_string (err), err);
    goto done;
  }

  if (old_state == state || comp->pending_state == state) {
    GST_DEBUG_OBJECT (comp->parent, "Component %s already in state %s", comp->name, gst_omx_state_to_string (state));
    goto done;
  }
  ...
  /* Reset some things */
  if ((old_state == OMX_StateExecuting || old_state == OMX_StatePause) && state < old_state) {
  ...
  if (err != OMX_ErrorNone) {
    GST_ERROR_OBJECT (comp->parent, "Error setting %s state from %d to %d: %s (0x%08x)"
            , comp->name, old_state, state, gst_omx_error_to_string (err), err);
  }
  return err;
}
...
gst_omx_component_get_state (GstOMXComponent * comp, GstClockTime timeout)
{
  ...
  if (comp->last_error != OMX_ErrorNone) {
    GST_ERROR_OBJECT (comp->parent, "Component %s in error state: %s (0x%08x)",
            comp->name, gst_omx_error_to_string (comp->last_error), comp->last_error);
  ...
  if (timeout != GST_CLOCK_TIME_NONE) {
    ...
    GST_DEBUG_OBJECT (comp->parent, "%s waiting for %" G_GINT64_FORMAT "us", comp->name, add);
  } else {
    GST_DEBUG_OBJECT (comp->parent, "%s waiting for signal", comp->name);
  }

  gst_omx_component_handle_messages (comp);
  while (signalled && comp->last_error == OMX_ErrorNone && comp->pending_state != OMX_StateInvalid) {
    g_mutex_lock (&comp->messages_lock);
    g_mutex_unlock (&comp->lock);
    if (!g_queue_is_empty (&comp->messages)) {
      ...
      signalled = g_cond_wait_until (&comp->messages_cond, &comp->messages_lock, wait_until);
    }
    g_mutex_unlock (&comp->messages_lock);
    g_mutex_lock (&comp->lock);
    if (signalled)
  ...
  if (signalled) {
    if (comp->last_error != OMX_ErrorNone) {
      GST_ERROR_OBJECT (comp->parent, "%s got error while waiting for state change: %s (0x%08x)",
                  comp->name, gst_omx_error_to_string (comp->last_error), comp->last_error);
      ret = OMX_StateInvalid;
    ...
    ret = OMX_StateInvalid;
    GST_WARNING_OBJECT (comp->parent, "%s timeout while waiting for state change", comp->name);
  }

done:
  g_mutex_unlock (&comp->lock);

  GST_DEBUG_OBJECT (comp->parent, "%s returning state %s", comp->name, gst_omx_state_to_string (ret));

  return ret;
}

gboolean
gst_omx_drm_init(GstOMXPort * port)
{
  Display *dpy;
  int eventBase = 0;
  int errorBase = 0;
  ...
  GST_INFO_OBJECT (port->comp->parent, "gst_omx_drm_init enter");

  dpy = XOpenDisplay(0);

  /* DRI2 */
  if (!DRI2QueryExtension(dpy, &eventBase, &errorBase)) {
    GST_ERROR_OBJECT (port->comp->parent, "failed to DRI2QueryExtension()");
    goto ERROR_CASE;
  }

  if (!DRI2QueryVersion(dpy, &dri2Major, &dri2Minor)) {
    GST_ERROR_OBJECT (port->comp->parent, "failed to DRI2QueryVersion");
    goto ERROR_CASE;
  }

  if (!DRI2Connect(dpy, RootWindow(dpy, DefaultScreen(dpy)), &driverName, &deviceName)) {
    GST_ERROR_OBJECT(port->comp->parent,"failed to DRI2Connect");
    goto ERROR_CASE;
  }
  ...
  if (port->drm_fd < 0) {
    GST_ERROR_OBJECT(port->comp->parent,"cannot open drm device (%s)", deviceName);
    goto ERROR_CASE;
  }
  ...
  if (ioctl(port->drm_fd, DRM_IOCTL_GET_MAGIC, &auth_arg)) {
    GST_ERROR_OBJECT(port->comp->parent,"cannot get drm auth magic");
    close(port->drm_fd);
    port->drm_fd = -1;
    goto ERROR_CASE;
  }
  ...
  if (!DRI2Authenticate(dpy, RootWindow(dpy, DefaultScreen(dpy)), auth_arg.magic)) {
    GST_ERROR_OBJECT(port->comp->parent,"cannot get drm authentication from X");
    close(port->drm_fd);
    port->drm_fd = -1;
    goto ERROR_CASE;
  }
  ...
  if (!port->bufmgr)
  {
      GST_ERROR_OBJECT (port->comp->parent, "fail to init buffer manager");
      close (port->drm_fd);
      port->drm_fd = -1;
      goto  ERROR_CASE;
  }
  ...
  GST_INFO_OBJECT(port->comp->parent, "gst_omx_drm_init leave");
  return TRUE;
...
void
gst_omx_drm_fini(GstOMXPort * port)
{
  if (port->bufmgr) {
    GST_DEBUG_OBJECT(port->comp->parent, "destroy tbm buffer manager");
    tbm_bufmgr_deinit(port->bufmgr);
    port->bufmgr = NULL;
  }

  if (port->drm_fd >= 0) {
    GST_DEBUG_OBJECT(port->comp->parent, "close drm_fd %d", port->drm_fd);
    close(port->drm_fd);
    port->drm_fd = -1;
  }
}
...
gst_omx_component_get_last_error (GstOMXComponent * comp)
{
  ...
  GST_DEBUG_OBJECT (comp->parent, "Returning last %s error: %s (0x%08x)", comp->name, gst_omx_error_to_string (err), err);
    ...
    gpointer param)
{
  ...
  GST_DEBUG_OBJECT (comp->parent, "Getting %s parameter at index 0x%08x", comp->name, index);
  err = OMX_GetParameter (comp->handle, index, param);
  GST_DEBUG_OBJECT (comp->parent, "Got %s parameter at index 0x%08x: %s (0x%08x)", comp->name, index, gst_omx_error_to_string (err), err);
    ...
    gpointer param)
{
  ...
  GST_DEBUG_OBJECT (comp->parent, "Setting %s parameter at index 0x%08x", comp->name, index);
  err = OMX_SetParameter (comp->handle, index, param);
  GST_DEBUG_OBJECT (comp->parent, "Set %s parameter at index 0x%08x: %s (0x%08x)", comp->name, index, gst_omx_error_to_string (err), err);
    ...
    gpointer config)
{
  ...
  GST_DEBUG_OBJECT (comp->parent, "Getting %s configuration at index 0x%08x", comp->name, index);
    ...
    gpointer config)
{
  ...
  GST_DEBUG_OBJECT (comp->parent, "Setting %s configuration at index 0x%08x", comp->name, index);
    ...
    GstOMXComponent * comp2, GstOMXPort * port2)
{
  ...
  g_return_val_if_fail (comp1->state == OMX_StateLoaded || !port1->port_def.bEnabled, OMX_ErrorUndefined);
  g_return_val_if_fail (port1 != NULL, OMX_ErrorUndefined);
  g_return_val_if_fail (port1->port_def.eDir == OMX_DirOutput, OMX_ErrorUndefined);
  g_return_val_if_fail (comp2 != NULL, OMX_ErrorUndefined);
  g_return_val_if_fail (comp2->state == OMX_StateLoaded || !port2->port_def.bEnabled, OMX_ErrorUndefined);
  g_return_val_if_fail (port2 != NULL, OMX_ErrorUndefined);
  g_return_val_if_fail (port2->port_def.eDir == OMX_DirInput, OMX_ErrorUndefined);
  g_return_val_if_fail (comp1->core == comp2->core, OMX_ErrorUndefined);

  g_mutex_lock (&comp1->lock);
  g_mutex_lock (&comp2->lock);
  GST_DEBUG_OBJECT (comp1->parent, "Setup tunnel between %s port %u and %s port %u",
  ...
  GST_DEBUG_OBJECT (comp1->parent, "Setup tunnel between %s port %u and %s port %u: %s (0x%08x)",
          comp1->name, port1->index, comp2->name, port2->index, gst_omx_error_to_string (err), err);
    ...
    GstOMXComponent * comp2, GstOMXPort * port2)
{
  ...
  g_mutex_lock (&comp1->lock);
  g_mutex_lock (&comp2->lock);
  GST_DEBUG_OBJECT (comp1->parent, "Closing tunnel between %s port %u and %s port %u",
          comp1->name, port1->index, comp2->name, port2->index);

  err = comp1->core->setup_tunnel (comp1->handle, port1->index, 0, 0);
  if (err != OMX_ErrorNone) {
    GST_ERROR_OBJECT (comp1->parent, "Failed to close tunnel on output side %s (0x%08x)", gst_omx_error_to_string (err), err);
  }
  err = comp2->core->setup_tunnel (0, 0, comp2->handle, port2->index);
  if (err != OMX_ErrorNone) {
    GST_ERROR_OBJECT (comp2->parent, "Failed to close tunnel on input side %s (0x%08x)", gst_omx_error_to_string (err), err);
  }

  port1->tunneled = FALSE;
  port2->tunneled = FALSE;

  GST_DEBUG_OBJECT (comp1->parent, "Closed tunnel between %s port %u and %s port %u",
          comp1->name, port1->index, comp2->name, port2->index);
    ...
    OMX_PARAM_PORTDEFINITIONTYPE * port_def)
{
  ...
  err = gst_omx_component_get_parameter (comp, OMX_IndexParamPortDefinition, port_def);
    ...
    OMX_PARAM_PORTDEFINITIONTYPE * port_def)
{
  ...
  if (port_def)
    err = gst_omx_component_set_parameter (comp, OMX_IndexParamPortDefinition, port_def);

  gst_omx_component_get_parameter (comp, OMX_IndexParamPortDefinition, &port->port_def);

  GST_DEBUG_OBJECT (comp->parent, "Updated %s port %u definition: %s (0x%08x)  :  (%lu * %lu) buffer_num = %d",
    comp->name, port->index, gst_omx_error_to_string (err), err, port->port_def.format.video.nFrameWidth, port->port_def.format.video.nFrameHeight, port->port_def.nBufferCountActual);
...
gst_omx_port_acquire_buffer (GstOMXPort * port, GstOMXBuffer ** buf)
{
  ...
  *buf = NULL;
  ...
  g_mutex_lock (&comp->lock);
  GST_DEBUG_OBJECT (comp->parent, "Acquiring %s buffer from port %u", comp->name, port->index);
...
retry:
  ...
  if ((err = comp->last_error) != OMX_ErrorNone) {
    GST_ERROR_OBJECT (comp->parent, "Component %s is in error state: %s", comp->name, gst_omx_error_to_string (err));
  ...
  if (port->port_def.eDir == OMX_DirInput) {
    if (comp->pending_reconfigure_outports) {
      gst_omx_component_handle_messages (comp);
      while (comp->pending_reconfigure_outports && (err = comp->last_error) == OMX_ErrorNone && !port->flushing) {
        GST_DEBUG_OBJECT (comp->parent, "Waiting for %s output ports to reconfigure", comp->name);
        g_mutex_lock (&comp->messages_lock);
        g_mutex_unlock (&comp->lock);
  ...
  if (port->port_def.eDir == OMX_DirOutput && port->settings_cookie != port->configured_settings_cookie) {
    if (!g_queue_is_empty (&port->pending_buffers)) {
      GST_DEBUG_OBJECT (comp->parent, "%s output port %u needs reconfiguration but has buffers pending", comp->name, port->index);
  ...
  if (port->port_def.eDir == OMX_DirOutput && port->eos) {
    if (!g_queue_is_empty (&port->pending_buffers)) {
      GST_DEBUG_OBJECT (comp->parent, "%s output port %u is EOS but has buffers pending", comp->name, port->index);
      _buf = g_queue_pop_head (&port->pending_buffers);
  ...
  gst_omx_component_handle_messages (comp);
  if (g_queue_is_empty (&port->pending_buffers)) {
    GST_DEBUG_OBJECT (comp->parent, "Queue of %s port %u is empty", comp->name, port->index);
    g_mutex_lock (&comp->messages_lock);
    g_mutex_unlock (&comp->lock);
    if (g_queue_is_empty (&comp->messages))
      g_cond_wait (&comp->messages_cond, &comp->messages_lock);
    ...
    GST_DEBUG_OBJECT (comp->parent, "%s port %u has pending buffers", comp->name, port->index);
    _buf = g_queue_pop_head (&port->pending_buffers);
    ret = GST_OMX_ACQUIRE_BUFFER_OK;
    goto done;
  }
  ...
  GST_DEBUG_OBJECT (comp->parent, "Acquired buffer %p (%p) from %s port %u: %d",
          _buf, (_buf ? _buf->omx_buf->pBuffer : NULL), comp->name, port->index, ret);
...
gst_omx_port_release_buffer (GstOMXPort * port, GstOMXBuffer * buf)
{
  ...
  if ((err = comp->last_error) != OMX_ErrorNone) {
    GST_ERROR_OBJECT (comp->parent, "Component %s is in error state: %s (0x%08x)", comp->name, gst_omx_error_to_string (err), err);
  ...
  if (port->flushing) {
    GST_DEBUG_OBJECT (comp->parent, "%s port %u is flushing, not releasing buffer", comp->name, port->index);

    if (port->port_def.eDir == OMX_DirInput) {
      g_queue_push_tail (&port->pending_buffers, buf);
    } else {
      if (port->disabled_pending) {
        g_queue_push_tail (&port->pending_buffers, buf);
      } else {
        buf->used = TRUE;
        GST_DEBUG_OBJECT (comp->parent, "OMX_FillThisBuffer. omx buf : %p", buf->omx_buf);
        err = OMX_FillThisBuffer (comp->handle, buf->omx_buf);
      }
    }
  ...
  if (port->port_def.eDir == OMX_DirInput) {
    GST_DEBUG_OBJECT (comp->parent, "OMX_EmptyThisBuffer. omx buf : %p", buf->omx_buf);
    err = OMX_EmptyThisBuffer (comp->handle, buf->omx_buf);
  } else {
    GST_DEBUG_OBJECT (comp->parent, "OMX_FillThisBuffer. omx buf : %p", buf->omx_buf);
    err = OMX_FillThisBuffer (comp->handle, buf->omx_buf);
  }
  GST_DEBUG_OBJECT (comp->parent, "Released buffer %p to %s port %u: %s (0x%08x)", buf, comp->name, port->index, gst_omx_error_to_string (err), err);
    ...
    gboolean flush)
{
  ...
  GST_DEBUG_OBJECT (comp->parent, "Setting %s port %d to %sflushing", comp->name, port->index, (flush ? "" : "not "));

  gst_omx_component_handle_messages (comp);

  if (! !flush == ! !port->flushing) {
    GST_DEBUG_OBJECT (comp->parent, "%s port %u was %sflushing already", comp->name, port->index, (flush ? "" : "not "));
    goto done;
  }
  ...
  if (flush) {
    ...
    if (err != OMX_ErrorNone) {
      GST_ERROR_OBJECT (comp->parent, "Error sending flush command to %s port %u: %s (0x%08x)",
    ...
    if (! !port->flushing != ! !flush) {
      GST_ERROR_OBJECT (comp->parent, "%s: another flush happened in the  meantime", comp->name);
      goto done;
    }
    ...
    if (timeout != GST_CLOCK_TIME_NONE) {
      ...
      if (add == 0) {
        if (!port->flushed || (port->buffers && port->buffers->len > g_queue_get_length (&port->pending_buffers)))
    ...
    gst_omx_component_handle_messages (comp);
    while (signalled && last_error == OMX_ErrorNone && !port->flushed && port->buffers) {
      g_mutex_lock (&comp->messages_lock);
      g_mutex_unlock (&comp->lock);
    ...
    GST_DEBUG_OBJECT (comp->parent, "%s port %d flushed", comp->name, port->index);
    if (last_error != OMX_ErrorNone) {
      GST_ERROR_OBJECT (comp->parent, "Got error while flushing %s port %u: %s (0x%08x)",
              comp->name, port->index, gst_omx_error_to_string (last_error), last_error);
      err = last_error;
      goto done;
    } else if (!signalled) {
      GST_ERROR_OBJECT (comp->parent, "Timeout while flushing %s port %u", comp->name, port->index);
      err = OMX_ErrorTimeout;
      goto done;
    }
  }
  ...
  GST_DEBUG_OBJECT (comp->parent, "Set %s port %u to %sflushing: %s (0x%08x)",
          comp->name, port->index, (flush ? "" : "not "), gst_omx_error_to_string (err), err);
  gst_omx_component_handle_messages (comp);
  g_mutex_unlock (&comp->lock);
...
gst_omx_port_is_flushing (GstOMXPort * port)
{
  ...
  if (flushing == TRUE)
    GST_WARNING_OBJECT (comp->parent, "%s port %u is flushing: %d", comp->name, port->index, flushing);
...
#ifdef USE_OMX_TARGET_QCOM
gboolean
create_platform_private_object(GstOMXPort *port, GstOMXBuffer *buf, guint buffer_index)
{
  OMX_QCOM_PLATFORM_PRIVATE_ENTRY *private_entry;
  OMX_QCOM_PLATFORM_PRIVATE_PMEM_INFO *private_pmem_info;

  private_entry = g_new0(OMX_QCOM_PLATFORM_PRIVATE_ENTRY, 1);
  if(!private_entry) {
    GST_ERROR("Could not allocate pPlatformEntry");
    return FALSE;
  }

  private_pmem_info = g_new0(OMX_QCOM_PLATFORM_PRIVATE_PMEM_INFO, 1);
  if(!private_pmem_info) {
    GST_ERROR("Could not allocate pPMEMInfo");
    return FALSE;
  }

  /* Initialize the Platform Entry */
  private_entry->type = OMX_QCOM_PLATFORM_PRIVATE_PMEM;
  private_entry->entry = private_pmem_info;

  /* Initialize the Platform List */
  buf->nEntries = 1;
  buf->entryList = private_entry;

  private_pmem_info->pmem_fd = port->mm_video_buf[buffer_index].handle.dmabuf_fd[0];

  private_pmem_info->offset = 0;
    ...
    const GList * buffers, const GList * images, guint n)
{
  ...
  g_return_val_if_fail (n != -1 || (!buffers && !images), OMX_ErrorBadParameter);

  if (n == -1)
    n = OMX_OUTBUF_MIN_NUMBER;

  if (n != port->port_def.nBufferCountActual) {
    GST_WARNING_OBJECT (comp->parent, "Alloc buffer as OMX_OUTBUF_MIN_NUMBER");
    port->port_def.nBufferCountActual = n;
    gst_omx_port_update_port_definition (port, &port->port_def);
  }

  g_return_val_if_fail (n == port->port_def.nBufferCountActual, OMX_ErrorBadParameter);

  GST_INFO_OBJECT (comp->parent, "Allocating %d buffers of size %lu for %s port %u",
          n, port->port_def.nBufferSize, comp->name, port->index);
  ...
  for (i = 0; i < n; i++) {
    ...
    if (buffers) {
      ...
      {
#ifdef USE_OMX_TARGET_QCOM
        if (!create_platform_private_object(port, buf, i))
          GST_ERROR("could not allocate private data structures");
#endif
        err = OMX_UseBuffer (comp->handle, &buf->omx_buf, port->index, buf, port->port_def.nBufferSize, l->data);
      }
      else if (port->comp->core->component_vendor == GST_OMX_VENDOR_SLSI_EXYNOS)
      {
        if (port->buffer_type == GST_OMX_BUFFER_TBM_VDEC_OUTPUT) {
          err = OMX_UseBuffer (comp->handle, &buf->omx_buf, port->index, buf, sizeof(MMVideoBuffer), l->data);
        } else {
          err = OMX_UseBuffer (comp->handle, &buf->omx_buf, port->index, buf, port->port_def.nBufferSize, l->data);
        }
      }
      else if (port->comp->core->component_vendor == GST_OMX_VENDOR_SPRD)
      {
        if (port->buffer_type == GST_OMX_BUFFER_TBM_VDEC_OUTPUT) {
#ifdef USE_OMX_TARGET_SPRD
          buf->phy_addr = 0; // we will send fd
          buf->shared_buf_fd = port->mm_video_buf[i].handle.dmabuf_fd[0];
          buf->phy_size = SPRD_ALIGN(port->port_def.format.video.nFrameWidth, port->port_def.format.video.nFrameHeight);
          err = OMX_UseBuffer (comp->handle, &buf->omx_buf, port->index, buf, buf->phy_size, l->data);
#endif
        } else {
          err = OMX_UseBuffer (comp->handle, &buf->omx_buf, port->index, buf, port->port_def.nBufferSize, l->data);
        }
      ...
      err = OMX_UseEGLImage (comp->handle, &buf->omx_buf, port->index, buf, l->data);
      buf->eglimage = TRUE;
    } else {
      err = OMX_AllocateBuffer (comp->handle, &buf->omx_buf, port->index, buf, port->port_def.nBufferSize);
      buf->eglimage = FALSE;

      if (port->codec_type == GST_OMX_CODEC_TYPE_VIDEO_ENC && port->index == 0) {
        encoder_media_buffer_type *media_buffer;
        media_buffer = (encoder_media_buffer_type *)buf->omx_buf->pBuffer;
        media_buffer->meta_handle = g_new0(native_handle, 1);
        GST_WARNING_OBJECT (comp->parent, "meta handle= %p", media_buffer->meta_handle );
      }
    }

    if (err != OMX_ErrorNone) {
      GST_ERROR_OBJECT (comp->parent, "Failed to allocate buffer for %s port %u: %s (0x%08x)", comp->name, port->index, gst_omx_error_to_string (err), err);
      gst_omx_port_deallocate_buffers_unlocked (port);
      goto done;
    }

    GST_DEBUG_OBJECT (comp->parent, "%s: allocated buffer %p (%p), port[%d] : %p", comp->name, buf, buf->omx_buf->pBuffer, port->index, port);
  ...
  GST_INFO_OBJECT (comp->parent, "Complete Allocated %d buffers for %s port %u: %s (0x%08x)", i, comp->name, port->index, gst_omx_error_to_string (err), err);
...
gst_omx_port_deallocate_buffers_unlocked (GstOMXPort * port)
{
  ...
  GST_INFO_OBJECT (comp->parent, "Deallocating %d buffers of %s port %u", port->port_def.nBufferCountActual, comp->name, port->index);

  gst_omx_component_handle_messages (port->comp);

  if (!port->buffers) {
    GST_DEBUG_OBJECT (comp->parent, "No buffers allocated for %s port %u", comp->name, port->index);
    goto done;
  }
  ...
  for (i = 0; i < n; i++) {
    ...
    if (buf->used) {
      GST_ERROR_OBJECT (comp->parent, "Trying to free used buffer %p of %s port %u", buf, comp->name, port->index);
    }
...
#ifdef USE_OMX_TARGET_QCOM
        GstOMXBuffer *private_list;
        OMX_QCOM_PLATFORM_PRIVATE_ENTRY *private_entry;
        OMX_QCOM_PLATFORM_PRIVATE_PMEM_INFO *private_pmem_info;

        private_list = buf->omx_buf->pAppPrivate;
        private_entry = private_list->entryList;
        private_pmem_info = private_entry->entry;

        if (private_pmem_info) {
          g_free(private_pmem_info);
          private_pmem_info = NULL;
        }

        if (private_entry) {
          g_free(private_entry);
          private_entry = NULL;
        }
#endif
        if (port->bo[i].bo_uv != NULL) {
          tbm_bo_unref(port->bo[i].bo_uv);
          port->bo[i].bo_uv = NULL;
        }

        if (port->bo[i].bo_y != NULL) {
          tbm_bo_unref(port->bo[i].bo_y);
          port->bo[i].bo_y = NULL;
        }

        buf->omx_buf->pBuffer = NULL;
      } else {
        if (port->codec_type == GST_OMX_CODEC_TYPE_VIDEO_ENC && port->index == 0) {
          encoder_media_buffer_type *media_buffer;
          media_buffer = (encoder_media_buffer_type *)buf->omx_buf->pBuffer;
          if(media_buffer != NULL && media_buffer->meta_handle != NULL) {
            g_free(media_buffer->meta_handle);
            media_buffer->meta_handle = NULL;
          }
        }
      }
      buf->omx_buf->pAppPrivate = NULL;
      GST_DEBUG_OBJECT (comp->parent, "%s: deallocating buffer %p (%p)", comp->name, buf, buf->omx_buf->pBuffer);

      tmp = OMX_FreeBuffer (comp->handle, port->index, buf->omx_buf);

      if (tmp != OMX_ErrorNone) {
        GST_ERROR_OBJECT (comp->parent, "Failed to deallocate buffer %d of %s port %u: %s (0x%08x)",
                i, comp->name, port->index, gst_omx_error_to_string (tmp), tmp);
        if (err == OMX_ErrorNone)
          err = tmp;
      }
    }
  ...
  GST_DEBUG_OBJECT (comp->parent, "Complete Deallocated %d buffers of %s port %u: %s (0x%08x)", i,
          comp->name, port->index, gst_omx_error_to_string (err), err);
...
gst_omx_port_set_enabled_unlocked (GstOMXPort * port, gboolean enabled)
{
  ...
  if (port->enabled_pending || port->disabled_pending) {
    GST_ERROR_OBJECT (comp->parent, "%s port %d enabled/disabled pending already", comp->name, port->index);
    err = OMX_ErrorInvalidState;
    goto done;
  }

  GST_INFO_OBJECT (comp->parent, "Setting %s port %u to %s", comp->name, port->index, (enabled ? "enabled" : "disabled"));
  ...
  if (enabled)
    err = OMX_SendCommand (comp->handle, OMX_CommandPortEnable, port->index, NULL);
  else
    err = OMX_SendCommand (comp->handle, OMX_CommandPortDisable, port->index, NULL);

  if (err != OMX_ErrorNone) {
    GST_ERROR_OBJECT (comp->parent, "Failed to send enable/disable command to %s port %u: %s (0x%08x)",
            comp->name, port->index, gst_omx_error_to_string (err), err);
    goto done;
  }
  ...
  GST_INFO_OBJECT (comp->parent, "Set %s port %u to %s%s: %s (0x%08x)",
          comp->name, port->index, (err == OMX_ErrorNone ? "" : "not "), (enabled ? "enabled" : "disabled"), gst_omx_error_to_string (err), err);
    ...
    GstClockTime timeout)
{
  ...
  GST_INFO_OBJECT (comp->parent, "Waiting for %s port %u to release all buffers", comp->name, port->index);

  if (timeout != GST_CLOCK_TIME_NONE) {
    gint64 add = timeout / (GST_SECOND / G_TIME_SPAN_SECOND);

    if (add == 0) {
      if (port->buffers && port->buffers->len > g_queue_get_length (&port->pending_buffers))
        err = OMX_ErrorTimeout;
      goto done;
    }
  ...
  last_error = OMX_ErrorNone;
  gst_omx_component_handle_messages (comp);
  while (signalled && last_error == OMX_ErrorNone && (port->buffers && port->buffers->len > g_queue_get_length (&port->pending_buffers))) {
    g_mutex_lock (&comp->messages_lock);
    g_mutex_unlock (&comp->lock);
  ...
  if (last_error != OMX_ErrorNone) {
    err = last_error;
    GST_ERROR_OBJECT (comp->parent, "Got error while waiting for %s port %u to release all buffers: %s (0x%08x)",
            comp->name, port->index, gst_omx_error_to_string (err), err);
    goto done;
  } else if (!signalled) {
    GST_ERROR_OBJECT (comp->parent, "Timeout waiting for %s port %u to release all buffers", comp->name, port->index);
    err = OMX_ErrorTimeout;
    goto done;
  }
  ...
  GST_DEBUG_OBJECT (comp->parent, "Waited for %s port %u to release all buffers: %s (0x%08x)",
          comp->name, port->index, gst_omx_error_to_string (err), err);
...
gst_omx_port_populate_unlocked (GstOMXPort * port)
{
  ...
  GST_DEBUG_OBJECT (comp->parent, "Populating %s port %d", comp->name, port->index);

  gst_omx_component_handle_messages (comp);

  if (port->flushing) {
    GST_DEBUG_OBJECT (comp->parent, "%s port %u is flushing", comp->name, port->index);
  ...
  if (port->port_def.eDir == OMX_DirOutput && port->buffers && !port->tunneled) {
    ...
    while ((buf = g_queue_pop_head (&port->pending_buffers))) {
      ...
      if (err != OMX_ErrorNone) {
        GST_ERROR_OBJECT (comp->parent, "Failed to pass buffer %p (%p) to %s port %u: %s (0x%08x)",
                buf, buf->omx_buf->pBuffer, comp->name, port->index, gst_omx_error_to_string (err), err);
        goto done;
      }
      GST_DEBUG_OBJECT (comp->parent, "Passed buffer %p (%p) to component %s", buf, buf->omx_buf->pBuffer, comp->name);
    }
  }
  ...
  GST_DEBUG_OBJECT (comp->parent, "Populated %s port %u: %s (0x%08x)", comp->name, port->index, gst_omx_error_to_string (err), err);
  gst_omx_component_handle_messages (comp);
...
gst_omx_port_wait_enabled_unlocked (GstOMXPort * port, GstClockTime timeout)
{
  ...
  GST_INFO_OBJECT (comp->parent, "Waiting for %s port %u to be %s", comp->name, port->index, (enabled ? "enabled" : "disabled"));
  ...
  gst_omx_port_update_port_definition (port, NULL);
  gst_omx_component_handle_messages (comp);
  while (signalled && last_error == OMX_ErrorNone && (! !port->port_def.bEnabled != ! !enabled || port->enabled_pending || port->disabled_pending)) {
    g_mutex_lock (&comp->messages_lock);
    g_mutex_unlock (&comp->lock);
  ...
  if (!signalled) {
    GST_ERROR_OBJECT (comp->parent, "Timeout waiting for %s port %u to be %s", comp->name, port->index, (enabled ? "enabled" : "disabled"));
    err = OMX_ErrorTimeout;
    goto done;
  } else if (last_error != OMX_ErrorNone) {
    GST_ERROR_OBJECT (comp->parent, "Got error while waiting for %s port %u to be %s: %s (0x%08x)",
                    comp->name, port->index, (enabled ? "enabled" : "disabled"), gst_omx_error_to_string (err), err);
    err = last_error;
  } else {
    if (enabled)
    {
  ...
  GST_INFO_OBJECT (comp->parent, "%s port %u is %s%s: %s (0x%08x)",
                comp->name, port->index, (err == OMX_ErrorNone ? "" : "not "), (enabled ? "enabled" : "disabled"), gst_omx_error_to_string (err), err);
...
gst_omx_port_is_enabled (GstOMXPort * port)
{
  ...
  GST_DEBUG_OBJECT (comp->parent, "%s port %u is enabled: %d", comp->name, port->index, enabled);
...
gst_omx_port_mark_reconfigured (GstOMXPort * port)
{
  ...
  g_mutex_lock (&comp->lock);
  GST_INFO_OBJECT (comp->parent, "Marking %s port %u is reconfigured", comp->name, port->index);
  ...
  if (port->port_def.eDir == OMX_DirOutput) {
    ...
    for (l = comp->pending_reconfigure_outports; l; l = l->next) {
      if (l->data == (gpointer) port) {
        comp->pending_reconfigure_outports = g_list_delete_link (comp->pending_reconfigure_outports, l);
        break;
      }
    }
    if (!comp->pending_reconfigure_outports)
  ...
  GST_INFO_OBJECT (comp->parent, "Marked %s port %u as reconfigured: %s (0x%08x)", comp->name, port->index, gst_omx_error_to_string (err), err);
...
gst_omx_parse_hacks (gchar ** hacks)
{
  ...
  while (*hacks) {
    if (g_str_equal (*hacks, "event-port-settings-changed-ndata-parameter-swap"))
      hacks_flags |= GST_OMX_HACK_EVENT_PORT_SETTINGS_CHANGED_NDATA_PARAMETER_SWAP;
...
_class_init (gpointer g_class, gpointer data)
{
  ...
  for (i = 0; i < G_N_ELEMENTS (base_types); i++) {
    ...
    if (G_TYPE_CHECK_CLASS_TYPE (g_class, gtype)) {
      class_data = (GstOMXClassData *) (((guint8 *) g_class) + base_types[i].offset);
      break;
    }
  }
  ...
  component_name = g_key_file_get_string (config, element_name, "component-name", NULL);
  g_assert (component_name != NULL);
  class_data->component_name = component_name;

  /* If this fails we simply don't set a role */
  if ((component_role = g_key_file_get_string (config, element_name, "component-role", NULL))) {
    GST_DEBUG ("Using component-role '%s' for element '%s'", component_role, element_name);
  ...
  err = NULL;
  in_port_index = g_key_file_get_integer (config, element_name, "in-port-index", &err);
  if (err != NULL) {
    GST_DEBUG ("No 'in-port-index' set for element '%s', auto-detecting: %s", element_name, err->message);
    in_port_index = -1;
    g_error_free (err);
  }
  ...
  err = NULL;
  out_port_index = g_key_file_get_integer (config, element_name, "out-port-index", &err);
  if (err != NULL) {
    GST_DEBUG ("No 'out-port-index' set for element '%s', auto-detecting: %s", element_name, err->message);
    out_port_index = -1;
    g_error_free (err);
  }
  ...
  /* Add pad templates */
  err = NULL;
    template_caps = g_key_file_get_string (config, element_name, "sink-template-caps", &err);
    if (err !=NULL) {
    GST_DEBUG ("No sink template caps specified for element '%s', using default '%s'", element_name, class_data->default_sink_template_caps);
    ...
    if (!caps) {
      GST_DEBUG ("Could not parse sink template caps '%s' for element '%s', using default '%s'",
                        template_caps, element_name, class_data->default_sink_template_caps);
      caps = gst_caps_from_string (class_data->default_sink_template_caps);
      g_assert (caps != NULL);
    } else {
      GST_DEBUG("sink caps : % " GST_PTR_FORMAT, caps);
    }
  }
  ...
  err = NULL;
    template_caps = g_key_file_get_string (config, element_name, "src-template-caps", &err);
  if (err !=NULL) {
    GST_DEBUG ("No src template caps specified for element '%s', using default '%s'", element_name, class_data->default_src_template_caps);
    ...
    caps = gst_caps_from_string (template_caps);
    if (!caps) {
      GST_DEBUG ("Could not parse src template caps '%s' for element '%s', using default '%s'",
                        template_caps, element_name, class_data->default_src_template_caps);
      caps = gst_caps_from_string (class_data->default_src_template_caps);
      g_assert (caps != NULL);
    } else {
      GST_DEBUG("src caps : % " GST_PTR_FORMAT, caps);
    }
  }
  ...
  if ((hacks = g_key_file_get_string_list (config, element_name, "hacks", NULL, NULL))) {
...
plugin_init (GstPlugin * plugin)
{
  ...
  config_dirs = g_new (gchar *, g_strv_length ((gchar **) system_config_dirs) + 3);
  ...
  gst_plugin_add_dependency (plugin, env_config_name, (const gchar **) (config_dirs + (env_config_dir ? 1 : 0)),
                config_name, GST_PLUGIN_DEPENDENCY_FLAG_NONE);

  config = g_key_file_new ();
  if (!g_key_file_load_from_dirs (config, *config_name, (const gchar **) config_dirs, NULL, G_KEY_FILE_NONE, &err)) {
    gchar *paths;

    paths = g_strjoinv (":", config_dirs);
    GST_ERROR ("Failed to load configuration file: %s (searched in: %s as per GST_OMX_CONFIG_DIR environment variable, the xdg user config "
        "directory (or XDG_CONFIG_HOME) and the system config directory (or XDG_CONFIG_DIRS)", err->message, paths);
    g_free (paths);
    g_error_free (err);
    goto done;
  }
  ...
  for (i = 0; i < n_elements; i++) {
    ...
    err = NULL;
    if (!(type_name = g_key_file_get_string (config, elements[i], "type-name", &err))) {
      GST_ERROR ("Unable to read 'type-name' configuration for element '%s': %s", elements[i], err->message);
      g_error_free (err);
      continue;
    }
    ...
    if (type == G_TYPE_INVALID) {
      GST_ERROR ("Invalid type name '%s' for element '%s'", type_name, elements[i]);
      g_free (type_name);
      continue;
    }
    if (!g_type_is_a (type, GST_TYPE_ELEMENT)) {
      GST_ERROR ("Type '%s' is no GstElement subtype for element '%s'", type_name, elements[i]);
      g_free (type_name);
      continue;
    }
    g_free (type_name);
    ...
    err = NULL;
    if (!(core_name = g_key_file_get_string (config, elements[i], "core-name", &err))) {
      GST_ERROR ("Unable to read 'core-name' configuration for element '%s': %s", elements[i], err->message);
      g_error_free (err);
      continue;
    }
    if (!g_file_test (core_name, G_FILE_TEST_IS_REGULAR)) {
      GST_ERROR ("Core '%s' does not exist for element '%s'", core_name, elements[i]);
      g_free (core_name);
      continue;
    }
    g_free (core_name);
    ...
    err = NULL;
    if (!(component_name = g_key_file_get_string (config, elements[i], "component-name", &err))) {
      GST_ERROR ("Unable to read 'component-name' configuration for element '%s': %s", elements[i], err->message);
      g_error_free (err);
      continue;
    }
    ...
    if (g_type_from_name (type_name) != G_TYPE_INVALID) {
      GST_ERROR ("Type '%s' already exists for element '%s'", type_name, elements[i]);
      g_free (type_name);
      continue;
    }
