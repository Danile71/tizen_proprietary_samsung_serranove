...
/* prototypes */
static void gst_omx_aac_enc_set_property (GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_omx_aac_enc_get_property (GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);
static gboolean gst_omx_aac_enc_set_format (GstOMXAudioEnc * enc, GstOMXPort * port, GstAudioInfo * info);
static GstCaps *gst_omx_aac_enc_get_caps (GstOMXAudioEnc * enc, GstOMXPort * port, GstAudioInfo * info);
static guint gst_omx_aac_enc_get_num_samples (GstOMXAudioEnc * enc, GstOMXPort * port, GstAudioInfo * info, GstOMXBuffer * buf);
...
G_DEFINE_TYPE_WITH_CODE (GstOMXAACEnc, gst_omx_aac_enc, GST_TYPE_OMX_AUDIO_ENC, DEBUG_INIT);
...
gst_omx_aac_enc_class_init (GstOMXAACEncClass * klass)
{
  ...
  audioenc_class->get_num_samples = GST_DEBUG_FUNCPTR (gst_omx_aac_enc_get_num_samples);
    ...
    GstAudioInfo * info)
{
  ...
  err = gst_omx_component_get_parameter (enc->enc, OMX_IndexParamAudioAac, &aac_profile);
  if (err != OMX_ErrorNone) {
    GST_ERROR_OBJECT (self, "Failed to get AAC parameters from component: %s (0x%08x)", gst_omx_error_to_string (err), err);
    return FALSE;
  }

  peercaps = gst_pad_peer_query_caps (GST_AUDIO_ENCODER_SRC_PAD (self), gst_pad_get_pad_template_caps (GST_AUDIO_ENCODER_SRC_PAD (self)));
  if (peercaps) {
    GstStructure *s;
    gint mpegversion = 0;
    ...
    if (gst_structure_get_int (s, "mpegversion", &mpegversion)) {
      profile_string = gst_structure_get_string (s, ((mpegversion == 2) ? "profile" : "base-profile"));
  ...
  err = gst_omx_component_set_parameter (enc->enc, OMX_IndexParamAudioAac, &aac_profile);
  if (err != OMX_ErrorNone) {
    GST_ERROR_OBJECT (self, "Error setting AAC parameters: %s (0x%08x)", gst_omx_error_to_string (err), err);
    return FALSE;
  }
    ...
    GstAudioInfo * info)
{
  ...
  err = gst_omx_component_get_parameter (enc->enc, OMX_IndexParamAudioAac, &aac_profile);
  if (err != OMX_ErrorNone) {
    GST_ERROR_OBJECT (enc, "Failed to get AAC parameters from component: %s (0x%08x)", gst_omx_error_to_string (err), err);
    return NULL;
  }
  ...
  if (mpegversion != 0)
    gst_caps_set_simple (caps, "mpegversion", G_TYPE_INT, mpegversion, "stream-format", G_TYPE_STRING, stream_format, NULL);
  ...
  if (aac_profile.nChannels != 0)
    gst_caps_set_simple (caps, "channels", G_TYPE_INT, aac_profile.nChannels, NULL);
  if (aac_profile.nSampleRate != 0)
    gst_caps_set_simple (caps, "rate", G_TYPE_INT, aac_profile.nSampleRate, NULL);
  ...
  if (aac_profile.eAACStreamFormat == OMX_AUDIO_AACStreamFormatRAW) {
    ...
    map.data[0] = ((aac_profile.eAACProfile & 0x1F) << 3) | ((sr_idx & 0xE) >> 1);
