...
/* prototypes */
static gboolean gst_omx_h263_dec_is_format_change (GstOMXVideoDec * dec, GstOMXPort * port, GstVideoCodecState * state);
static gboolean gst_omx_h263_dec_set_format (GstOMXVideoDec * dec, GstOMXPort * port, GstVideoCodecState * state);
...
G_DEFINE_TYPE_WITH_CODE (GstOMXH263Dec, gst_omx_h263_dec, GST_TYPE_OMX_VIDEO_DEC, DEBUG_INIT);
...
gst_omx_h263_dec_class_init (GstOMXH263DecClass * klass)
{
  ...
  videodec_class->is_format_change = GST_DEBUG_FUNCPTR (gst_omx_h263_dec_is_format_change);
...
static gboolean
gst_omx_h263_dec_is_format_change (GstOMXVideoDec * dec, GstOMXPort * port, GstVideoCodecState * state)
{
  return FALSE;
}

static gboolean
gst_omx_h263_dec_set_format (GstOMXVideoDec * dec, GstOMXPort * port, GstVideoCodecState * state)
{
  gboolean ret;
