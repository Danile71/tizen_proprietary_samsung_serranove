...
/* prototypes */
static gboolean gst_omx_h263_enc_set_format (GstOMXVideoEnc * enc, GstOMXPort * port, GstVideoCodecState * state);
static GstCaps *gst_omx_h263_enc_get_caps (GstOMXVideoEnc * enc, GstOMXPort * port, GstVideoCodecState * state);
...
G_DEFINE_TYPE_WITH_CODE (GstOMXH263Enc, gst_omx_h263_enc, GST_TYPE_OMX_VIDEO_ENC, DEBUG_INIT);
...
static gboolean
gst_omx_h263_enc_set_format (GstOMXVideoEnc * enc, GstOMXPort * port, GstVideoCodecState * state)
{
    ...
    GstVideoCodecState * state)
{
  ...
  if (err == OMX_ErrorNone) {
    ...
    gst_caps_set_simple (caps, "profile", G_TYPE_UINT, profile, "level", G_TYPE_UINT, level, 
            "variant", G_TYPE_STRING, "itu", "h263version", G_TYPE_STRING, "h263", NULL);
  }
