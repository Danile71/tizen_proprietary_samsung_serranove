...
/* prototypes */
static gboolean gst_omx_mpeg4_video_enc_set_format (GstOMXVideoEnc * enc, GstOMXPort * port, GstVideoCodecState * state);
static GstCaps *gst_omx_mpeg4_video_enc_get_caps (GstOMXVideoEnc * enc, GstOMXPort * port, GstVideoCodecState * state);
...
G_DEFINE_TYPE_WITH_CODE (GstOMXMPEG4VideoEnc, gst_omx_mpeg4_video_enc, GST_TYPE_OMX_VIDEO_ENC, DEBUG_INIT);
...
gst_omx_mpeg4_video_enc_class_init (GstOMXMPEG4VideoEncClass * klass)
{
  ...
  videoenc_class->set_format = GST_DEBUG_FUNCPTR (gst_omx_mpeg4_video_enc_set_format);
  videoenc_class->get_caps = GST_DEBUG_FUNCPTR (gst_omx_mpeg4_video_enc_get_caps);
    ...
    GstVideoCodecState * state)
{
  ...
  if (err != OMX_ErrorNone) {
    GST_WARNING_OBJECT (self, "Getting profile/level not supported by component");
    return TRUE;
  }
  ...
  if (peercaps) {
    ...
    intersection = gst_caps_intersect (peercaps, gst_pad_get_pad_template_caps (GST_VIDEO_ENCODER_SRC_PAD (enc)));
  ...
  err = gst_omx_component_set_parameter (GST_OMX_VIDEO_ENC (self)->enc, OMX_IndexParamVideoProfileLevelCurrent, &param);
    ...
    GstVideoCodecState * state)
{
  ...
  caps = gst_caps_new_simple ("video/mpeg", "mpegversion", G_TYPE_INT, 4, "systemstream", G_TYPE_BOOLEAN, FALSE, NULL);
