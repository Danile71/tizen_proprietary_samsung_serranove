...
/* prototypes */
static gboolean gst_omx_theora_dec_is_format_change (GstOMXVideoDec * dec, GstOMXPort * port, GstVideoCodecState * state);
static gboolean gst_omx_theora_dec_set_format (GstOMXVideoDec * dec, GstOMXPort * port, GstVideoCodecState * state);
static GstFlowReturn gst_omx_theora_dec_handle_frame (GstVideoDecoder * decoder, GstVideoCodecFrame * frame);
...
G_DEFINE_TYPE_WITH_CODE (GstOMXTheoraDec, gst_omx_theora_dec, GST_TYPE_OMX_VIDEO_DEC, DEBUG_INIT);
...
gst_omx_theora_dec_class_init (GstOMXTheoraDecClass * klass)
{
  ...
  videodec_class->is_format_change = GST_DEBUG_FUNCPTR (gst_omx_theora_dec_is_format_change);
  videodec_class->set_format = GST_DEBUG_FUNCPTR (gst_omx_theora_dec_set_format);

  videodec_class->cdata.default_sink_template_caps = "video/x-theora, " "width=(int) [1,MAX], " "height=(int) [1,MAX]";
...
static gboolean
gst_omx_theora_dec_is_format_change (GstOMXVideoDec * dec, GstOMXPort * port, GstVideoCodecState * state)
{
  return FALSE;
}

static gboolean
gst_omx_theora_dec_set_format (GstOMXVideoDec * dec, GstOMXPort * port, GstVideoCodecState * state)
{
  gboolean ret;
...
gst_omx_theora_dec_handle_frame (GstVideoDecoder * decoder, GstVideoCodecFrame * frame)
{
  ...
  if (GST_BUFFER_FLAG_IS_SET (frame->input_buffer, GST_BUFFER_FLAG_HEADER)) {
    ...
    self->header = gst_buffer_append (self->header, gst_buffer_ref (frame->input_buffer));
  ...
  return GST_VIDEO_DECODER_CLASS (gst_omx_theora_dec_parent_class)->handle_frame (GST_VIDEO_DECODER (self), frame);
}
