...
static void gst_omx_video_enc_set_property (GObject * object, guint prop_id, const GValue * value, GParamSpec * pspec);
static void gst_omx_video_enc_get_property (GObject * object, guint prop_id, GValue * value, GParamSpec * pspec);
...
static gboolean gst_omx_video_enc_set_format (GstVideoEncoder * encoder, GstVideoCodecState * state);
static gboolean gst_omx_video_enc_reset (GstVideoEncoder * encoder, gboolean hard);
static GstFlowReturn gst_omx_video_enc_handle_frame (GstVideoEncoder * encoder, GstVideoCodecFrame * frame);
static gboolean gst_omx_video_enc_finish (GstVideoEncoder * encoder);
static gboolean gst_omx_video_enc_propose_allocation (GstVideoEncoder * encoder, GstQuery * query);
static GstCaps *gst_omx_video_enc_getcaps (GstVideoEncoder * encoder, GstCaps * filter);
static GstFlowReturn gst_omx_video_enc_drain (GstOMXVideoEnc * self, gboolean at_eos);
static GstFlowReturn gst_omx_video_enc_handle_output_frame (GstOMXVideoEnc * self, GstOMXPort * port, GstOMXBuffer * buf, GstVideoCodecFrame * frame);
static void gst_omx_video_enc_release_frame (GstVideoCodecFrame * frame);
...
#define GST_OMX_VIDEO_ENC_IDR_PERIOD_DEFAULT (0x0000001d)
#define GST_OMX_VIDEO_ENC_BYTE_STREAM_DEFAULT (FALSE)
...
G_DEFINE_ABSTRACT_TYPE_WITH_CODE (GstOMXVideoEnc, gst_omx_video_enc, GST_TYPE_VIDEO_ENCODER, DEBUG_INIT);
...
gst_omx_video_enc_class_init (GstOMXVideoEncClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_IDR_PERIOD,
      g_param_spec_uint ("idr-period", "set interval of I-frame",
          "set interval of I-frame (0x0000001d=component default)",
          0, G_MAXUINT, GST_OMX_VIDEO_ENC_IDR_PERIOD_DEFAULT,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS |
          GST_PARAM_MUTABLE_READY));

  g_object_class_install_property (gobject_class, PROP_BYTE_STREAM,
      g_param_spec_boolean ("byte-stream", "Stream format setting",
          "Parameter to set encoding stream format. Default is avc",
          GST_OMX_VIDEO_ENC_BYTE_STREAM_DEFAULT,
          G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS |
          GST_PARAM_MUTABLE_READY));
  ...
  element_class->change_state = GST_DEBUG_FUNCPTR (gst_omx_video_enc_change_state);
  ...
  video_encoder_class->set_format = GST_DEBUG_FUNCPTR (gst_omx_video_enc_set_format);
  video_encoder_class->handle_frame = GST_DEBUG_FUNCPTR (gst_omx_video_enc_handle_frame);
  video_encoder_class->finish = GST_DEBUG_FUNCPTR (gst_omx_video_enc_finish);
  video_encoder_class->propose_allocation = GST_DEBUG_FUNCPTR (gst_omx_video_enc_propose_allocation);
...
gst_omx_video_enc_init (GstOMXVideoEnc * self)
{
  ...
  self->idr_period = GST_OMX_VIDEO_ENC_IDR_PERIOD_DEFAULT;
  self->byte_stream = GST_OMX_VIDEO_ENC_BYTE_STREAM_DEFAULT;
...
#ifdef CODEC_ENC_INPUT_DUMP
static inline void
gst_omx_video_enc_input_dump (MMVideoBuffer *inbuf, GstOMXVendor component_vendor)
{
  ...
  if (component_vendor != GST_OMX_VENDOR_SPRD) {
    ...
    fwrite(temp, inbuf->width[0] * inbuf->height[0] * 3 / 2, 1, fp);
    GST_WARNING ("SPRD codec encoder input dumped!!");
  }
  fclose(fp);
}
#endif
...
gboolean
gst_omx_video_enc_create_meta_data(OMX_U8 *pBuffer, MMVideoBuffer *buf, int aligned_size)
{
#ifdef USE_OMX_TARGET_SPRD
  sprd_encoder_meta_data *media_buffer;

  media_buffer = (sprd_encoder_meta_data *)pBuffer;
  media_buffer->buffer_type = kMetadataBufferTypeCameraSource;
  media_buffer->phy_addr = buf->handle.paddr[0];
  media_buffer->virt_addr = buf->data[0];
  media_buffer->shared_buf_fd = buf->handle.dmabuf_fd[0];

  GST_DEBUG ("[TIZEN BUFFER] phy[%p] vir[%p] fd[%d] type[%d]", media_buffer->phy_addr, media_buffer->virt_addr,
    media_buffer->shared_buf_fd, media_buffer->buffer_type);
#endif
#ifdef USE_OMX_TARGET_QCOM
  encoder_media_buffer_type *media_buffer;

  media_buffer = (encoder_media_buffer_type *)pBuffer;

  media_buffer->meta_handle->version = sizeof(native_handle);
  media_buffer->meta_handle->numInts = 2;
  media_buffer->meta_handle->numFds = 1;
  media_buffer->meta_handle->data[0] = buf->handle.dmabuf_fd[0];
  media_buffer->meta_handle->data[1] = 0; /* GST_BUFFER_OFFSET 0 */
  media_buffer->meta_handle->data[2] = aligned_size;

  media_buffer->buffer_type = kMetadataBufferTypeCameraSource;

  GST_DEBUG ("[TIZEN BUFFER] Version[%d] Size[%d] fd[%d]", media_buffer->meta_handle->version,
    media_buffer->meta_handle->data[2], media_buffer->meta_handle->data[0]);
#endif
...
static void
gst_omx_video_enc_setup_port(GstOMXVideoEnc *self)
{
  if (self->enc_in_port->comp->core->component_vendor == GST_OMX_VENDOR_QCT) {
    GST_INFO_OBJECT (self, "QCT Vendor selected.");

    self->enc_in_port->omx_allocate = TRUE;
    self->enc_out_port->omx_allocate = TRUE;

//    self->enc_in_port->buffer_type = GST_OMX_BUFFER_DEFAULT;
//    self->enc_out_port->buffer_type = GST_OMX_BUFFER_TBM_VDEC_OUTPUT;

  } else if (self->enc_in_port->comp->core->component_vendor == GST_OMX_VENDOR_SLSI_EXYNOS) {
    GST_INFO_OBJECT (self, "Exynos Vendor selected.");

//    self->enc_in_port->omx_allocate = FALSE;
//    self->enc_out_port->omx_allocate = FALSE;

//    self->enc_in_port->buffer_type = GST_OMX_BUFFER_GST;
//    self->enc_out_port->buffer_type = GST_OMX_BUFFER_TBM_VDEC_OUTPUT;

     /* get extension index and set platform specific buffer enable */
//    set_enable_platformSpecificBuffer (self);
  } else if (self->enc_in_port->comp->core->component_vendor == GST_OMX_VENDOR_SPRD) {
    ...
    self->enc_in_port->buffer_type = GST_OMX_BUFFER_DEFAULT;
    self->enc_out_port->buffer_type = GST_OMX_BUFFER_DEFAULT;
  }
  else
    GST_DEBUG_OBJECT (self, "Unknown Vendor!!!");

  self->enc_in_port->codec_type = GST_OMX_CODEC_TYPE_VIDEO_ENC;
  self->enc_out_port->codec_type = GST_OMX_CODEC_TYPE_VIDEO_ENC;
...
gst_omx_video_enc_open (GstVideoEncoder * encoder)
{
  ...
  if (gst_omx_component_get_state (self->enc, GST_CLOCK_TIME_NONE) != OMX_StateLoaded)
    return FALSE;
  ...
  if (in_port_index == -1 || out_port_index == -1) {
    ...
    err = gst_omx_component_get_parameter (self->enc, OMX_IndexParamVideoInit, &param);
    if (err != OMX_ErrorNone) {
      ...
      GST_DEBUG_OBJECT (self, "Detected %u ports, starting at %u", param.nPorts, param.nStartPortNumber);
  ...
  gst_omx_video_enc_setup_port(self);
  ...
  /* Set properties */
  {
    ...
    if (self->control_rate != 0xffffffff || self->target_bitrate != 0xffffffff) {
      ...
      err = gst_omx_component_get_parameter (self->enc, OMX_IndexParamVideoBitrate, &bitrate_param);
      ...
      if (err == OMX_ErrorNone) {
        ...
        if (err == OMX_ErrorUnsupportedIndex) {
          GST_WARNING_OBJECT (self, "Setting a bitrate not supported by the component");
        } else if (err == OMX_ErrorUnsupportedSetting) {
          GST_WARNING_OBJECT (self, "Setting bitrate settings %u %u not supported by the component", self->control_rate, self->target_bitrate);
        } else if (err != OMX_ErrorNone) {
          GST_ERROR_OBJECT (self, "Failed to set bitrate parameters: %s (0x%08x)", gst_omx_error_to_string (err), err);
          return FALSE;
        }
      } else {
        GST_ERROR_OBJECT (self, "Failed to get bitrate parameters: %s (0x%08x)", gst_omx_error_to_string (err), err);
      }
    }

    if (self->quant_i_frames != 0xffffffff || self->quant_p_frames != 0xffffffff || self->quant_b_frames != 0xffffffff) {
      ...
      err = gst_omx_component_get_parameter (self->enc, OMX_IndexParamVideoQuantization, &quant_param);
      ...
      if (err == OMX_ErrorNone) {
        ...
        err = gst_omx_component_set_parameter (self->enc, OMX_IndexParamVideoQuantization, &quant_param);
        if (err == OMX_ErrorUnsupportedIndex) {
          GST_WARNING_OBJECT (self, "Setting quantization parameters not supported by the component");
        } else if (err == OMX_ErrorUnsupportedSetting) {
          GST_WARNING_OBJECT (self, "Setting quantization parameters %u %u %u not supported by the component",
              self->quant_i_frames, self->quant_p_frames, self->quant_b_frames);
        } else if (err != OMX_ErrorNone) {
          GST_ERROR_OBJECT (self, "Failed to set quantization parameters: %s (0x%08x)", gst_omx_error_to_string (err), err);
          return FALSE;
        }
      } else {
        GST_ERROR_OBJECT (self, "Failed to get quantization parameters: %s (0x%08x)",
...

{
    ...
    if (self->enc_in_port->comp->core->component_vendor == GST_OMX_VENDOR_SPRD) {
      OMX_VIDEO_CONFIG_METADATA param;
      GST_OMX_INIT_STRUCT (&param);

      param.nPortIndex = self->enc_in_port->index;
      param.bStoreMetaData  = TRUE;
      GST_WARNING ("OMX_IndexParamStoreMetaDataBuffer set to %d", param.bStoreMetaData);

      err = gst_omx_component_set_parameter (self->enc, OMX_IndexParamStoreMetaDataBuffer, &param);

      if (err != OMX_ErrorNone) {
        GST_ERROR ("Failed to set metadata buffer parameters: %s (0x%08x)", gst_omx_error_to_string (err), err);
        return FALSE;
      }
    } else if (self->enc_in_port->comp->core->component_vendor == GST_OMX_VENDOR_QCT) {
      OMX_VIDEO_CONFIG_METADATA param;
      GST_OMX_INIT_STRUCT (&param);

      param.nPortIndex = self->enc_in_port->index;
      param.bStoreMetaData  = TRUE;
      GST_WARNING ("OMX_QcomIndexParamVideoEncodeMetaBufferMode set to %d", param.bStoreMetaData);

      err = gst_omx_component_set_parameter (self->enc, OMX_QcomIndexParamVideoEncodeMetaBufferMode, &param);
...
{
  GstOMXVideoEnc *self = GST_OMX_VIDEO_ENC (object);
  GST_WARNING_OBJECT (self, "prop id :%d , value : %llu", prop_id, value);
  switch (prop_id) {
    ...
    case PROP_TARGET_BITRATE:
      ...
      if (self->enc) {
        ...
        err = gst_omx_component_set_config (self->enc, OMX_IndexConfigVideoBitrate, &config);
        if (err != OMX_ErrorNone)
          GST_ERROR_OBJECT (self, "Failed to set bitrate parameter: %s (0x%08x)", gst_omx_error_to_string (err), err);
      }
      break;
    case PROP_IDR_PERIOD:
      self->idr_period = g_value_get_int (value);
      {
        ...
        GST_OMX_INIT_STRUCT (&param);
        param.nPortIndex = self->enc_out_port->index;
        gst_omx_component_get_parameter (self->enc, OMX_IndexParamVideoAvc, &param);
...
gst_omx_video_enc_change_state (GstElement * element, GstStateChange transition)
{
  ...
  ret = GST_ELEMENT_CLASS (gst_omx_video_enc_parent_class)->change_state (element, transition);
...
_find_nearest_frame (GstOMXVideoEnc * self, GstOMXBuffer * buf)
{
  ...
  g_list_foreach (frames, (GFunc) gst_omx_video_enc_release_frame, NULL);
  g_list_free (frames);
    ...
    GstOMXBuffer * buf, GstVideoCodecFrame * frame)
{
  ...
  if ((buf->omx_buf->nFlags & OMX_BUFFERFLAG_CODECCONFIG) && buf->omx_buf->nFilledLen > 0) {
    ...
    state = gst_video_encoder_set_output_state (GST_VIDEO_ENCODER (self), caps, self->input_state);
    state->codec_data = codec_data;

    if (!gst_video_encoder_negotiate (GST_VIDEO_ENCODER (self))) {
      /*Modification : codec data already set_caps, so unref frame whenever negotiate ok or not*/
      //gst_omx_video_enc_release_frame (frame);
    ...
    if ((klass->cdata.hacks & GST_OMX_HACK_SYNCFRAME_FLAG_NOT_USED) || (buf->omx_buf->nFlags & OMX_BUFFERFLAG_SYNCFRAME)) {
      if (frame) {
...
gst_omx_video_enc_loop (GstOMXVideoEnc * self)
{
  ...
  if (!gst_pad_has_current_caps (GST_VIDEO_ENCODER_SRC_PAD (self)) || acq_return == GST_OMX_ACQUIRE_BUFFER_RECONFIGURE) {
    GstCaps *caps;
    GstVideoCodecState *state;
  ...
  GST_DEBUG_OBJECT (self, "Handling buffer: 0x%08x %lu", buf->omx_buf->nFlags, buf->omx_buf->nTimeStamp);
...
flow_error:
  {
    ...
    {
      GST_ELEMENT_ERROR (self, STREAM, FAILED, ("Internal data stream error."), ("stream stopped, reason %s", gst_flow_get_name (flow_ret)));
      gst_pad_push_event (GST_VIDEO_ENCODER_SRC_PAD (self), gst_event_new_eos ());
      gst_pad_pause_task (GST_VIDEO_ENCODER_SRC_PAD (self));
    }
    self->started = FALSE;
...
release_error:
  {
    GST_ELEMENT_ERROR (self, LIBRARY, SETTINGS, (NULL), ("Failed to relase output buffer to component: %s (0x%08x)", gst_omx_error_to_string (err), err));
    gst_pad_push_event (GST_VIDEO_ENCODER_SRC_PAD (self), gst_event_new_eos ());
    gst_pad_pause_task (GST_VIDEO_ENCODER_SRC_PAD (self));
    self->downstream_flow_ret = GST_FLOW_ERROR;
    self->started = FALSE;
...
gst_omx_video_enc_get_supported_colorformats (GstOMXVideoEnc * self)
{
  ...
  do {
    ...
    err = gst_omx_component_get_parameter (self->enc, OMX_IndexParamVideoPortFormat, &param);
    ...
    if(port->comp->core->component_vendor == GST_OMX_VENDOR_QCT)
      param.eColorFormat = OMX_EXT_COLOR_FormatNV12LPhysicalAddress;
    ...
    if (err == OMX_ErrorNone || err == OMX_ErrorNoMore) {
      switch (param.eColorFormat) {
        ...
        case OMX_SPRD_COLOR_FormatYUV420SemiPlanar:
            ...
            break;
        default:
          GST_DEBUG_OBJECT (self, "Component supports unsupported color format %d at index %d", param.eColorFormat, param.nIndex);
          break;
      }
    }
    ...
    GstVideoCodecState * state)
{
  ...
  GST_DEBUG_OBJECT (self, "Setting new format %s", gst_video_format_to_string (info->finfo->format));

  gst_omx_port_get_port_definition (self->enc_in_port, &port_def);

  needs_disable = gst_omx_component_get_state (self->enc, GST_CLOCK_TIME_NONE) != OMX_StateLoaded;
  ...
  if (needs_disable) {
    ...
    if (gst_omx_port_set_enabled (self->enc_out_port, FALSE) != OMX_ErrorNone)
      return FALSE;
    if (gst_omx_port_wait_buffers_released (self->enc_in_port, 5 * GST_SECOND) != OMX_ErrorNone)
      return FALSE;
    if (gst_omx_port_wait_buffers_released (self->enc_out_port, 1 * GST_SECOND) != OMX_ErrorNone)
      return FALSE;
    if (gst_omx_port_deallocate_buffers (self->enc_in_port) != OMX_ErrorNone)
      return FALSE;
    if (gst_omx_port_deallocate_buffers (self->enc_out_port) != OMX_ErrorNone)
      return FALSE;
    if (gst_omx_port_wait_enabled (self->enc_in_port, 1 * GST_SECOND) != OMX_ErrorNone)
      return FALSE;
    if (gst_omx_port_wait_enabled (self->enc_out_port, 1 * GST_SECOND) != OMX_ErrorNone)
      return FALSE;
  ...
  if (!negotiation_map) {
    ...
    switch (info->finfo->format) {
      ...
      case GST_VIDEO_FORMAT_NV12:
        ...
        break;
      default:
        GST_ERROR_OBJECT (self, "Unsupported format %s", gst_video_format_to_string (info->finfo->format));
        return FALSE;
        break;
    }
  } else {
...
#ifdef USE_OMX_TARGET_QCOM
  port_def.format.video.nStride = ALIGN(info->width, 128);       /* safe (?) default */
#else
  if (port_def.nBufferAlignment)
    port_def.format.video.nStride = (info->width + port_def.nBufferAlignment - 1) & (~(port_def.nBufferAlignment - 1));
  else
...
#ifdef USE_OMX_TARGET_QCOM
  port_def.format.video.nSliceHeight = ALIGN(info->height, 128);
#else
  ...
  switch (port_def.format.video.eColorFormat) {
    case OMX_COLOR_FormatYUV420Planar:
    case OMX_COLOR_FormatYUV420PackedPlanar:
      port_def.nBufferSize = (port_def.format.video.nStride * port_def.format.video.nFrameHeight) +
                                             2 * ((port_def.format.video.nStride / 2) * ((port_def.format.video.nFrameHeight + 1) / 2));
      break;

    case OMX_COLOR_FormatYUV420SemiPlanar:
      port_def.nBufferSize = (port_def.format.video.nStride * port_def.format.video.nFrameHeight) +
                                             (port_def.format.video.nStride * ((port_def.format.video.nFrameHeight + 1) / 2));
      break;
  ...
  if (needs_disable) {
    ...
    if (gst_omx_component_get_state (self->enc, GST_CLOCK_TIME_NONE) != OMX_StateIdle)
      return FALSE;

    if (gst_omx_component_set_state (self->enc, OMX_StateExecuting) != OMX_ErrorNone)
      return FALSE;

    if (gst_omx_component_get_state (self->enc, GST_CLOCK_TIME_NONE) != OMX_StateExecuting)
      return FALSE;
  }
  ...
  self->downstream_flow_ret = GST_FLOW_OK;
  gst_pad_start_task (GST_VIDEO_ENCODER_SRC_PAD (self), (GstTaskFunction) gst_omx_video_enc_loop, encoder, NULL);
    ...
    GstOMXBuffer * outbuf)
{
  ...
  if (info->width != port_def->format.video.nFrameWidth || info->height != port_def->format.video.nFrameHeight) {
    GST_ERROR_OBJECT (self, "Width or height do not match");
    goto done;
  }

  /* Same strides and everything */
  if (gst_buffer_get_size (inbuf) == outbuf->omx_buf->nAllocLen - outbuf->omx_buf->nOffset) {
    outbuf->omx_buf->nFilledLen = gst_buffer_get_size (inbuf);

    gst_buffer_extract (inbuf, 0, outbuf->omx_buf->pBuffer + outbuf->omx_buf->nOffset, outbuf->omx_buf->nFilledLen);
    ret = TRUE;
    goto done;
  }
  ...
  switch (info->finfo->format) {
    case GST_VIDEO_FORMAT_I420:{
      ...
      for (i = 0; i < 3; i++) {
        ...
        if ((dest + dest_stride * height) > (outbuf->omx_buf->pBuffer + outbuf->omx_buf->nAllocLen)) {
    ...
    case GST_VIDEO_FORMAT_NV12:{
      ...
      for (i = 0; i < 2; i++) {
        ...
        dest = outbuf->omx_buf->pBuffer + outbuf->omx_buf->nOffset;
        if (i == 1)
          dest += port_def->format.video.nSliceHeight * port_def->format.video.nStride;
    ...
    case GST_VIDEO_FORMAT_SN12:{
        ...
        if (!ext_memory) {
            GST_WARNING_OBJECT (self, "null MMVideoBuffer in hw color format. skip this.");
            goto done;
        }
        ...
        if (ext_buf != NULL && ext_buf->type == MM_VIDEO_BUFFER_TYPE_DMABUF_FD) {
          GST_LOG_OBJECT (self, "enc. fd[0]:%d  fd[1]:%d  fd[2]:%d  w[0]:%d  h[0]:%d   buf_share_method:%d",
              ext_buf->handle.dmabuf_fd[0], ext_buf->handle.dmabuf_fd[0], ext_buf->handle.dmabuf_fd[0], ext_buf->width[0], ext_buf->height[0], ext_buf->type);
        }
        else if (ext_buf != NULL && ext_buf->type == MM_VIDEO_BUFFER_TYPE_TBM_BO)
        {
          tbm_bo_handle handle_fd;

          handle_fd = tbm_bo_get_handle (ext_buf->handle.bo[0], TBM_DEVICE_MM);
          ext_buf->handle.dmabuf_fd[0] = handle_fd.u32;

          if (ext_buf->handle.dmabuf_fd[0] == 0) {
            GST_ERROR_OBJECT (self, "input TIZEN_BUFFER(%p) with Wrong FD: bo[0]:%p fd[0]:%d a[0]:%p p[0]:%p w[0]:%d h[0]:%d",
                inbuf, ext_buf->handle.bo[0], ext_buf->handle.dmabuf_fd[0], ext_buf->data[0], ext_buf->handle.paddr[0], ext_buf->width[0], ext_buf->height[0]);
          } else {
            GST_DEBUG_OBJECT (self, "input TIZEN_BUFFER(%p): bo[0]:%p fd[0]:%d a[0]:%p p[0]:%p w[0]:%d h[0]:%d",
                inbuf, ext_buf->handle.bo[0], ext_buf->handle.dmabuf_fd[0], ext_buf->data[0], ext_buf->handle.paddr[0], ext_buf->width[0], ext_buf->height[0]);
          }
        }
        else if (ext_buf != NULL && ext_buf->type == MM_VIDEO_BUFFER_TYPE_PHYSICAL_ADDRESS)
        {
          GST_DEBUG_OBJECT (self, "enc. fd[0]:%d  fd[1]:%d  fd[2]:%d  w[0]:%d  h[0]:%d   buf_share_method:%d",
              ext_buf->handle.paddr[0], ext_buf->handle.paddr[1], ext_buf->handle.paddr[2], ext_buf->width[0], ext_buf->height[0], ext_buf->type);
        }
        else if (ext_buf != NULL && ext_buf->type == 0)
        {
          GST_WARNING_OBJECT (self, "enc input buf uses hw addr");
        }
        else
        {
...
#ifdef CODEC_ENC_INPUT_DUMP
        gst_omx_video_enc_input_dump (ext_buf, self->enc_in_port->comp->core->component_vendor);
#endif

#ifdef USE_OMX_TARGET_SPRD
        outbuf->omx_buf->nFilledLen = sizeof(sprd_encoder_meta_data);
        gst_omx_video_enc_create_meta_data(outbuf->omx_buf->pBuffer, ext_buf, 0);
#elif defined(USE_OMX_TARGET_QCOM)
        //private_data *private_object = g_new0(private_data, 1);
        int aligned_size = MSM_ALIGN(ext_buf->width[0], ext_buf->height[0]);

        if (gst_omx_video_enc_create_meta_data(outbuf->omx_buf->pBuffer, ext_buf, aligned_size)) {
          outbuf->omx_buf->nFilledLen = sizeof(encoder_media_buffer_type);
        } else {
          GST_DEBUG_OBJECT (self, "could not allocate meta data structures");
        }

        if (!create_platform_private_object(self->enc_in_port, outbuf, 0)) {
          GST_DEBUG_OBJECT (self, "could not allocate private data structures");
        }
#else
    ...
    GstVideoCodecFrame * frame)
{
  ...
  while (acq_ret != GST_OMX_ACQUIRE_BUFFER_OK) {
    ...
    if (GST_VIDEO_CODEC_FRAME_IS_FORCE_KEYFRAME (frame)) {
      ...
      err = gst_omx_component_set_config (self->enc, OMX_IndexConfigVideoIntraVOPRefresh, &config);
      if (err != OMX_ErrorNone)
        GST_ERROR_OBJECT (self, "Failed to force a keyframe: %s (0x%08x)", gst_omx_error_to_string (err), err);
    }
    ...
    if (duration != GST_CLOCK_TIME_NONE) {
      buf->omx_buf->nTickCount = gst_util_uint64_scale (buf->omx_buf->nFilledLen, duration, gst_buffer_get_size (frame->input_buffer));
...
full_buffer:
  {
    GST_ELEMENT_ERROR (self, LIBRARY, FAILED, (NULL), ("Got OpenMAX buffer with no free space (%p, %u/%u)", buf, buf->omx_buf->nOffset, buf->omx_buf->nAllocLen));
    gst_omx_video_enc_release_frame (frame);
...
reconfigure_error:
  {
    GST_ELEMENT_ERROR (self, LIBRARY, SETTINGS, (NULL), ("Unable to reconfigure input port"));
    gst_omx_video_enc_release_frame (frame);
    return GST_FLOW_ERROR;
  }
buffer_fill_error:
  {
    GST_ELEMENT_ERROR (self, RESOURCE, WRITE, (NULL), ("Failed to write input into the OpenMAX buffer"));
    gst_omx_video_enc_release_frame (frame);
    return GST_FLOW_ERROR;
  }
release_error:
  {
    gst_omx_video_enc_release_frame (frame);
    GST_ELEMENT_ERROR (self, LIBRARY, SETTINGS, (NULL), ("Failed to relase input buffer to component: %s (0x%08x)", gst_omx_error_to_string (err), err));
    return GST_FLOW_ERROR;
  }
}
    ...
    GstQuery * query)
{
  ...
  return GST_VIDEO_ENCODER_CLASS (gst_omx_video_enc_parent_class)->propose_allocation (encoder, query);
}
...
gst_omx_video_enc_getcaps (GstVideoEncoder * encoder, GstCaps * filter)
{
  ...
  if (!gst_caps_is_empty (comp_supported_caps)) {
    GstCaps *ret = gst_video_encoder_proxy_getcaps (encoder, comp_supported_caps, filter);
...
static void
gst_omx_video_enc_release_frame (GstVideoCodecFrame * frame)
{
  g_return_if_fail (frame != NULL);
  g_return_if_fail (frame->ref_count > 0);

  if (frame->input_buffer)
    GST_DEBUG ("unref input buffer : %p", frame->input_buffer);
