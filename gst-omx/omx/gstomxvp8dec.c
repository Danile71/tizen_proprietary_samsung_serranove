...
/* prototypes */
static gboolean gst_omx_vp8_dec_is_format_change (GstOMXVideoDec * dec, GstOMXPort * port, GstVideoCodecState * state);
static gboolean gst_omx_vp8_dec_set_format (GstOMXVideoDec * dec, GstOMXPort * port, GstVideoCodecState * state);
...
G_DEFINE_TYPE_WITH_CODE (GstOMXVP8Dec, gst_omx_vp8_dec, GST_TYPE_OMX_VIDEO_DEC, DEBUG_INIT);
...
gst_omx_vp8_dec_class_init (GstOMXVP8DecClass * klass)
{
  ...
  videodec_class->is_format_change = GST_DEBUG_FUNCPTR (gst_omx_vp8_dec_is_format_change);
  videodec_class->set_format = GST_DEBUG_FUNCPTR (gst_omx_vp8_dec_set_format);

  videodec_class->cdata.default_sink_template_caps = "video/x-vp8, " "width=(int) [1,MAX], " "height=(int) [1,MAX]";
