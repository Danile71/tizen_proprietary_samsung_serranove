  ...
  if (hlsdemux->current_key) {
    ...
    if (hlsdemux->key_url
      ...
      key_fragment =
          ...
          hlsdemux->client->main->uri : NULL, demux->user_agent, demux->cookies, FALSE, FALSE,
...
#ifdef GST_ADAPTIVE_MODIFICATION
//Cookie / UA Pointer Corruption is seen during HLS Streaming .
//Defensive patch is kept to get new renewed cookie/UA pointer  before passing it on to UpStream Soup element.
  gst_adaptive_demux_get_cookies(adaptive_demux);
  gst_adaptive_demux_get_user_agent(adaptive_demux);

  download =
      gst_uri_downloader_fetch_uri (adaptive_demux->downloader, uri, main_uri, adaptive_demux->user_agent, adaptive_demux->cookies,
      TRUE, TRUE, TRUE, err);
#else
  download =
  ...
  if (download == NULL) {
    if (update && !main_checked
      ...
      download =
          ...
          main_uri, NULL, adaptive_demux->user_agent, adaptive_demux->cookies, TRUE, TRUE, TRUE, &err2);
#else
      download =
          gst_uri_downloader_fetch_uri (adaptive_demux->downloader,
