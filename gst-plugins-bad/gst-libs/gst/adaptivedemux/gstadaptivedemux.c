...
#ifdef GST_ADAPTIVE_MODIFICATION
        /* Get UA / Cookies from upstream src element */
        gst_adaptive_demux_get_user_agent(demux);
        gst_adaptive_demux_get_cookies(demux);
#endif
...
#ifdef GST_ADAPTIVE_MODIFICATION
gboolean
gst_adaptive_demux_get_cookies(GstAdaptiveDemux *demux)
{
  GstQuery *cquery;
  ...
  res = gst_pad_peer_query (demux->sinkpad, cquery);
  if (res) {
    const GstStructure *s;
    const GValue *value;
    ...
    GST_INFO_OBJECT (demux, "Received cookies from upstream element : %s", demux->cookies? *(demux->cookies) : NULL);
    ret = TRUE;
  } else {
...
gboolean
gst_adaptive_demux_get_user_agent(GstAdaptiveDemux *demux)
{
  GstQuery *cquery;
  GstStructure *structure;
  gboolean res = FALSE;
  ...
  if (res) {
    ...
    GST_INFO_OBJECT (demux, "Received UA from upstream element : %s", demux->user_agent);
    ret = TRUE;
  } else {
...
gst_adaptive_demux_update_manifest_default (GstAdaptiveDemux * demux)
{
  ...
  download = gst_uri_downloader_fetch_uri (demux->downloader,
      demux->manifest_uri, NULL, demux->user_agent, demux->cookies, TRUE, TRUE, TRUE, NULL);
#else
  download = gst_uri_downloader_fetch_uri (demux->downloader,
