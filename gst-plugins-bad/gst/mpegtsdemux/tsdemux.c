...
#endif
//caps modification done for both audio and video streams by adding hls_streaming flag.
//This flag will be used by parser / decoder to identify whether the stream is a HLS stream or not for their processing.
    ...
    gst_caps_set_simple (caps,"hls_streaming", G_TYPE_BOOLEAN, TRUE, NULL);
    GST_DEBUG("*** modified caps for hls_streaming = %" GST_PTR_FORMAT,
        caps);
#endif
...
#ifdef GST_TSDEMUX_MODIFICATION
    /* Push pending tags */
    if (stream->taglist) {
  ...
  for (iter = demux->program->stream_list; iter; iter = g_list_next (iter)) {
    ...
    if (stream->pad) {
      ...
      gst_ts_demux_stream_send_stream_start ((MpegTSBase *) demux,
          (MpegTSBaseStream *) stream, stream->pad);  /* from patch 0005 */
