...
#ifdef GST_BUG_PATCH
static void gst_h264_parse_reset (GstBaseParse * parse);
#endif
...
#ifdef GST_BUG_PATCH
  parse_class->reset = GST_DEBUG_FUNCPTR (gst_h264_parse_reset);
#endif
...
#ifdef GST_BUG_PATCH
static void
gst_h264_parse_reset (GstBaseParse * parse)
{
  GstH264Parse *h264parse = (GstH264Parse *)parse;
  h264parse->state = 0;
#else
static void
...
#ifdef GST_H264PARSE_MODIFICATION
      /*MODIFICATION : for the case which codec_data already had SPS/PPS,
                        but frames not also contains really frame_data, but also has SPS or PPS*/
      if (!h264parse->header && !GST_H264_PARSE_STATE_VALID (h264parse,
              GST_H264_PARSE_STATE_VALID_PICTURE_HEADERS))
        return FALSE;
#else
