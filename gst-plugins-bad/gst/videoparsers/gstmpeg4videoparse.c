...
gst_mpeg4vparse_update_src_caps (GstMpeg4VParse * mp4vparse)
{
  ...
  /* perhaps we have a framerate */
  {
    ...
    // During Live HTTP / RTSP mpeg4 links , since fixed_vop_time_increment value is 0 , so framerate field does not get set
    // As a result , it creates a problem for decodebin for selecting sprddec_mpeg4.
    // Temporary patch kept for having a non-zero fps_den value.
    if (fps_den == 0)
    {
	GST_LOG_OBJECT (mp4vparse, "Forcefully setting fixed_vop_time_increment to 1 for Live Streams");
	fps_den = 1;
    }
