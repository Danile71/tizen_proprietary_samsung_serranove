...
#ifdef GST_EXT_VIDEODECODER
#define MAX_BUFFER_LOG_CNT 20
#define BUFFER_LOG_INTERVAL 50
#endif
...
static void gst_video_decoder_init (GstVideoDecoder * dec, GstVideoDecoderClass * klass);

static void gst_video_decoder_finalize (GObject * object);

static gboolean gst_video_decoder_setcaps (GstVideoDecoder * dec, GstCaps * caps);
static gboolean gst_video_decoder_sink_event (GstPad * pad, GstObject * parent, GstEvent * event);
static gboolean gst_video_decoder_src_event (GstPad * pad, GstObject * parent, GstEvent * event);
static GstFlowReturn gst_video_decoder_chain (GstPad * pad, GstObject * parent, GstBuffer * buf);
static gboolean gst_video_decoder_sink_query (GstPad * pad, GstObject * parent, GstQuery * query);
static GstStateChangeReturn gst_video_decoder_change_state (GstElement * element, GstStateChange transition);
static gboolean gst_video_decoder_src_query (GstPad * pad, GstObject * parent, GstQuery * query);
static void gst_video_decoder_reset (GstVideoDecoder * decoder, gboolean full, gboolean flush_hard);

static GstFlowReturn gst_video_decoder_decode_frame (GstVideoDecoder * decoder, GstVideoCodecFrame * frame);

static GstClockTime gst_video_decoder_get_frame_duration (GstVideoDecoder * decoder, GstVideoCodecFrame * frame);
static GstVideoCodecFrame *gst_video_decoder_new_frame (GstVideoDecoder * decoder);
static GstFlowReturn gst_video_decoder_clip_and_push_buf (GstVideoDecoder * decoder, GstBuffer * buf);
static GstFlowReturn gst_video_decoder_flush_parse (GstVideoDecoder * dec, gboolean at_eos);

static void gst_video_decoder_clear_queues (GstVideoDecoder * dec);

static gboolean gst_video_decoder_sink_event_default (GstVideoDecoder * decoder, GstEvent * event);
static gboolean gst_video_decoder_src_event_default (GstVideoDecoder * decoder, GstEvent * event);
static gboolean gst_video_decoder_decide_allocation_default (GstVideoDecoder * decoder, GstQuery * query);
static gboolean gst_video_decoder_propose_allocation_default (GstVideoDecoder * decoder, GstQuery * query);
static gboolean gst_video_decoder_negotiate_default (GstVideoDecoder * decoder);
static GstFlowReturn gst_video_decoder_parse_available (GstVideoDecoder * dec, gboolean at_eos, gboolean new_buffer);
static gboolean gst_video_decoder_negotiate_unlocked (GstVideoDecoder * decoder);
static gboolean gst_video_decoder_sink_query_default (GstVideoDecoder * decoder, GstQuery * query);
static gboolean gst_video_decoder_src_query_default (GstVideoDecoder * decoder, GstQuery * query);
...
gst_video_decoder_get_type (void)
{
  ...
  if (g_once_init_enter (&type)) {
    ...
    _type = g_type_register_static (GST_TYPE_ELEMENT, "GstVideoDecoder", &info, G_TYPE_FLAG_ABSTRACT);
...
gst_video_decoder_class_init (GstVideoDecoderClass * klass)
{
  ...
  GST_DEBUG_CATEGORY_INIT (videodecoder_debug, "videodecoder", 0, "Base Video Decoder");
  ...
  gstelement_class->change_state = GST_DEBUG_FUNCPTR (gst_video_decoder_change_state);
...
gst_video_decoder_init (GstVideoDecoder * decoder, GstVideoDecoderClass * klass)
{
  ...
  gst_pad_set_event_function (pad, GST_DEBUG_FUNCPTR (gst_video_decoder_sink_event));
  gst_pad_set_query_function (pad, GST_DEBUG_FUNCPTR (gst_video_decoder_sink_query));
  ...
  gst_pad_set_event_function (pad, GST_DEBUG_FUNCPTR (gst_video_decoder_src_event));
  gst_pad_set_query_function (pad, GST_DEBUG_FUNCPTR (gst_video_decoder_src_query));
  ...
  decoder->num_live_buffer = 0;

  gst_video_decoder_reset (decoder, TRUE, TRUE);
}

static gboolean
gst_video_rawvideo_convert (GstVideoCodecState * state, GstFormat src_format, gint64 src_value, GstFormat * dest_format, gint64 * dest_value)
{
  gboolean res = FALSE;
  guint vidsize;
  guint fps_n, fps_d;
  ...
  if (src_format == GST_FORMAT_BYTES && *dest_format == GST_FORMAT_DEFAULT && vidsize) {
    /* convert bytes to frames */
    *dest_value = gst_util_uint64_scale_int (src_value, 1, vidsize);
    res = TRUE;
  } else if (src_format == GST_FORMAT_DEFAULT && *dest_format == GST_FORMAT_BYTES && vidsize) {
    /* convert bytes to frames */
    *dest_value = src_value * vidsize;
    res = TRUE;
  } else if (src_format == GST_FORMAT_DEFAULT && *dest_format == GST_FORMAT_TIME && fps_n) {
    /* convert frames to time */
    *dest_value = gst_util_uint64_scale (src_value, GST_SECOND * fps_d, fps_n);
    res = TRUE;
  } else if (src_format == GST_FORMAT_TIME && *dest_format == GST_FORMAT_DEFAULT && fps_d) {
    /* convert time to frames */
    *dest_value = gst_util_uint64_scale (src_value, fps_n, GST_SECOND * fps_d);
    res = TRUE;
  } else if (src_format == GST_FORMAT_TIME && *dest_format == GST_FORMAT_BYTES && fps_d && vidsize) {
    /* convert time to bytes */
    *dest_value = gst_util_uint64_scale (src_value, fps_n * (guint64) vidsize, GST_SECOND * fps_d);
    res = TRUE;
  } else if (src_format == GST_FORMAT_BYTES && *dest_format == GST_FORMAT_TIME && fps_n && vidsize) {
    /* convert bytes to time */
    *dest_value = gst_util_uint64_scale (src_value, GST_SECOND * fps_d, fps_n * (guint64) vidsize);
    res = TRUE;
  }
...
static gboolean
gst_video_encoded_video_convert (gint64 bytes, gint64 time, GstFormat src_format, gint64 src_value, GstFormat * dest_format, gint64 * dest_value)
{
  gboolean res = FALSE;
  ...
  switch (src_format) {
    ...
    case GST_FORMAT_TIME:
      ...
      break;
    default:
      GST_DEBUG ("unhandled conversion from %d to %d", src_format, *dest_format);
      res = FALSE;
  }
...
static GstVideoCodecState *
_new_output_state (GstVideoFormat fmt, guint width, guint height, GstVideoCodecState * reference)
{
  GstVideoCodecState *state;
  ...
  if (reference) {
    ...
    GST_DEBUG ("reference par %d/%d fps %d/%d", ref->par_n, ref->par_d, ref->fps_n, ref->fps_d);
    tgt->par_n = ref->par_n;
    tgt->par_d = ref->par_d;
    tgt->fps_n = ref->fps_n;
    tgt->fps_d = ref->fps_d;
  ...
  GST_DEBUG ("reference par %d/%d fps %d/%d", state->info.par_n, state->info.par_d, state->info.fps_n, state->info.fps_d);
...
gst_video_decoder_setcaps (GstVideoDecoder * decoder, GstCaps * caps)
{
  ...
  if (decoder->priv->input_state) {
    GST_DEBUG_OBJECT (decoder, "Checking if caps changed old %" GST_PTR_FORMAT " new %" GST_PTR_FORMAT,
...
gst_video_decoder_push_event (GstVideoDecoder * decoder, GstEvent * event)
{
  ...
  GST_DEBUG_OBJECT (decoder, "pushing event %s", gst_event_type_get_name (GST_EVENT_TYPE (event)));

  return gst_pad_push_event (decoder->srcpad, event);
}

static GstFlowReturn
gst_video_decoder_parse_available (GstVideoDecoder * dec, gboolean at_eos, gboolean new_buffer)
{
  ...
  while (available || new_buffer) {
    ...
    ret = decoder_class->parse (dec, priv->current_frame, priv->input_adapter, at_eos);
    if (ret != GST_FLOW_OK)
      break;
...
static gboolean
gst_video_decoder_sink_event_default (GstVideoDecoder * decoder, GstEvent * event)
{
  ...
  switch (GST_EVENT_TYPE (event)) {
...
#ifdef GST_EXT_VIDEODECODER
      /* Push tag event. Videodecoder didn't send tag event before runnig chain. */
    ...
    case GST_EVENT_SEGMENT:
    {
      ...
      if (segment.format == GST_FORMAT_TIME) {
        ...
        if (priv->do_estimate_rate &&
            gst_pad_query_convert (decoder->sinkpad, GST_FORMAT_BYTES, segment.start, GST_FORMAT_TIME, &start)) {
          /* best attempt convert */
          /* as these are only estimates, stop is kept open-ended to avoid
           * premature cutting */
          GST_DEBUG_OBJECT (decoder, "converted to TIME start %" GST_TIME_FORMAT, GST_TIME_ARGS (start));
          segment.start = start;
    ...
    case GST_EVENT_FLUSH_STOP:
    {
      ...
      priv->current_frame_events = _flush_events (decoder->srcpad, decoder->priv->current_frame_events);
  ...
  if (event) {
    if (!GST_EVENT_IS_SERIALIZED (event) || forward_immediate) {
      ...
      GST_VIDEO_DECODER_STREAM_LOCK (decoder);
      decoder->priv->current_frame_events = g_list_prepend (decoder->priv->current_frame_events, event);
      GST_VIDEO_DECODER_STREAM_UNLOCK (decoder);
      ret = TRUE;
    }
  }
...
static gboolean
gst_video_decoder_sink_event (GstPad * pad, GstObject * parent, GstEvent * event)
{
  GstVideoDecoder *decoder;
  GstVideoDecoderClass *decoder_class;
  gboolean ret = FALSE;
  ...
  GST_DEBUG_OBJECT (decoder, "received event %d, %s", GST_EVENT_TYPE (event), GST_EVENT_TYPE_NAME (event));
...
{
  return dec->priv->do_estimate_rate && (dec->priv->bytes_out > 0) && (dec->priv->time > GST_SECOND);
}
...
gst_video_decoder_do_seek (GstVideoDecoder * dec, GstEvent * event)
{
  ...
  gst_event_parse_seek (event, &rate, &format, &flags, &start_type, &start_time, &end_type, &end_time);
  ...
  gst_segment_do_seek (&seek_segment, rate, format, flags, start_type, start_time, end_type, end_time, NULL);
  start_time = seek_segment.position;

  if (!gst_pad_query_convert (dec->sinkpad, GST_FORMAT_TIME, start_time, GST_FORMAT_BYTES, &start)) {
    GST_DEBUG_OBJECT (dec, "conversion failed");
    return FALSE;
  }

  seqnum = gst_event_get_seqnum (event);
  event = gst_event_new_seek (1.0, GST_FORMAT_BYTES, flags, GST_SEEK_TYPE_SET, start, GST_SEEK_TYPE_NONE, -1);
...
static gboolean
gst_video_decoder_src_event_default (GstVideoDecoder * decoder, GstEvent * event)
{
  GstVideoDecoderPrivate *priv;
  gboolean res = FALSE;
  ...
  switch (GST_EVENT_TYPE (event)) {
    case GST_EVENT_SEEK:
    {
      ...
      if (!(res = gst_pad_query_convert (decoder->srcpad, format, start, GST_FORMAT_TIME, &tstart)))
        goto convert_error;
      if (!(res = gst_pad_query_convert (decoder->srcpad, format, stop, GST_FORMAT_TIME, &tstop)))
        goto convert_error;

      /* then seek with time on the peer */
      event = gst_event_new_seek (rate, GST_FORMAT_TIME, flags, start_type, tstart, stop_type, tstop);
      gst_event_set_seqnum (event, seqnum);
    ...
    case GST_EVENT_QOS:
    {
      ...
      GST_DEBUG_OBJECT (decoder, "got QoS %" GST_TIME_FORMAT ", %" G_GINT64_FORMAT ", %g", GST_TIME_ARGS (timestamp), diff, proportion);
...
gst_video_decoder_src_query_default (GstVideoDecoder * dec, GstQuery * query)
{
  ...
  switch (GST_QUERY_TYPE (query)) {
    case GST_QUERY_POSITION:
    {
      ...
      time = gst_segment_to_stream_time (&dec->output_segment, GST_FORMAT_TIME, time);

      GST_LOG_OBJECT (dec, "query %p: our time: %" GST_TIME_FORMAT, query, GST_TIME_ARGS (time));

      /* and convert to the final format */
      gst_query_parse_position (query, &format, NULL);
      if (!(res = gst_pad_query_convert (pad, GST_FORMAT_TIME, time, format, &value)))
        break;

      gst_query_set_position (query, format, value);

      GST_LOG_OBJECT (dec, "query %p: we return %" G_GINT64_FORMAT " (format %u)", query, value, format);
      break;
    }
    case GST_QUERY_DURATION:
    {
      ...
      if (format == GST_FORMAT_TIME && gst_video_decoder_do_byte (dec)) {
        ...
        if (gst_pad_peer_query_duration (dec->sinkpad, GST_FORMAT_BYTES, &value)) {
          GST_LOG_OBJECT (dec, "upstream size %" G_GINT64_FORMAT, value);
          if (gst_pad_query_convert (dec->sinkpad, GST_FORMAT_BYTES, value, GST_FORMAT_TIME, &value)) {
    ...
    case GST_QUERY_CONVERT:
    {
      ...
      if (dec->priv->output_state != NULL)
        res = gst_video_rawvideo_convert (dec->priv->output_state, src_fmt, src_val, &dest_fmt, &dest_val);
      else
        res = FALSE;
    ...
    case GST_QUERY_LATENCY:
    {
      ...
      if (res) {
        ...
        GST_DEBUG_OBJECT (dec, "Peer qlatency: live %d, min %" GST_TIME_FORMAT " max %" GST_TIME_FORMAT,
...
gst_video_decoder_src_query (GstPad * pad, GstObject * parent, GstQuery * query)
{
  ...
  GST_DEBUG_OBJECT (decoder, "received query %d, %s", GST_QUERY_TYPE (query), GST_QUERY_TYPE_NAME (query));
...
static gboolean
gst_video_decoder_sink_query_default (GstVideoDecoder * decoder, GstQuery * query)
{
  ...
  switch (GST_QUERY_TYPE (query)) {
    case GST_QUERY_CONVERT:
    {
      ...
      gst_query_parse_convert (query, &src_fmt, &src_val, &dest_fmt, &dest_val);
      GST_VIDEO_DECODER_STREAM_LOCK (decoder);
      res = gst_video_encoded_video_convert (priv->bytes_out, priv->time, src_fmt, src_val, &dest_fmt, &dest_val);
      GST_VIDEO_DECODER_STREAM_UNLOCK (decoder);
      if (!res)
        goto error;
      gst_query_set_convert (query, src_fmt, src_val, dest_fmt, dest_val);
...
static gboolean
gst_video_decoder_sink_query (GstPad * pad, GstObject * parent, GstQuery * query)
{
  GstVideoDecoder *decoder;
  GstVideoDecoderClass *decoder_class;
  gboolean ret = FALSE;
...
gst_video_decoder_add_timestamp (GstVideoDecoder * decoder, GstBuffer * buffer)
{
  ...
  GST_LOG_OBJECT (decoder, "adding PTS %" GST_TIME_FORMAT " DTS %" GST_TIME_FORMAT " (offset:%" G_GUINT64_FORMAT ")",
      GST_TIME_ARGS (GST_BUFFER_PTS (buffer)), GST_TIME_ARGS (GST_BUFFER_DTS (buffer)), priv->input_offset);
...
static void
gst_video_decoder_get_timestamp_at_offset (GstVideoDecoder * decoder,
        guint64 offset, GstClockTime * pts, GstClockTime * dts, GstClockTime * duration)
{
  ...
  *duration = GST_CLOCK_TIME_NONE;
  ...
  GST_LOG_OBJECT (decoder, "got PTS %" GST_TIME_FORMAT " DTS %" GST_TIME_FORMAT " @ offs %" G_GUINT64_FORMAT " (wanted offset:%" G_GUINT64_FORMAT ")",
    ...
    gboolean flush_hard)
{
  ...
  if (full || flush_hard) {
    ...
    g_list_free_full (priv->current_frame_events, (GDestroyNotify) gst_event_unref);
  ...
  priv->time = 0;
#ifdef GST_EXT_VIDEODECODER
  priv->inbuf_log_cnt = 0;
  priv->outbuf_log_cnt = 0;
#endif
...
gst_video_decoder_chain_forward (GstVideoDecoder * decoder, GstBuffer * buf, gboolean at_eos)
{
  ...
  if (priv->packetized) {
    ...
    if (decoder->input_segment.rate < 0.0) {
      priv->parse_gather = g_list_prepend (priv->parse_gather, priv->current_frame);
    } else {
...
gst_video_decoder_flush_decode (GstVideoDecoder * dec)
{
  ...
  while (walk) {
    ...
    GST_DEBUG_OBJECT (dec, "decoding frame %p buffer %p, PTS %" GST_TIME_FORMAT ", DTS %" GST_TIME_FORMAT, frame, frame->input_buffer,
        GST_TIME_ARGS (GST_BUFFER_PTS (frame->input_buffer)), GST_TIME_ARGS (GST_BUFFER_DTS (frame->input_buffer)));
...
gst_video_decoder_flush_parse (GstVideoDecoder * dec, gboolean at_eos)
{
  ...
  while (walk) {
    ...
    GST_DEBUG_OBJECT (dec, "parsing buffer %p, PTS %" GST_TIME_FORMAT ", DTS %" GST_TIME_FORMAT,
        buf, GST_TIME_ARGS (GST_BUFFER_PTS (buf)), GST_TIME_ARGS (GST_BUFFER_DTS (buf)));
    ...
    for (walk2 = frame->events; walk2;) {
      ...
      if (GST_EVENT_TYPE (event) <= GST_EVENT_SEGMENT) {
        ...
        if (GST_EVENT_TYPE (event) == GST_EVENT_SEGMENT) {
          ...
          GST_DEBUG_OBJECT (dec, "Segment at frame %p %" GST_TIME_FORMAT, frame, GST_TIME_ARGS (GST_BUFFER_PTS (frame->input_buffer)));
          gst_event_copy_segment (event, &segment);
          if (segment.format == GST_FORMAT_TIME) {
            dec->output_segment = segment;
          }
        }
        dec->priv->pending_events = g_list_append (dec->priv->pending_events, event);
  ...
  while (walk) {
    ...
    if (G_LIKELY (res == GST_FLOW_OK)) {
      ...
      else if (priv->last_timestamp_out != GST_CLOCK_TIME_NONE && GST_BUFFER_DURATION (buf) != GST_CLOCK_TIME_NONE) {
        GST_BUFFER_TIMESTAMP (buf) = priv->last_timestamp_out - GST_BUFFER_DURATION (buf);
        priv->last_timestamp_out = GST_BUFFER_TIMESTAMP (buf);
        GST_LOG_OBJECT (dec, "Calculated TS %" GST_TIME_FORMAT " working backwards", GST_TIME_ARGS (priv->last_timestamp_out));
      }
    ...
    priv->output_queued = g_list_delete_link (priv->output_queued, priv->output_queued);
    walk = priv->output_queued;
  }
...
gst_video_decoder_chain (GstPad * pad, GstObject * parent, GstBuffer * buf)
{
...
#ifdef GST_EXT_VIDEODECODER
  if ((decoder->priv->inbuf_log_cnt < MAX_BUFFER_LOG_CNT)||(decoder->priv->inbuf_log_cnt % BUFFER_LOG_INTERVAL == 0)) {
    GST_WARNING_OBJECT (decoder, "IN_BUF [%d]: gst_buf= %p PTS= %" GST_TIME_FORMAT " DTS %" GST_TIME_FORMAT " size= %u",
      decoder->priv->inbuf_log_cnt, buf, GST_TIME_ARGS(GST_BUFFER_PTS (buf)), GST_TIME_ARGS (GST_BUFFER_DTS (buf)), gst_buffer_get_size (buf));
  } else {
    GST_LOG_OBJECT (decoder, "IN_BUF : gst_buf= %p PTS= %" GST_TIME_FORMAT " DTS %" GST_TIME_FORMAT " size= %u",
      buf, GST_TIME_ARGS(GST_BUFFER_PTS (buf)), GST_TIME_ARGS (GST_BUFFER_DTS (buf)), gst_buffer_get_size (buf));
  }
  decoder->priv->inbuf_log_cnt++;
#else
  GST_LOG_OBJECT (decoder, "chain PTS %" GST_TIME_FORMAT ", DTS %" GST_TIME_FORMAT " duration %" GST_TIME_FORMAT " size %" G_GSIZE_FORMAT,
    GST_TIME_ARGS (GST_BUFFER_PTS (buf)),GST_TIME_ARGS (GST_BUFFER_DTS (buf)),GST_TIME_ARGS (GST_BUFFER_DURATION (buf)), gst_buffer_get_size (buf));
#endif
  ...
  if (decoder->input_segment.format == GST_FORMAT_UNDEFINED) {
    ...
    GST_WARNING_OBJECT (decoder, "Received buffer without a new-segment. Assuming timestamps start from 0.");
...
not_negotiated:
  {
    GST_ELEMENT_ERROR (decoder, CORE, NEGOTIATION, (NULL), ("decoder not initialized"));
    gst_buffer_unref (buf);
...
gst_video_decoder_change_state (GstElement * element, GstStateChange transition)
{
  ...
  switch (transition) {
    case GST_STATE_CHANGE_NULL_TO_READY:
      ...
      GST_WARNING_OBJECT(decoder, "GST_STATE_CHANGE_NULL_TO_READY");
      if (decoder_class->open && !decoder_class->open (decoder))
        goto open_failed;
      break;
    case GST_STATE_CHANGE_READY_TO_PAUSED:
      /* Initialize device/library if needed */
      GST_WARNING_OBJECT(decoder, "GST_STATE_CHANGE_READY_TO_PAUSED");
    ...
    case GST_STATE_CHANGE_PAUSED_TO_PLAYING:
      GST_WARNING_OBJECT(decoder, "GST_STATE_CHANGE_PAUSED_TO_PLAYING");
      break;
    default:
      break;
  }
  ...
  switch (transition) {
    case GST_STATE_CHANGE_PLAYING_TO_PAUSED:
      GST_WARNING_OBJECT(decoder, "GST_STATE_CHANGE_PLAYING_TO_PAUSED");
#ifdef GST_EXT_VIDEODECODER
      decoder->priv->inbuf_log_cnt = 0;
      decoder->priv->outbuf_log_cnt = 0;
#endif
      break;
    case GST_STATE_CHANGE_PAUSED_TO_READY:{
      GST_WARNING_OBJECT(decoder, "GST_STATE_CHANGE_PAUSED_TO_READY");
      ...
      if (decoder_class->stop) {
        ...
        GST_VIDEO_DECODER_STREAM_LOCK (decoder);
        g_list_free_full (decoder->priv->pending_events, (GDestroyNotify) gst_event_unref);
    ...
    case GST_STATE_CHANGE_READY_TO_NULL:
      ...
      GST_WARNING_OBJECT(decoder, "GST_STATE_CHANGE_READY_TO_NULL");
...
static void
gst_video_decoder_prepare_finish_frame (GstVideoDecoder * decoder, GstVideoCodecFrame * frame, gboolean dropping)
{
  GstVideoDecoderPrivate *priv = decoder->priv;
  GList *l, *events = NULL;
  gboolean sync;
  ...
  GST_LOG_OBJECT (decoder,
      "finish frame %p (#%d) sync:%d PTS:%" GST_TIME_FORMAT " DTS:%" GST_TIME_FORMAT,
      frame, frame->system_frame_number, sync, GST_TIME_ARGS (frame->pts), GST_TIME_ARGS (frame->dts));
  ...
  if (dropping || !decoder->priv->output_state) {
    ...
    decoder->priv->pending_events = g_list_concat (decoder->priv->pending_events, events);
  } else {
    for (l = g_list_last (decoder->priv->pending_events); l; l = g_list_previous (l)) {
  ...
  if (GST_CLOCK_TIME_IS_VALID (frame->pts)) {
    if (frame->pts != priv->base_timestamp) {
      GST_DEBUG_OBJECT (decoder, "sync timestamp %" GST_TIME_FORMAT " diff %" GST_TIME_FORMAT,
  ...
  if (frame->duration == GST_CLOCK_TIME_NONE) {
    ...
    GST_LOG_OBJECT (decoder, "Guessing duration %" GST_TIME_FORMAT " for frame...", GST_TIME_ARGS (frame->duration));
  }
   ...
   * so a good guess is lowest unsent DTS */
  {
    ...
    if (!priv->reordered_output && !GST_CLOCK_TIME_IS_VALID (frame->pts) && !seen_none && GST_CLOCK_TIME_IS_VALID (priv->pts_delta)) {
      frame->pts = min_ts + priv->pts_delta;
      GST_DEBUG_OBJECT (decoder, "no valid PTS, using oldest DTS %" GST_TIME_FORMAT, GST_TIME_ARGS (frame->pts));
    }
    ...
    if (!GST_CLOCK_TIME_IS_VALID (frame->pts) && !seen_none) {
      frame->pts = min_ts;
      GST_DEBUG_OBJECT (decoder, "no valid PTS, using oldest PTS %" GST_TIME_FORMAT, GST_TIME_ARGS (frame->pts));
    }
  }
  ...
  if (frame->pts == GST_CLOCK_TIME_NONE) {
    ...
    if (frame->duration != GST_CLOCK_TIME_NONE) {
      ...
      GST_LOG_OBJECT (decoder, "Guessing timestamp %" GST_TIME_FORMAT " for frame...", GST_TIME_ARGS (frame->pts));
    } else if (sync && frame->dts != GST_CLOCK_TIME_NONE) {
      frame->pts = frame->dts;
      GST_LOG_OBJECT (decoder, "Setting DTS as PTS %" GST_TIME_FORMAT " for frame...", GST_TIME_ARGS (frame->pts));
    }
  }

  if (GST_CLOCK_TIME_IS_VALID (priv->last_timestamp_out)) {
    if (frame->pts < priv->last_timestamp_out) {
      GST_WARNING_OBJECT (decoder, "decreasing timestamp (%" GST_TIME_FORMAT " < %" GST_TIME_FORMAT ")",
...
gst_video_decoder_release_frame (GstVideoDecoder * dec, GstVideoCodecFrame * frame)
{
  ...
  if (frame->events) {
    dec->priv->pending_events = g_list_concat (dec->priv->pending_events, frame->events);
    frame->events = NULL;
  }
  GST_VIDEO_DECODER_STREAM_UNLOCK (dec);
...
gst_video_decoder_drop_frame (GstVideoDecoder * dec, GstVideoCodecFrame * frame)
{
  ...
  GST_DEBUG_OBJECT (dec, "dropping frame %" GST_TIME_FORMAT, GST_TIME_ARGS (frame->pts));
  ...
  stream_time = gst_segment_to_stream_time (segment, GST_FORMAT_TIME, timestamp);
  qostime = gst_segment_to_running_time (segment, GST_FORMAT_TIME, timestamp);
  jitter = GST_CLOCK_DIFF (qostime, earliest_time);
  qos_msg = gst_message_new_qos (GST_OBJECT_CAST (dec), FALSE, qostime, stream_time, timestamp, GST_CLOCK_TIME_NONE);
  gst_message_set_qos_values (qos_msg, jitter, proportion, 1000000);
  gst_message_set_qos_stats (qos_msg, GST_FORMAT_BUFFERS, dec->priv->processed, dec->priv->dropped);
 ...
 */
GstFlowReturn
gst_video_decoder_finish_frame (GstVideoDecoder * decoder, GstVideoCodecFrame * frame)
{
  GstFlowReturn ret = GST_FLOW_OK;
  GstVideoDecoderPrivate *priv = decoder->priv;
  ...
  if (G_UNLIKELY (priv->output_state_changed || (priv->output_state && needs_reconfigure))) {
  ...
  if (priv->tags && priv->tags_changed) {
    gst_video_decoder_push_event (decoder, gst_event_new_tag (gst_tag_list_ref (priv->tags)));
    priv->tags_changed = FALSE;
  }

  /* no buffer data means this frame is skipped */
  if (!frame->output_buffer || GST_VIDEO_CODEC_FRAME_IS_DECODE_ONLY (frame)) {
    GST_DEBUG_OBJECT (decoder, "skipping frame %" GST_TIME_FORMAT, GST_TIME_ARGS (frame->pts));
    goto done;
  }
...
gst_video_decoder_clip_and_push_buf (GstVideoDecoder * decoder, GstBuffer * buf)
{
  ...
  if (gst_segment_clip (segment, GST_FORMAT_TIME, start, stop, &cstart, &cstop)) {
    ...
    GST_LOG_OBJECT (decoder, "accepting buffer inside segment: %" GST_TIME_FORMAT " %"
        GST_TIME_FORMAT " seg %" GST_TIME_FORMAT " to %" GST_TIME_FORMAT " time %" GST_TIME_FORMAT,
        GST_TIME_ARGS (GST_BUFFER_PTS (buf)),
        GST_TIME_ARGS (GST_BUFFER_PTS (buf) + GST_BUFFER_DURATION (buf)),
        GST_TIME_ARGS (segment->start), GST_TIME_ARGS (segment->stop),
        GST_TIME_ARGS (segment->time));
  } else {
    GST_LOG_OBJECT (decoder, "dropping buffer outside segment: %" GST_TIME_FORMAT " %" GST_TIME_FORMAT
        " seg %" GST_TIME_FORMAT " to %" GST_TIME_FORMAT " time %" GST_TIME_FORMAT,
    ...
    GST_DEBUG_OBJECT (decoder, "decoder->num_live_buffer %d",decoder->num_live_buffer);
...
#ifdef GST_EXT_VIDEODECODER
  if ((decoder->priv->outbuf_log_cnt < MAX_BUFFER_LOG_CNT)||(decoder->priv->outbuf_log_cnt % BUFFER_LOG_INTERVAL == 0)) {
    GST_WARNING_OBJECT (decoder, "OUT_BUF [%d]: gst_buf= %p PTS= %" GST_TIME_FORMAT " DTS %" GST_TIME_FORMAT " size= %u",
       decoder->priv->outbuf_log_cnt, buf, GST_TIME_ARGS(GST_BUFFER_PTS (buf)), GST_TIME_ARGS (GST_BUFFER_DTS (buf)), gst_buffer_get_size (buf));
  } else {
    GST_LOG_OBJECT (decoder, "OUT_BUF : gst_buf= %p PTS= %" GST_TIME_FORMAT " DTS %" GST_TIME_FORMAT " size= %u",
      buf, GST_TIME_ARGS(GST_BUFFER_PTS (buf)), GST_TIME_ARGS (GST_BUFFER_DTS (buf)), gst_buffer_get_size (buf));
  }
  decoder->priv->outbuf_log_cnt++;
#else
...
#ifdef GST_EXT_VIDEODECODER
  if ((decoder->priv->outbuf_log_cnt < MAX_BUFFER_LOG_CNT)||(decoder->priv->outbuf_log_cnt % BUFFER_LOG_INTERVAL == 0)) {
    GST_WARNING_OBJECT (decoder, "gst_pad_push end[%d]: return = %d", decoder->priv->outbuf_log_cnt, ret);
  } else {
    GST_LOG_OBJECT (decoder, "gst_pad_push end: return = %d", ret);
  }
#endif
...
gst_video_decoder_add_to_frame (GstVideoDecoder * decoder, int n_bytes)
{
  ...
  if (gst_adapter_available (priv->output_adapter) == 0) {
    priv->frame_offset = priv->input_offset - gst_adapter_available (priv->input_adapter);
  }
...
gst_video_decoder_get_pending_frame_size (GstVideoDecoder * decoder)
{
  ...
  GST_LOG_OBJECT (decoder, "Current pending frame has %" G_GSIZE_FORMAT "bytes", ret);

  return ret;
}

static guint64
gst_video_decoder_get_frame_duration (GstVideoDecoder * decoder, GstVideoCodecFrame * frame)
{
  ...
  return gst_util_uint64_scale (GST_SECOND, state->info.fps_d, state->info.fps_n);
}
...
static GstFlowReturn
gst_video_decoder_decode_frame (GstVideoDecoder * decoder, GstVideoCodecFrame * frame)
{
  GstVideoDecoderPrivate *priv = decoder->priv;
  GstVideoDecoderClass *decoder_class;
  GstFlowReturn ret = GST_FLOW_OK;
  ...
  if (GST_VIDEO_CODEC_FRAME_IS_SYNC_POINT (frame) && GST_CLOCK_TIME_IS_VALID (frame->pts) && GST_CLOCK_TIME_IS_VALID (frame->dts)) {
    ...
    GST_DEBUG_OBJECT (decoder, "PTS delta %d ms", (gint) (priv->pts_delta / GST_MSECOND));
  }
  ...
  if (g_list_length (priv->frames) > 10) {
    GST_DEBUG_OBJECT (decoder, "decoder frame list getting long: %d frames, possible internal leaking?", g_list_length (priv->frames));
  }

  frame->deadline = gst_segment_to_running_time (&decoder->input_segment, GST_FORMAT_TIME, frame->pts);
 ...
 */
GstVideoCodecState *
gst_video_decoder_set_output_state (GstVideoDecoder * decoder, GstVideoFormat fmt, guint width, guint height, GstVideoCodecState * reference)
{
  GstVideoDecoderPrivate *priv = decoder->priv;
  GstVideoCodecState *state;
  ...
  if (priv->output_state != NULL && priv->output_state->info.fps_n > 0) {
    priv->qos_frame_duration = gst_util_uint64_scale (GST_SECOND, priv->output_state->info.fps_d, priv->output_state->info.fps_n);
  } else {
    priv->qos_frame_duration = 0;
  }
...
static gboolean
gst_video_decoder_decide_allocation_default (GstVideoDecoder * decoder, GstQuery * query)
{
...
static gboolean
gst_video_decoder_propose_allocation_default (GstVideoDecoder * decoder, GstQuery * query)
{
  return TRUE;
}
...
gst_video_decoder_negotiate_pool (GstVideoDecoder * decoder, GstCaps * caps)
{
  ...
  GST_DEBUG_OBJECT (decoder, "ALLOCATION (%d) params: %" GST_PTR_FORMAT, ret, query);
...
gst_video_decoder_negotiate_default (GstVideoDecoder * decoder)
{
  ...
  if (!state) {
    GST_DEBUG_OBJECT (decoder, "Trying to negotiate the pool with out setting the o/p format");
...
gst_video_decoder_allocate_output_buffer (GstVideoDecoder * decoder)
{
  ...
  GST_VIDEO_DECODER_STREAM_LOCK (decoder);
  needs_reconfigure = gst_pad_check_reconfigure (decoder->srcpad);
  if (G_UNLIKELY (!decoder->priv->output_state || decoder->priv->output_state_changed || needs_reconfigure)) {
    if (!gst_video_decoder_negotiate_unlocked (decoder)) {
      if (decoder->priv->output_state) {
  ...
  if (flow != GST_FLOW_OK) {
    GST_INFO_OBJECT (decoder, "couldn't allocate output buffer, flow %s", gst_flow_get_name (flow));
 ...
 */
GstFlowReturn
gst_video_decoder_allocate_output_frame (GstVideoDecoder * decoder, GstVideoCodecFrame * frame)
{
  ...
  flow_ret = gst_buffer_pool_acquire_buffer (decoder->priv->pool, &frame->output_buffer, NULL);
 ...
 */
GstClockTimeDiff
gst_video_decoder_get_max_decode_time (GstVideoDecoder * decoder, GstVideoCodecFrame * frame)
{
  ...
  if (GST_CLOCK_TIME_IS_VALID (earliest_time) && GST_CLOCK_TIME_IS_VALID (frame->deadline))
...
GstFlowReturn
_gst_video_decoder_error (GstVideoDecoder * dec, gint weight, GQuark domain, gint code, gchar * txt, gchar * dbg, const gchar * file,
    const gchar * function, gint line)
{
  ...
  if (dec->priv->max_errors >= 0 && dec->priv->error_count > dec->priv->max_errors) {
    gst_element_message_full (GST_ELEMENT (dec), GST_MESSAGE_ERROR, domain, code, txt, dbg, file, function, line);
    return GST_FLOW_ERROR;
  } else {
    g_free (txt);
    g_free (dbg);
...
gst_video_decoder_set_latency (GstVideoDecoder * decoder, GstClockTime min_latency, GstClockTime max_latency)
{
  ...
  gst_element_post_message (GST_ELEMENT_CAST (decoder), gst_message_new_latency (GST_OBJECT_CAST (decoder)));
}
