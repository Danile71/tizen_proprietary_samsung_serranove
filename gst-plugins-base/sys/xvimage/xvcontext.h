...
{
  return
      GST_XVCONTEXT_CAST (gst_mini_object_ref (GST_MINI_OBJECT_CAST
          (xvcontext)));
}
...
GstXWindow *gst_xvcontext_create_xwindow_from_xid (GstXvContext * context,
    XID xid);

void gst_xwindow_destroy (GstXWindow * window);

void gst_xwindow_set_event_handling (GstXWindow * window,
...
void gst_xvcontext_add_displaying_buffer (GstXvContext * context,
    XV_DATA_PTR img_data, GstBuffer * buffer);
void gst_xvcontext_remove_displaying_buffer (GstXvContext * context,
    unsigned int *gem_name);

#ifdef USE_FLUSH_BUFFER
gboolean gst_xvimagesink_make_flush_buffer (GstXvContext * context);
#endif
