...
gst_soup_http_src_query (GstBaseSrc * bsrc, GstQuery * query)
{
  ...
  switch (GST_QUERY_TYPE (query)) {
    ...
    case GST_QUERY_CUSTOM:
    {
        ...
        if (gst_structure_has_name (s, "HTTPCookies")) {  /* HTTP Cookies */

          GSList *cookie_list, *c;
          gchar **cookies, **array;
          GValue value = { 0, { { 0 } } };

          GST_DEBUG_OBJECT (src, "custom query for HTTPCookies");
	 ...
	 {  /* HTTP UserAgent */
          ...
          g_value_set_string (&value, src->user_agent);
          GST_DEBUG_OBJECT (src, "custom query for HTTPUserAgent");
	 ...
	 {
          GST_DEBUG_OBJECT (src, "Unsupported query");
          ret = FALSE;
        }
      }
      break;
