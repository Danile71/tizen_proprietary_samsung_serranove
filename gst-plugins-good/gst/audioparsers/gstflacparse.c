...
static GstCaps *gst_flac_parse_get_sink_caps (GstBaseParse * parse,
    GstCaps * filter);
#ifdef GST_EXT_FLACPARSER_MODIFICATION
static gboolean gst_flac_parse_do_seek (GstBaseParse * parse, GstPad * pad,
    GstEvent * event);
#define FLAC_LARGE_FILE_SIZE ( 50 * 1024 * 1024)    /* 50 mbyte */
#endif
...
#ifdef GST_EXT_FLACPARSER_MODIFICATION
  if (flacparse->first_frame == TRUE) {
    gboolean ret = FALSE;
    gint64 upstream_total_bytes = 0;
    GstFormat fmt = GST_FORMAT_BYTES;
    if (!gst_pad_peer_query_duration (parse->sinkpad, fmt, &upstream_total_bytes))
      upstream_total_bytes = 0;
    GST_INFO_OBJECT (flacparse, "gst_pad_query_peer_duration -upstream_total_bytes (%"G_GUINT64_FORMAT")", upstream_total_bytes);
    flacparse->file_size = upstream_total_bytes;
    flacparse->first_frame = FALSE;
  }
#endif
...
gst_flac_parse_src_event (GstBaseParse * parse, GstEvent * event)
{
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    case GST_EVENT_SEEK:
    {
      ...
      res = gst_flac_parse_do_seek(parse, srcpad, event);
...
gst_flac_parse_do_seek (GstBaseParse * parse, GstPad * pad, GstEvent * event)
{
  ...
  if (pad_mode != GST_PAD_MODE_PULL && flacparse->file_size) {
    GST_INFO_OBJECT (flacparse, "amr parser is not pull mode. Seek will be handled by demuxer");
    return FALSE;
  } else if (!flacparse->file_size) {
    GST_INFO_OBJECT (flacparse, "Seek will be handled by demuxer");
    return FALSE;
  }
  gst_base_parse_get_upstream_size (parse, &total_file_size);

  if (total_file_size > FLAC_LARGE_FILE_SIZE ) {
    gst_base_parse_set_seek_mode(parse, 0);
    GST_INFO_OBJECT (flacparse, "larger than big size (50MB)");
    return FALSE;
  }
#else
  GST_WARNING_OBJECT (flacparse, "baseparser does not define get private param functions. Seek will be handled by demuxer");
  return FALSE;
#endif
