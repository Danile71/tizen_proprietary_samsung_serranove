...
gst_avi_demux_parse_stream (GstAviDemux * avi, GstBuffer * buf)
{
  ...
  if (codec_name && tag_name) {
    ...
    gst_tag_list_add (stream->taglist, GST_TAG_MERGE_APPEND, tag_name,
        codec_name, NULL);
#ifdef GST_EXT_AVIDEMUX_MODIFICATION
    /* post now, send event on pad later */
    GST_DEBUG_OBJECT (avi, "Posting tags %" GST_PTR_FORMAT, stream->taglist);
    gst_element_post_message (GST_ELEMENT_CAST (avi),
        gst_message_new_tag (GST_OBJECT_CAST (avi),
            gst_tag_list_copy (stream->taglist)));
#endif
    g_free (codec_name);
  }
...
gst_avi_demux_find_next (GstAviDemux * avi, gfloat rate)
{
  ...
  for (i = 0; i < avi->num_streams; i++) {
    ...
    if (stream->pad && (GST_PAD_LAST_FLOW_RETURN (stream->pad) == GST_FLOW_EOS
                                ||GST_PAD_LAST_FLOW_RETURN (stream->pad) == GST_FLOW_NOT_LINKED))
#else
    if (stream->pad && GST_PAD_LAST_FLOW_RETURN (stream->pad) == GST_FLOW_EOS)
#endif
      continue;
