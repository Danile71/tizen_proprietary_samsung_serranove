    ...
    const GstTagList * end_tags, GstTagList * merged_tags)
{
  ...
  if (merged_tags) {
    for(i = 0; i < num; i++) {
      ...
      if (gst_tag_list_get_string(merged_tags, tag_check_list[i], &string)) {
        if (!gst_id3demux_tag_is_valid(merged_tags, tag_check_list[i])) {
          ...
          if (gst_tag_list_get_string(start_tags, tag_check_list[i], &string)) {
            gst_tag_list_add(merged_tags, GST_TAG_MERGE_REPLACE, tag_check_list[i], string, NULL);
            GST_DEBUG_OBJECT (tagdemux, "modified %s tag: %s", tag_check_list[i], string);
          }
