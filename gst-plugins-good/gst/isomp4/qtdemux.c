...
struct _QtDemuxStream
{
  ...
  GstByteReader stts;
  GstByteReader stss;
#ifndef GST_EXT_QTDEMUX_MODIFICATION
  GstByteReader stps;
#endif
  GstByteReader ctts;
  ...
  switch (GST_QUERY_TYPE (query)) {
    ...
    case GST_QUERY_DURATION:{
      ...
      if (fmt == GST_FORMAT_TIME) {
         ...
         * Updated Code from ===> if (!res && !qtdemux->fragmented) {
         * Since we are not sure of the condition of qtdemux->fragmented case for DASH/SS ,
         * So using qtdemux->is_dash_stream variable for differentiating the same.*/
        if (!res && !qtdemux->is_dash_stream && !qtdemux->fragmented) {
#else
        if (!res) {
#endif
          guint64 duration;
...
#ifdef GST_EXT_QTDEMUX_MODIFICATION
  /*
  For DASH/SS Streaming cases , qtdemux was not passing on the sinkpad stream caps to subsequent elements.
  As a result , hls_streaming tag was not getting set to TRUE , resulting in HW Decoder not handling this as Adaptive Case.
  */
  if (caps)
  {
	gchar *caps_str;
	caps_str = gst_caps_to_string (caps);
	if (strstr(caps_str,"hls_streaming"))
	{
		GST_WARNING_OBJECT (demux, "DASH Streaming case : setting dash flag to TRUE !!");
		demux->is_dash_stream = TRUE;
	}
	g_free (caps_str);
  }
#endif
...
gst_qtdemux_reset (GstQTDemux * qtdemux, gboolean hard)
{
  ...
  if (qtdemux->Subtitle_language_list) {
    g_list_foreach (qtdemux->Subtitle_language_list, (GFunc)g_free, NULL);
    ...
    guint64 moof_offset, QtDemuxStream * stream)
{
  ...
  /* NOTE @stream ignored */
#ifdef GST_EXT_QTDEMUX_MODIFICATION
  if ((qtdemux->is_dash_stream)&&(!buffer))
  {
      GST_ELEMENT_ERROR (qtdemux, STREAM, DEMUX,
        (_("[DASH-MOOF Corrupted]This file is corrupt and cannot be played.")), (NULL));
      return FALSE;
  }
#endif
...
#ifdef GST_EXT_QTDEMUX_MODIFICATION
       if ((qtdemux->is_dash_stream)&&(!stream))
       {
        GST_ERROR_OBJECT (qtdemux, "NULL Stream pointer !!!");
       }
       else
       {
#endif
...
gst_qtdemux_loop_state_movie (GstQTDemux * qtdemux)
{
  ...
  for (i = 0; i < qtdemux->n_streams; i++) {
    ...
    stream = qtdemux->streams[i];
#ifdef GST_EXT_QTDEMUX_MODIFICATION
    if (stream->pad && GST_PAD_LAST_FLOW_RETURN (stream->pad) != GST_FLOW_NOT_LINKED)
    {
...
gst_qtdemux_configure_stream (GstQTDemux * qtdemux, QtDemuxStream * stream)
{
  ...
  For DASH/SS Streaming cases , qtdemux was not passing on the sinkpad stream caps to subsequent elements.
  As a result , hls_streaming tag was not getting set to TRUE , resulting in HW Decoder not handling this as Adaptive Case.
  [TEMP] As per QTDemux design , gst_qtdemux_setcaps is not used for passing the caps to src pad.
  Instead separately qtdemux_parse_trak will create audio/video caps separately. So the hls_streaming flag is added as below.
  Incase of better way , we will revert the current changes.
  */
  if (stream->caps)
  {
	if (qtdemux->is_dash_stream)
	{
		stream->caps = gst_caps_make_writable (stream->caps);
		gst_caps_set_simple (stream->caps,"hls_streaming", G_TYPE_BOOLEAN, TRUE, NULL);
		GST_DEBUG_OBJECT (qtdemux,"DASH/SS Demuxer case !! appending hls_streaming flag in caps = %"GST_PTR_FORMAT,stream->caps);
	}
  }
#endif
  if (stream->subtype == FOURCC_vide) {
    ...
    GNode * trak)
{
  ...
  if ((edts = qtdemux_tree_get_child_by_type (trak, FOURCC_edts))) {
    ...
    if (stream->n_segments) {
      GstClockTime movie_duration = QTSTREAMTIME_TO_GSTTIME (stream, stream->duration);
...
{
#ifndef GST_EXT_AVIDEMUX_MODIFICATION
