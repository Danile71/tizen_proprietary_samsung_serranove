...
gst_matroska_demux_reset (GstElement * element)
{
  ...
  if (demux->Subtitle_language_list) {
    g_list_foreach (demux->Subtitle_language_list, (GFunc)g_free, NULL);
...
gst_matroska_demux_add_stream (GstMatroskaDemux * demux, GstEbmlRead * ebml)
{
  ...
  if (context->pending_tags)
    gst_tag_list_unref (context->pending_tags);
#endif
  ...
  if (list) {
    ...
    GST_DEBUG_OBJECT (demux, "Posting tags %" GST_PTR_FORMAT, list);
    gst_element_post_message (GST_ELEMENT_CAST (demux), gst_message_new_tag (GST_OBJECT_CAST (demux), gst_tag_list_copy (list)));
  }
#endif
    ...
    gboolean is_simpleblock)
{
  ...
  if (ret == GST_FLOW_OK && readblock) {
    ...
    if (delta_unit && stream->set_discont) {
      ...
      GST_LOG_OBJECT (demux, "skipping delta unit");
      goto done;
    }
#endif
    ...
    {
      GST_LOG_OBJECT (demux, "skipping delta unit when create index table.");
      goto done;
    }
#endif
    for (n = 0; n < laces; n++) {
      ...
      if(demux->index_table_created == FALSE) {
        ...
        if(flags == 128 || (!delta_unit && stream->type == GST_MATROSKA_TRACK_TYPE_VIDEO)) {
          ...
          if(!demux->index_table_array_creation) {
            GST_INFO_OBJECT (demux, "Keyframe time stamp is %"GST_TIME_FORMAT"pos %"G_GUINT64_FORMAT, GST_TIME_ARGS(idx->time), idx->pos);
            GST_INFO_OBJECT (demux, "Index table creation first time");
            ...
            GST_INFO_OBJECT (demux, "size of the index is %d",demux->common.index->len);
            demux->index_table_array_creation = TRUE;
            demux->duration = GST_BUFFER_TIMESTAMP(sub) + GST_BUFFER_DURATION (sub);
          } else {
            GST_INFO_OBJECT (demux, "Adding Keyframe info to index table");
            GST_INFO_OBJECT (demux, "Keyframe time stamp is %"GST_TIME_FORMAT"pos %"G_GUINT64_FORMAT, GST_TIME_ARGS(idx->time), idx->pos);
            g_array_append_val(demux->common.index, *idx);
            GST_INFO_OBJECT (demux, "size of the index is %d",demux->common.index->len);
            demux->duration = GST_BUFFER_TIMESTAMP(sub) + GST_BUFFER_DURATION (sub);
            GST_INFO_OBJECT (demux, "The duration of the video is %"GST_TIME_FORMAT, GST_TIME_ARGS(demux->duration));
          }
          free(idx);
          idx = NULL;
        } else {
          ...
          demux->duration = GST_BUFFER_TIMESTAMP(sub) + GST_BUFFER_DURATION (sub);
          GST_INFO_OBJECT (demux, "duration is in capturing not setting%"GST_TIME_FORMAT, GST_TIME_ARGS(demux->duration));
          GST_INFO_OBJECT (demux, "duration in segment duration still not set%"GST_TIME_FORMAT, GST_TIME_ARGS(demux->common.segment.duration));
        }
      } else {
...
//Index table Creation Logic
      if((demux->seek_head_cluster_info_absent || demux->seek_head_cue_info_absent) && !demux->index_table_created) {
        GST_INFO_OBJECT(demux,"Index table needs to be created");
        ...
        GST_INFO_OBJECT(demux,"the duration of the clip is %"GST_TIME_FORMAT, GST_TIME_ARGS(demux->duration));
        GST_INFO_OBJECT(demux,"Index table build time is time: %llu\n",(end-start));
        GST_INFO_OBJECT(demux,"size of the index is %d",demux->common.index->len);
        //gst_matroska_demux_sync_streams (demux);
        ...
        GST_INFO_OBJECT(demux,"index table creation completed");
      }
#endif
...
static gboolean gst_matroska_demux_create_index_table(GstMatroskaDemux* demux)
{
  ...
  while(TRUE) {
    ...
    if (ret == GST_FLOW_EOS || ret != GST_FLOW_OK) {
