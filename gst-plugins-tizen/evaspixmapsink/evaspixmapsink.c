...
static void *gst_evaspixmapsink_get_buffer (Display * dpy, Pixmap pixmap, int width,
    int height, int i);
    ...
    void *event_info)
{
  ...
  if (ret != GST_FLOW_OK) {
    GST_ERROR_OBJECT (evaspixmapsink, "evas epipe reset ret=%d, need to check", ret);
    return GST_STATE_CHANGE_FAILURE;
  }
...
#ifndef _TIZEN_PUBLIC_
        if (gst_video_format_from_fourcc (formats[i].id) == GST_VIDEO_FORMAT_SR32) {
          vformat = GST_VIDEO_FORMAT_SR32;
          ...
          if (xpixmap && damage_ev->drawable == xpixmap->pixmap) {
            ...
            if (g_cnt < 20) {
              evaspixmapsink->pixmap_addr[i] =
                  ...
                  i);
              GST_WARNING ("OUT_DUMP_%2.2d : pixmap[%d] addr %p (w : %d, h : %d)",
