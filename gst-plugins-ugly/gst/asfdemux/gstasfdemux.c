...
#include "asfpacket.h"
#if defined(ASFDEMUX_MODIFICATION) && defined(DRM_ENABLE)
...
static void gst_asf_demux_get_property (GObject * object, guint prop_id, GValue * value,
    GParamSpec * pspec);
...
#if defined(ASFDEMUX_MODIFICATION) && defined(DRM_ENABLE)
static void asfdemux_post_drm_license_error (GstASFDemux * demux, int drm_error);
#endif
...
#ifdef ASFDEMUX_MODIFICATION
#ifdef FEATURE_SECURE_PLAYBACK_TZ
    g_object_class_install_property (gobject_class, ARG_SECURE_PLAY,
      g_param_spec_boolean ("secure-play", "whether secure play mode or not",
        "TRUE->secure mode, FALSE-> non-secure-mode", FALSE,
        G_PARAM_READWRITE));
#endif
    ...
    const GValue * value, GParamSpec * pspec)
{
  ...
  switch (prop_id) {
#ifdef ASFDEMUX_MODIFICATION
#if defined(DRM_ENABLE) && defined(FEATURE_SECURE_PLAYBACK_TZ)
    case ARG_SECURE_PLAY:
      demux->secure_mode = g_value_get_boolean (value);
      break;
#endif
...
#if defined(DRM_ENABLE) && defined(FEATURE_SECURE_PLAYBACK_TZ)
    case ARG_SECURE_PLAY:
      g_value_set_boolean (value, demux->secure_mode);
      break;
#endif
    case ARG_FILE_LOCATION:
...
gst_asf_demux_reset (GstASFDemux * demux, gboolean chain_reset)
{
  ...
  if (demux->hFileHandle) {
    drm_trusted_result_e trusted_result = drm_util_pr_finalize(&demux->hFileHandle);
    if (trusted_result != DRM_TRUSTED_RETURN_SUCCESS) {
...
#if defined(DRM_ENABLE)
  demux->drm_success = TRUE;
  demux->license_status = DRM_LICENSE_STATUS_VALID;
  demux->error_posted = FALSE;
  demux->set_decrypt_state = FALSE;
#endif
...
#if defined(DRM_ENABLE) && defined(FEATURE_SECURE_PLAYBACK_TZ)
  demux->secure_mode = TRUE;
  GST_DEBUG("enabling the trustzone");
#endif
...
#ifdef ASFDEMUX_MODIFICATION
  if (flow == GST_FLOW_NOT_SUPPORTED) {
    GST_WARNING_OBJECT (demux, "un-supported media file: %s", gst_flow_get_name (flow));
...
#if defined(ASFDEMUX_MODIFICATION) && defined(DRM_ENABLE) && defined(FEATURE_SECURE_PLAYBACK_TZ)
    if (stream->payloads->len > 0 || demux->secure_mode) {
#else
...
#if defined(ASFDEMUX_MODIFICATION) && defined(DRM_ENABLE)
    if (GST_CLOCK_TIME_IS_VALID (payload->ts) &&
      payload->ts < demux->segment.start && !demux->secure_mode) {
#else
    if (GST_CLOCK_TIME_IS_VALID (payload->ts) &&
      payload->ts < demux->segment.start) {
#endif
...
gst_asf_demux_push_complete_payloads (GstASFDemux * demux, gboolean force)
{
  ...
  while ((stream = gst_asf_demux_find_stream_with_complete_payload (demux))) {
    ...
    demux->segment.position = GST_BUFFER_PTS (payload->buf);

#if defined(DRM_ENABLE) && defined(FEATURE_SECURE_PLAYBACK_TZ)
    if (demux->hFileHandle) {
      if(demux->secure_mode && demux->vidStrmId == (stream->id -1)) {
        GstBuffer *dbuf = NULL;
        drm_trusted_result_e bret = FALSE;
        GstFlowReturn ret = GST_FLOW_OK;
        GST_LOG_OBJECT (demux, ">>>>>>>> creating frame size  = %d\n", payload->mo_size);

        /* 1. Create decrypt payload buffer with frame size */
        GST_LOG_OBJECT (demux, "calling pad_alloc Buffer...");

        //ret = gst_pad_alloc_buffer(stream->pad, GST_BUFFER_OFFSET_NONE, payload->mo_size, stream->caps, &(dbuf));
        dbuf = gst_buffer_new_allocate (NULL, payload->mo_size, NULL);
        if (!dbuf) {
            GST_ERROR_OBJECT(demux, "fail to create dbuf.");
            return FALSE;
        }
        GST_DEBUG_OBJECT (demux, "buffer alloced size = %d", gst_buffer_get_size(dbuf));
        GST_DEBUG_OBJECT (demux, "buffer address = %p", dbuf);

        /* 2. call decrypt DRM apis */
        drm_trusted_result_e trusted_result = drm_util_pr_decrypt_payload (demux->hFileHandle, payload, dbuf);
        if (DRM_TRUSTED_RETURN_SUCCESS != trusted_result) {
          GST_ERROR_OBJECT (demux, "failed to perform decryption");
          asfdemux_post_trusted_drm_error(demux, trusted_result);
          gst_buffer_unref (dbuf);
          return FALSE;
        }

        GST_DEBUG_OBJECT (stream->pad, "decrypting video frame done TZ case...");

        gst_buffer_set_size(dbuf, gst_buffer_get_size(payload->buf));

        GST_DEBUG_OBJECT (demux, "current frame size = %d", payload->mo_size);

        //gst_buffer_copy_metadata (dbuf, payload->buf, GST_BUFFER_COPY_ALL);
        gst_buffer_copy_into (dbuf, payload->buf, GST_BUFFER_COPY_METADATA, 0, -1);

        /* 3. Unref payload.buf */
        gst_buffer_unref (payload->buf);

        /* 4. re-assign payload.buf with decrypted buffer */
        payload->buf = dbuf;
      } else {
        drm_trusted_result_e trusted_result = drm_util_pr_decrypt_payload (demux->hFileHandle, payload, NULL);
        if (trusted_result != DRM_TRUSTED_RETURN_SUCCESS) {
          GST_ERROR_OBJECT(demux, "failed to perform decryption");
          asfdemux_post_trusted_drm_error(demux, trusted_result);
          return FALSE;
...
#elif defined(DRM_ENABLE)
    GST_INFO("decryption doing before pushing non-tz case");
    if (demux->hFileHandle) {
      drm_trusted_result_e trusted_result = drm_util_pr_decrypt_payload (demux->hFileHandle, payload, NULL);
      if (trusted_result != DRM_TRUSTED_RETURN_SUCCESS) {
        GST_ERROR_OBJECT(demux, "failed to perform decryption");
...
#if defined(ASFDEMUX_MODIFICATION) && defined(DRM_ENABLE) && defined(FEATURE_SECURE_PLAYBACK_TZ)
  if (demux->secure_mode == TRUE) {
    if (G_UNLIKELY (!demux->activated_streams)) {
      if (!gst_asf_demux_check_activate_streams (demux, TRUE)) {// activate by force
        GST_INFO_OBJECT (demux, "failed to active streams by force");
        goto parse_error;
      }
      GST_INFO_OBJECT (demux, "forced stream activations is completed");
    }
  }
#endif
...
pause:
  {
    ...
    if (!sent_eos) {
      if (flow == GST_FLOW_EOS)
        gst_asf_demux_send_event_unlocked (demux, gst_event_new_eos ());
#if defined(ASFDEMUX_MODIFICATION) && defined(DRM_ENABLE)
      else if (demux->drm_success && (flow < GST_FLOW_EOS || flow == GST_FLOW_NOT_LINKED)) {
#else
    ...
    asf_stream_audio * audio, guint16 id, guint8 ** p_data, guint64 * p_size)
{
  ...
  if (tags) {
    ...
    GST_DEBUG_OBJECT (demux, "Posting tags %" GST_PTR_FORMAT, tags);
    gst_element_post_message (GST_ELEMENT_CAST (demux),
        gst_message_new_tag (GST_OBJECT_CAST (demux),
            gst_tag_list_copy (tags)));
  }
#endif
    ...
    guint8 ** p_data, guint64 * p_size)
{
  ...
  if (video->tag == GST_MAKE_FOURCC ('P', 'R', 'D', 'Y')) {
    ...
    if (!extradata && !gst_buffer_get_size(extradata)) {
      GstMapInfo map;
      gst_buffer_map(extradata, &map, GST_MAP_READ);
      extra_codec = *(guint32*)((map.data+(size_left-4)));
    ...
    if(demux->secure_mode) {
      ...
      caps_str_mime = gst_caps_to_string(caps);
      GST_INFO("cap are %s", caps_str);
      mime = strtok(caps_str_mime, ",");
      pch = strchr(caps_str,',');
      strcat(mime, "-secure");
      strcat(mime, pch);
      GST_INFO("changed caps are %s", mime);
      ...
      if(caps_str_mime)
        g_free(caps_str_mime);

      gst_caps_set_simple (caps, "secure", G_TYPE_INT, 1, NULL);
    }
#endif
...
static AsfStream *
gst_asf_demux_parse_stream_object (GstASFDemux * demux, guint8 * data, guint64 size)
{
...
#ifdef ASFDEMUX_MODIFICATION
      /*WMV file with any video codec is disbaled as per product spec*/
      GST_ERROR_OBJECT (demux, "WMV file format with video stream is not supported.");
...
static GstFlowReturn
gst_asf_demux_process_ext_stream_props (GstASFDemux * demux, guint8 * data, guint64 size)
{
    ...
    guint64 * p_size)
{
  ...
  else if (obj.id >= ASF_OBJ_PLAYREADY_ENCRYPTION_OBJECT)
  {
    GST_ERROR ("This file is encrypted. obj.id = %d", obj.id);
    return GST_FLOW_NOT_SUPPORTED;
  }
#endif
  ...
  switch (obj.id) {
    ...
    case ASF_OBJ_BITRATE_PROPS:
      ret =
          gst_asf_demux_process_bitrate_props_object (demux, *p_data, obj_data_size);
      break;
    case ASF_OBJ_EXT_CONTENT_DESC:
      ret =
          gst_asf_demux_process_ext_content_desc (demux, *p_data, obj_data_size);
      break;
    ...
    case ASF_OBJ_PLAYREADY_ENCRYPTION_OBJECT:{
      ...
      drm_result_e drm_status;
      //gboolean is_drm_file = FALSE;

      if (demux->playback_protected)  {
        GST_ERROR_OBJECT (demux, "File is playback protected, so exiting now");
        asfdemux_post_trusted_drm_error (demux, DRM_TRUSTED_RETURN_OPL_ERROR);
        demux->error_posted = TRUE;
        demux->drm_success = FALSE;
        return GST_FLOW_ERROR;
      }
      ...
      if (!gst_pad_peer_query (demux->sinkpad, query)) {
        ...
        GST_ELEMENT_ERROR (demux, CORE, FAILED, (NULL), ("failed to get uri from upstream"));
        gst_query_unref (query);
        return GST_FLOW_ERROR;
      }
      gst_query_parse_uri (query, &uri);
      GST_LOG_OBJECT(demux, "successful got uri and value is %s",uri);
      gst_query_unref (query);
      query = NULL;

      if (uri == NULL) {
        GST_ELEMENT_ERROR (demux, CORE, FAILED, (NULL), ("URI is NULL"));
        GST_ERROR_OBJECT (demux, "uri value is NULL...");
        return GST_FLOW_ERROR;
      }
      ...
      if (demux->hFileHandle == NULL) {
        ...
        gboolean bret = FALSE;
        drm_trusted_result_e trusted_result;

        bret = drm_get_license_status (uri, DRM_PERMISSION_TYPE_PLAY, &license_status);
        if (DRM_RETURN_SUCCESS != bret || license_status != DRM_LICENSE_STATUS_VALID) {
          GST_ERROR_OBJECT (demux, "failed to get license status : 0x%x", license_status);
          asfdemux_post_drm_license_error (demux, license_status);
          demux->drm_success = FALSE;
          g_free (uri);
          return GST_FLOW_ERROR;
        } else {
          GST_DEBUG_OBJECT(demux, "license is successful");
        }

      /* Get DRM Type */
      drm_status = drm_get_file_type (uri, &file_type);
      if (drm_status != DRM_RETURN_SUCCESS) {
        GST_ERROR ("Error in drm_get_file_type(), error=%d", drm_status);
            demux->drm_success = FALSE;
            g_free (uri);
            return GST_FLOW_ERROR;
      }
      ...
      GST_DEBUG ("filepath = [%s], file_type = [%d]", uri, file_type);
      ...
      trusted_result = drm_util_pr_init (&demux->hFileHandle, uri, &demux->is_drm_file, file_type);
        if (trusted_result == DRM_TRUSTED_RETURN_SUCCESS) {
          GST_DEBUG_OBJECT(demux, "Init playready DONE! [%s]", uri);
        } else {
          GST_ERROR_OBJECT(demux, "Init playready FAILED.... [%s]", uri);
          asfdemux_post_trusted_drm_error (demux, trusted_result);
          demux->drm_success = FALSE;
          g_free (uri);
          return GST_FLOW_ERROR;
        ...
        GST_DEBUG_OBJECT(demux, "Already Init playready DONE! [%s]", uri);
      }
      g_free (uri);
      ret = GST_FLOW_OK;
      break;
...
#ifndef FEATURE_SECURE_PLAYBACK_TZ
...
#ifdef ASFDEMUX_MODIFICATION
#if defined(DRM_ENABLE) && defined(FEATURE_SECURE_PLAYBACK_TZ)
gboolean gst_asf_demux_check_file_type (GstASFDemux *demux)
{
  GstQuery *query = NULL;
  ...
  if(demux == NULL) {
    GST_ERROR_OBJECT (demux, "demux object NULL");
    gst_query_unref (query);
    query = NULL;
    return FALSE;
  }
  ...
  if(demux->file_location) {
    if(strlen(demux->file_location) == 0) {
      GST_LOG_OBJECT(demux, "Assigning NULL to file_location since it is blank for appsrc");
      demux->file_location = NULL;
    }
  }

  if(!demux->secure_mode || demux->file_location == NULL) {
    if (!gst_pad_peer_query (demux->sinkpad, query)) {
      GST_ERROR_OBJECT (demux, "failed to query URI from upstream");
      if(demux->null_ready_state) {
        GST_LOG_OBJECT(demux, "Returning true even though query to upstream failed to avoid cases where appsrc is the source plugin");
        demux->null_ready_state = FALSE;
        gst_query_unref (query);
        query = NULL;
        return TRUE;
  ...
  if(demux->file_location != NULL && demux->secure_mode){
    file_path = demux->file_location;
  }
  ...
  GST_INFO_OBJECT (demux, "file path : %s", file_path);
  GST_INFO_OBJECT (demux, "modified path : %s", modified_path);
  drm_result = drm_get_file_type (modified_path, &file_type);
  if (drm_result != DRM_RETURN_SUCCESS) {
    ...
    if (file_type == DRM_TYPE_PLAYREADY || file_type == DRM_TYPE_PLAYREADY_ENVELOPE) {
      ...
      if (demux->license_status != DRM_LICENSE_STATUS_VALID ) {
        if (!demux->error_posted) {
          asfdemux_post_drm_license_error (demux, demux->license_status);
          GST_WARNING_OBJECT(demux, "license error is posted in PAUSED state");
          demux->error_posted = TRUE;
        }
        if(file_path){
           g_free(file_path);
    ...
    if (demux->hFileHandle == NULL && (demux->license_status == DRM_LICENSE_STATUS_VALID) && !demux->error_posted )  {
      demux->license_status = DRM_LICENSE_STATUS_UNDEFINED;
      gboolean bret = FALSE;
      bret = drm_get_license_status (modified_path, DRM_PERMISSION_TYPE_PLAY, &demux->license_status);
      if (DRM_RETURN_SUCCESS != bret || demux->license_status != DRM_LICENSE_STATUS_VALID) {
        GST_ERROR_OBJECT (demux, "failed to get license status : 0x%x", demux->license_status);
  #ifdef FEATURE_SECURE_PLAYBACK_TZ
        if (demux->license_status != DRM_LICENSE_STATUS_VALID) {
          GST_WARNING_OBJECT(demux, "license status kept to post in PAUSED state");
          demux->drm_success = FALSE;
          return TRUE;
        }
  #else
        asfdemux_post_drm_license_error (demux, demux->license_status);
        GST_WARNING_OBJECT(demux, "license error posted in PAUSED state");
        demux->error_posted = TRUE;
        demux->drm_success = FALSE;
        return FALSE;
  #endif
  ...
  if (DRM_TRUSTED_RETURN_SUCCESS != drm_util_pr_init (&demux->hFileHandle, modified_path, &demux->is_drm_file, file_type)) {
    GST_ERROR_OBJECT(demux, "drm_init failed [%s]", modified_path);
    if(file_path){
      g_free(file_path);
      file_path=NULL;
      modified_path=NULL;
  ...
  if (demux->is_drm_file) {
    GST_INFO_OBJECT (demux, "DRM file");
  } else {
    GST_INFO_OBJECT (demux, "non-DRM file");
    demux->secure_mode = FALSE;
  }
  if(file_path){
     g_free(file_path);
     file_path=NULL;
...
gst_asf_demux_change_state (GstElement * element, GstStateChange transition)
{
  ...
  switch (transition) {
    case GST_STATE_CHANGE_NULL_TO_READY:{
      ...
      if(demux->secure_mode) {
      if (!gst_asf_demux_check_file_type (demux))
    ...
    case GST_STATE_CHANGE_READY_TO_PAUSED:{
      ...
      if(!demux->secure_mode || (demux->secure_mode && demux->license_status != DRM_LICENSE_STATUS_VALID) ) {
        if (!gst_asf_demux_check_file_type (demux))
          return GST_STATE_CHANGE_FAILURE;
      }
      break;
...
gst_asf_demux_create_simple_index_table (GstASFDemux *demux)
{
  ...
  if (!gst_asf_demux_pull_data (demux, demux->data_offset, demux->data_size, &buf, NULL))
    goto error;
  GST_LOG_OBJECT (demux, "Received buffer of size = %d", gst_buffer_get_size (buf));
  GST_DEBUG ("Received buffer of size = %d\n", gst_buffer_get_size (buf));

  sidx_queue = g_queue_new ();
  if (NULL == sidx_queue) {
    GST_ERROR_OBJECT (demux, "Failed to create GQueue");
    GST_DEBUG ("Failed to create GQueue\n");
    goto error;
  }

  /* Parsing of all packets to know the key frames available */
  while (offset < gst_buffer_get_size (buf)) {
    /* Create sub buffer of size packet_size */
  //  packet_buf = gst_buffer_create_sub (buf, offset, demux->packet_size);
    packet_buf = gst_buffer_copy_region (buf, GST_BUFFER_COPY_MEMORY, offset, demux->packet_size);
    if (NULL == packet_buf) {
      GST_ERROR_OBJECT (demux, "Failed to create sub buffer");
      GST_DEBUG ("Failed to create sub buffer\n");
      goto error;
    }
    ...
    if (FALSE == bret) {
      ...
      GST_DEBUG ( "Failed to parse packet\n");
      goto error;
    }
  ...
  GST_DEBUG("Total keyframe entries = %d\n", g_queue_get_length (sidx_queue));
  GST_LOG_OBJECT (demux, "Parsing of File is finished...Going to build sidx_entries");
  GST_DEBUG ("Parsing of File is finished...Going to build sidx_entries\n");
  ...
  GST_DEBUG ("Number of simple index entries = %d\n", demux->sidx_num_entries);

  if(demux->sidx_entries) {
    GST_DEBUG_OBJECT(demux, "freeing the index table memory");
    GST_DEBUG("freeing the index table memory\n");
    g_free(demux->sidx_entries);
  }
  ...
  for (i = 0; i < demux->sidx_num_entries; i++) {
	...
	GST_DEBUG (" = packet %4u  count : %2d\n",demux->sidx_entries[i].packet, demux->sidx_entries[i].count);
...
#ifdef DRM_ENABLE
void
asfdemux_post_trusted_drm_error(GstASFDemux * demux, int drm_error)
{
  switch (drm_error) {
    case DRM_TRUSTED_RETURN_OPL_ERROR:
      GST_ERROR_OBJECT (demux, "Trusted OPL violation error");
      GST_ELEMENT_ERROR (demux, STREAM, DECRYPT_NOKEY, ("Trusted opl violation error"), (NULL));
      break;
    case DRM_TRUSTED_RETURN_FILE_ERROR:
      GST_ERROR_OBJECT (demux, "Trusted File Type Error");
      GST_ELEMENT_ERROR (demux, STREAM, DECRYPT_NOKEY, ("Trusted File Type Error"), (NULL));
      break;
    case DRM_TRUSTED_RETURN_INTERNAL_ERROR:
      default:
      GST_ERROR_OBJECT (demux, "posting error as undefined");
      GST_ELEMENT_ERROR (demux, STREAM, DECRYPT_NOKEY, ("Trusted undefind error"), (NULL));
      break;
  }
}

static void
asfdemux_post_drm_license_error(GstASFDemux * demux, int drm_error)
{
  switch (drm_error) {
    case DRM_LICENSE_STATUS_EXPIRED:
      GST_ERROR_OBJECT (demux, "posting error as rights expired");
      GST_ELEMENT_ERROR (demux, STREAM, DECRYPT_NOKEY, ("rights expired"), (NULL));
      break;
    case DRM_LICENSE_STATUS_NO_LICENSE:
      GST_ERROR_OBJECT (demux, "posting error as no rights");
      GST_ELEMENT_ERROR (demux, STREAM, DECRYPT_NOKEY, ("no rights"), (NULL));
      break;
    case DRM_LICENSE_STATUS_FUTURE_USE:
      GST_ERROR_OBJECT (demux, "posting error as rights for future");
      GST_ELEMENT_ERROR (demux, STREAM, DECRYPT_NOKEY, ("has future rights"), (NULL));
      break;
    case DRM_LICENSE_STATUS_UNDEFINED:
      default:
      GST_ERROR_OBJECT (demux, "posting error as undefined");
      GST_ELEMENT_ERROR (demux, STREAM, DECRYPT_NOKEY, ("undefind error"), (NULL));
      break;
  }
}
#endif
