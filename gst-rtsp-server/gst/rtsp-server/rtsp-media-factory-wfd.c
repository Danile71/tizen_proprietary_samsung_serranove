...
void  gst_rtsp_media_factory_wfd_set (GstRTSPMediaFactoryWFD * factory,
    ...
    guint64 audio_buffer_time, gboolean audio_do_timestamp, guint mtu_size, gboolean audio_only, gchar *file_path)
#else
void  gst_rtsp_media_factory_wfd_set (GstRTSPMediaFactoryWFD * factory,
    guint8 videosrc_type, gchar *audio_device, guint64 audio_latency_time,
...
#ifdef COMMERCIAL_FEATURE
static void appsrc_push_default_image (GstElement *appsrc, guint unused_size, gpointer user_data)
{
  GstRTSPMediaFactoryWFD * factory = (GstRTSPMediaFactoryWFD*)user_data;
  ...
  read_fd = fopen(factory->priv->file_path, "r");
  if (read_fd) {
    fseek(read_fd, 0, SEEK_END);
    file_size = ftell(read_fd);
    fseek(read_fd, 0, SEEK_SET);

    img_buff = malloc(file_size);
    read_bytes = fread(img_buff, file_size, 1, read_fd);
    fclose(read_fd);
  }

  if (factory->priv->audio_src)
    clock = gst_element_get_clock (factory->priv->audio_src);

  if (clock) {
    timestamp = gst_clock_get_time (clock);
    GST_INFO("CLOCK_BUFFER ts = %" GST_TIME_FORMAT , GST_TIME_ARGS (timestamp));
    if (!gst_element_get_base_time (factory->priv->audio_src)) {
      timestamp = GST_CLOCK_TIME_NONE;
    } else {
      timestamp -= gst_element_get_base_time (factory->priv->audio_src);
    }
   ...
   memcpy(map.data, img_buff,  gst_buffer_get_size(buffer));
   gst_buffer_unmap (buffer, &map);
   GST_BUFFER_TIMESTAMP (buffer) = timestamp;
   GST_BUFFER_DURATION (buffer) = gst_util_uint64_scale_int (GST_SECOND, 1, 10);
   gst_app_src_push_buffer ((GstAppSrc *)appsrc, buffer);
   free(img_buff);
}

static gboolean
_rtsp_media_factory_wfd_create_audio_default_image (GstRTSPMediaFactoryWFD * factory,
    GstBin * srcbin)
{
  ...
  if (NULL == appsrc) {
    GST_ERROR_OBJECT (factory, "failed to create appsrc element");
    goto create_error;
  }
  jpegparser = gst_element_factory_make ("jpegparse", "jpegparser");
  if (NULL == jpegparser) {
    GST_ERROR_OBJECT (factory, "failed to create jpegparser element");
    goto create_error;
  }
  ...
  g_object_set (venc, "byte-stream", TRUE, NULL);
  ...
  g_signal_connect (appsrc, "need-data", G_CALLBACK (appsrc_push_default_image), factory);

  gst_bin_add_many (srcbin, appsrc, jpegparser, jpegDec, videoscale, vcaps, venc, vparse, vqueue, NULL);

  if (!gst_element_link_many (appsrc, jpegparser, jpegDec, videoscale, vcaps,  venc, vparse, vqueue, NULL)) {
...
#ifdef COMMERCIAL_FEATURE
  if (priv->audio_only && priv->file_path != NULL && (strstr(priv->file_path, ".jpg") || strstr(priv->file_path, ".jpeg"))) {
      if (!_rtsp_media_factory_wfd_create_audio_default_image(factory, srcbin)) {
        GST_ERROR_OBJECT (factory, "failed to create audio default image bin...");
        goto create_error;
      }
  }
#endif
