...
#define CLEAR_PENDING_EOS(bsrc) \
  G_STMT_START { \
    GST_WARNING_OBJECT (bsrc, "CLEAR_PENDING_EOS");\
...
gst_base_src_send_event (GstElement * element, GstEvent * event)
{
  ...
  bclass = GST_BASE_SRC_GET_CLASS (src);
#ifdef GST_ADD_LOG
  GST_WARNING_OBJECT (src, "handling event %p %" GST_PTR_FORMAT, event, event);
#else
  ...
  switch (GST_EVENT_TYPE (event)) {
...
#ifdef GST_ADD_LOG
      GST_WARNING_OBJECT (src, "pushing flush-start event downstream");
#else /* GST_ADD_LOG */
    ...
    case GST_EVENT_EOS:
    {
...
#ifdef GST_ADD_LOG
      GST_WARNING_OBJECT (src, "EOS marked, calling unlock");
#else /* GST_ADD_LOG */
...
#ifdef GST_ADD_LOG
      GST_WARNING_OBJECT (src, "unlock called, waiting for LIVE_LOCK");
#else /* GST_ADD_LOG */
      ...
      GST_LIVE_LOCK (src);
#ifdef GST_ADD_LOG
      GST_WARNING_OBJECT (src, "LIVE_LOCK acquired, calling unlock_stop");
#else /* GST_ADD_LOG */
...
gst_base_src_default_event (GstBaseSrc * src, GstEvent * event)
{
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    case GST_EVENT_FLUSH_START:
       ...
       * when in pull mode. */
#ifdef GST_ADD_LOG
      GST_WARNING_OBJECT (src, "set flusing TRUE");
#endif /* GST_ADD_LOG */
...
gst_base_src_loop (GstPad * pad)
{
  ...
  if (G_UNLIKELY (ret != GST_FLOW_OK)) {
#ifdef GST_ADD_LOG
    GST_WARNING_OBJECT (src, "pausing after gst_base_src_get_range() = %s",
        gst_flow_get_name (ret));
#else /* GST_ADD_LOG */
...
#ifdef GST_ADD_LOG
    GST_WARNING_OBJECT (src, "pausing task, reason %s", reason);
#else /* GST_ADD_LOG */
...
#ifdef GST_ADD_LOG
  GST_WARNING_OBJECT (basesrc, "stopping source");
#else /* GST_ADD_LOG */
