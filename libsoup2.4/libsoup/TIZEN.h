...
#define ENABLE_TIZEN_IGNORE_SNIFFED_VIDEO_MP4_MIMETYPE 1 /* Raveendra Karu : Use Content-Type received from the server and ignore the sniffed mime type "video/mp4", if the Content-Type received from the server begins with "audio/" */
#define ENABLE_TIZEN_VALIDATE_CACHE_OBJECT_ON_SOUP_GET_FEATURE 1 /* Sowmya B : Adding Null check in soup-session.c for soup_session_get_feature() on SOUP_TYPE_CACHE */
