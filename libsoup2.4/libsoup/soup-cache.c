...
//START ENABLE_TIZEN_COMPRESSION_PROXY
	char *url = NULL;
//END ENABLE_TIZEN_COMPRESSION_PROXY
	...
	while (lru_entry &&
		 ...
		 */
//START ENABLE_TIZEN_COMPRESSION_PROXY
		url = g_strdup (old_entry->uri);
//END ENABLE_TIZEN_COMPRESSION_PROXY
		if (soup_cache_entry_remove (cache, old_entry, TRUE)) {
//START ENABLE_TIZEN_COMPRESSION_PROXY
			g_signal_emit (cache, signals[REMOVED_CACHE_RESOURCE], 0, url);
//END ENABLE_TIZEN_COMPRESSION_PROXY
...
//START ENABLE_TIZEN_COMPRESSION_PROXY
typedef struct _SoupCachePushHandle SoupCachePushHandle;
//END ENABLE_TIZEN_COMPRESSION_PROXY
...
typedef struct {
	...
	SoupCacheEntry *entry;
//START ENABLE_TIZEN_COMPRESSION_PROXY
	SoupCachePushHandle *push_handle;
//END ENABLE_TIZEN_COMPRESSION_PROXY
} StreamHelper;

//START ENABLE_TIZEN_COMPRESSION_PROXY
typedef struct _SoupCachePushHandle {
	GInputStream *stream;
	StreamHelper *helper;
	GCancellable *cancellable;
} SoupCachePushHandle;
...
static SoupCacheResponseUC has_response (SoupCache *cache, SoupMessage* msg);
static gpointer push_start (SoupCache   *cache, SoupMessage *msg);
static void push_ongoing (gpointer handle, const char* data, int length);
static void push_cancel (gpointer handle);
//END ENABLE_TIZEN_COMPRESSION_PROXY
...
//START ENABLE_TIZEN_COMPRESSION_PROXY
		...
		if (soup_cache_entry_remove (cache, entry, TRUE))
		    g_signal_emit (cache, signals[REMOVED_CACHE_RESOURCE], 0, url);
//END ENABLE_TIZEN_COMPRESSION_PROXY
...
//START ENABLE_TIZEN_COMPRESSION_PROXY
	cache_class->has_response = has_response;
	cache_class->push_start = push_start;
	cache_class->push_ongoing = push_ongoing;
	cache_class->push_cancel = push_cancel;

	signals[REMOVED_CACHE_RESOURCE] =
	g_signal_new ("removed_cache_resource",
		      G_OBJECT_CLASS_TYPE (gobject_class),
		      G_SIGNAL_RUN_FIRST,
		      G_STRUCT_OFFSET (SoupCacheClass, removed_cache_resource),
		      NULL, NULL,
		      // _soup_marshal_NONE__STRING,
		      NULL,
		      G_TYPE_NONE, 1,
		      G_TYPE_STRING);

	signals[CACHE_CLEARED] =
	g_signal_new ("cache_cleared",
		      G_OBJECT_CLASS_TYPE (gobject_class),
		      G_SIGNAL_RUN_FIRST,
		      G_STRUCT_OFFSET (SoupCacheClass, cache_cleared),
		      NULL, NULL,
...
//START ENABLE_TIZEN_COMPRESSION_PROXY
	g_signal_emit (cache, signals[CACHE_CLEARED], 0);
//END ENABLE_TIZEN_COMPRESSION_PROXY
}
...
//START ENABLE_TIZEN_COMPRESSION_PROXY
static SoupCacheResponseUC
has_response (SoupCache *cache, SoupMessage* msg)
{
	SoupCacheResponse response;
	SoupCacheEntry *entry;
	SoupCacheResponseUC ret = SOUP_CACHE_RESPONSE_UC_NONE;

	if(!msg) {
		return SOUP_CACHE_RESPONSE_UC_NONE;
	}
	...
	switch (response) {
	case SOUP_CACHE_RESPONSE_FRESH:
		ret = SOUP_CACHE_RESPONSE_UC_FRESH;
		break;
	case SOUP_CACHE_RESPONSE_NEEDS_VALIDATION:
		ret = SOUP_CACHE_RESPONSE_UC_NEEDS_VALIDATION;
		break;
	case SOUP_CACHE_RESPONSE_STALE:
		ret = SOUP_CACHE_RESPONSE_UC_STALE;
		break;
	default:
		ret = SOUP_CACHE_RESPONSE_UC_NONE;
		break;
	}
...
static void
clear_push_handle (SoupCachePushHandle *push_handle)
{
	if (push_handle) {
		if (push_handle->stream)
		    push_handle->stream = NULL;
		if (push_handle->helper)
		    push_handle->helper = NULL;
		if (push_handle->cancellable) {
			g_object_unref (push_handle->cancellable);
			push_handle->cancellable = NULL;
		}
	}
}

static void
destroy_push_handle (SoupCachePushHandle *push_handle)
{
	if (push_handle) {
		clear_push_handle (push_handle);
		g_slice_free (SoupCachePushHandle, push_handle);
	}
}

static gpointer
push_start (SoupCache *cache, SoupMessage *msg)
{
	SoupCacheEntry *entry;
	SoupCacheability cacheability;
	GInputStream *istream;
	...
	file = get_file_from_entry (cache, entry);
	istream = soup_cache_input_stream_new (NULL, file);
	g_object_unref (file);

	push_handle = g_slice_new0 (SoupCachePushHandle);
	push_handle->stream = istream;
	push_handle->helper = (StreamHelper *)helper;
	push_handle->cancellable = g_object_ref (entry->cancellable);

	helper->push_handle = push_handle;
	g_signal_connect (istream, "caching-finished", G_CALLBACK (istream_caching_finished), helper);
...
static void
push_ongoing (gpointer handle, const char* data, int length)
{
	SoupCachePushHandle *push_handle = (SoupCachePushHandle*)handle;
	GInputStream *stream = NULL;

	if (!push_handle || !push_handle->stream || g_cancellable_is_cancelled (push_handle->cancellable)) {
		if (length)
			clear_push_handle (push_handle);
		else
			destroy_push_handle (push_handle);
		return;
	}

	stream = push_handle->stream;

	if (length) {
		soup_cache_input_stream_push_ongoing (stream, (void *) data, length);
	} else {
		soup_cache_input_stream_push_ongoing (stream, NULL, 0);
		destroy_push_handle (push_handle);
	}
}

static void
push_cancel (gpointer handle)
{
	SoupCachePushHandle *push_handle = (SoupCachePushHandle*)handle;

	if (!push_handle || !push_handle->stream || !push_handle->helper || g_cancellable_is_cancelled (push_handle->cancellable)) {

	} else {
		if (!g_cancellable_is_cancelled (push_handle->helper->entry->cancellable))
		    g_cancellable_cancel (push_handle->helper->entry->cancellable);
		soup_cache_input_stream_push_cancel (push_handle->stream);
