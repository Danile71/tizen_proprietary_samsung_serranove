...
#define DEFAULT_TSCHED_WATERMARK_USEC (60*PA_USEC_PER_MSEC)        /* 60ms  -- Fill up when only this much is left in the buffer */
/* FIXME spreadtrum */
#if 0
...
/*             pa_log_debug("%lu frames to write", (unsigned long) frames); */
...
#elif TIZEN_DEBUG_ENABLE
            {
                ...
                if(u->log_count == 0 || u->log_count > interval) {
                    pa_log_info("ALSA-SINK : Wrote %lu bytes (of possible %lu bytes), total(%llu)", (unsigned long) written, (unsigned long) n_bytes, u->write_count);
...
#ifdef SUPPORT_UHQA
static int sink_set_uhqa_mixer(struct userdata *u, bool enable)
{
    pa_sample_spec ss;
    ...
    // get master sink name.
    master_name = pa_proplist_gets(u->sink->proplist, PA_PROP_DEVICE_MASTER_DEVICE);
    if (master_name)
        sink = pa_namereg_get(u->sink->core, master_name, PA_NAMEREG_SINK);
    else
        sink = u->sink;

    ss = sink->sample_spec;
    device_string = pa_proplist_gets(sink->proplist, PA_PROP_DEVICE_STRING);

    if(pa_streq(device_string, "hw:0,0")) {
        if (ss.rate >= 192000 && ss.format == PA_SAMPLE_S24_32LE) {
            if(enable) {
                ret = pa_alsa_set_mixer_control("SLIM_0_RX Format", 1);
                pa_log_warn("ret(%d), device(%s), SLIM_0_RX Format(%d:%s)", ret, u->device_name, 1, "S24_LE");
                ret = pa_alsa_set_mixer_control("SLIM_0_RX SampleRate", 2);
                pa_log_warn("ret(%d), device(%s), SLIM_0_RX SampleRate(%d:%s)", ret, u->device_name, 2, "KHZ_192");
            } else {
                ret = pa_alsa_set_mixer_control("SLIM_0_RX Format", 0);
                pa_log_warn("ret(%d), device(%s), SLIM_0_RX Format(%d:%s)", ret, u->device_name, 0, "S16_LE");
                ret = pa_alsa_set_mixer_control("SLIM_0_RX SampleRate", 0);
                pa_log_warn("ret(%d), device(%s), SLIM_0_RX SampleRate(%d:%s)", ret, u->device_name, 0, "KHZ_48");
            }
        }
    }
}
...
#ifdef SUPPORT_UHQA
    /* should be called before set_hw_params */
    sink_set_uhqa_mixer(u, 1);
#endif
#endif
