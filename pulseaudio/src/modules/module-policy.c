...
enum {
    ...
    // call settings
    SUBCOMMAND_SET_CALL_NETWORK_TYPE, // 21
...
{
    uint32_t ret = SOUND_BLUETOOTH_BANDWIDTH_NB;
...
static uint32_t __get_route_flag(struct userdata *u) {
    ...
    if (u->session == AUDIO_SESSION_VOICECALL || u->session == AUDIO_SESSION_VIDEOCALL || u->session == AUDIO_SESSION_VOIP) {
        ...
        if (__is_mute_policy()) {
            ...
            if(u->active_device_out != AUDIO_DEVICE_OUT_WIRED_ACCESSORY) {
...
static audio_return_t policy_set_active_device(struct userdata *u, uint32_t device_in, uint32_t device_out, uint32_t is_start, uint32_t* need_update) {
    ...
    *need_update = (u->active_device_out == device_out) ? FALSE : TRUE;
    ...
    pa_log_info("set_active_device is_start:%d, session:%s,%s in:%s->%s out:%s->%s flag:%x->%x muteall:%d call_muted:%d",
            ...
            u->active_route_flag, u->cache_route_flag, u->muteall, u->call_muted);
...
static audio_return_t __update_volume(struct userdata *u, uint32_t stream_idx, uint32_t volume_type, uint32_t volume_level, uint32_t fade)
{
    ...
    PA_IDXSET_FOREACH(si, u->core->sink_inputs, idx) {
        ...
        if ((stream_idx == idx) || ((stream_idx == PA_INVALID_INDEX) && (audio_info.stream.volume_type == volume_type))) {
            ...
            if (fade)
                pa_sink_input_set_smooth_volume(si, &cv, TRUE, TRUE);
            else
    ...
    if (direction == AUDIO_DIRECTION_OUT) {
        PA_IDXSET_FOREACH(si, u->core->sink_inputs, idx) {
            ...
            if ((stream_idx == idx) || ((stream_idx == PA_INVALID_INDEX) && (audio_info.stream.volume_type == volume_type))) {
                ...
                if(fade)
                    pa_sink_input_set_smooth_mute(si, (pa_bool_t)mute, TRUE);
                else
...
static void __set_volume_fade_timeout_cb(pa_mainloop_api *m, pa_time_event *e, const struct timeval *t, void *userdata)
{
    ...
    if((fade_userdata != NULL) && (fade_userdata->count > 0)) {
        ...
        if(fade_userdata != NULL) {
            if (fade_userdata->time_event != NULL) {
                pa_core_rttime_restart(fade_userdata->u->core, e, PA_USEC_INVALID);
                fade_userdata->u->core->mainloop->time_free(fade_userdata->time_event);
                fade_userdata->time_event = NULL;
            }
...
static audio_return_t policy_volume_fade(struct userdata* u, uint32_t stream_idx, uint32_t up_down, uint32_t duration)
{
    ...
    pa_rtclock_now_args(&now);
    fade_userdata->time_event = pa_core_rttime_new(u->core, now, __set_volume_fade_timeout_cb, fade_userdata);
    if(!(fade_userdata->time_event)) {
...
/* Use mute instead of volume for muteall */
        ...
        case SUBCOMMAND_SET_ROUTE_INFO: {
            ...
            if (u->audio_mgr.intf.set_route_info) {
                pa_log_error("set_route_info. key(%s), value(%s), addtional_info(%p)", key, value, addtional_info);
...
#endif
        {
            /* pulsesink probe stream selects voip sink. due to voip driver limitations in msm chipset,
              * when probe stream is destroyed before creating voip rx stream caused buffer underrun.
              * This will lead to sample drop and strange noise in video call. Following code selects null sink
              * for probe stream during video call.
              */
            if (u->board_config && pa_streq(u->board_config->codec, AUDIO_CODEC_MSM8X16)) { /* this is sprd msm voip */
                char * policy = NULL, *name = NULL;
                policy = pa_proplist_gets(new_data->proplist, PA_PROP_MEDIA_POLICY);
                name = pa_proplist_gets(new_data->proplist, PA_PROP_MEDIA_NAME);
                if (policy && pa_streq(policy, "voip") && name && pa_streq(name, "pulsesink probe")) {
                    new_data->sink = pa_namereg_get(c, "null", PA_NAMEREG_SINK);
                    goto exit;
                }
            }
...
static pa_hook_result_t sink_input_move_start_cb(pa_core *core, pa_sink_input *i, struct userdata *u) {
   ...
   if (core->state == PA_CORE_SHUTDOWN)
    ...
    if (AUDIO_IS_ERROR((audio_ret = policy_set_mute(u, i->index, (uint32_t)-1, AUDIO_DIRECTION_OUT, 1, false)))) {
...
static pa_hook_result_t sink_input_move_finish_cb(pa_core *core, pa_sink_input *i, struct userdata *u) {
    ...
    if (u->bt_off_idx == -1 && !u->muteall) {
        if (AUDIO_IS_ERROR((audio_ret = __update_volume(u, i->index, (uint32_t)-1, (uint32_t)-1, false)))) {
            pa_log_debug("__update_volume for stream[%d] returns error:0x%x", i->index, audio_ret);
        }
        if (AUDIO_IS_ERROR((audio_ret = policy_set_mute(u, i->index, (uint32_t)-1, AUDIO_DIRECTION_OUT, 0, false)))) {
