...
static pa_hook_result_t sink_input_put_cb(pa_core *core, pa_sink_input *i, struct userdata *u) {
    ...
    uint32_t gain_type = AUDIO_GAIN_TYPE_DEFAULT;
    pa_hook_result_t ret = 0;
#endif
    pa_core_assert_ref(core);
    pa_sink_input_assert_ref(i);

#ifdef __TIZEN__
    audio_data = pa_shared_get(core, "tizen-audio-data");
    audio_intf = (audio_interface_t *)pa_shared_get(core, "tizen-audio-interface");

    ret = process(u, i, true);

    role = pa_proplist_gets(i->proplist, PA_PROP_MEDIA_ROLE);
    if (i != NULL && role != NULL) {
        si_gain_type_str = pa_proplist_gets(i->proplist, PA_PROP_MEDIA_TIZEN_GAIN_TYPE);
        if (si_gain_type_str)
            pa_atou(si_gain_type_str, &gain_type);

        if(pa_streq(role, "phone") && (gain_type == AUDIO_GAIN_TYPE_SHUTTER1 ||
            gain_type == AUDIO_GAIN_TYPE_SHUTTER2 || gain_type == AUDIO_GAIN_TYPE_CAMCODING)) {
            double volume_linear = 1.0f;
            double volume_gain = 1.0f;
            audio_info_t audio_info;
            pa_cvolume cv;

            u->def = pa_namereg_get_default_sink(core);

            if (audio_intf->set_route_info)
                audio_intf->set_route_info(audio_data, "fm_mute", "1", NULL);

            if (audio_intf->get_route)
                audio_intf->get_route(audio_data, &u->session, &u->subsession, &u->device_in, &u->device_out, &u->device_flags);

            if (audio_intf->set_route && u->device_out != AUDIO_DEVICE_OUT_SPEAKER) {
                audio_intf->set_route(audio_data, u->session, u->subsession, u->device_in, AUDIO_DEVICE_OUT_SPEAKER, u->device_flags);

                audio_info.stream.gain_type = gain_type;
                if(audio_intf->get_gain_value) {
                    audio_intf->get_gain_value(audio_data, &audio_info, 0, &volume_gain);
                }
                volume_linear = volume_linear * volume_gain;

                pa_cvolume_set(&cv, i->sample_spec.channels, pa_sw_volume_from_linear(volume_linear));
                pa_sink_input_set_volume(i, &cv, TRUE, TRUE);
            }

            pa_log_info("route was changed because of shutter. session(%d:%d), in(%d), out(%d), volume(%f), flags(%x), get_route(%p), set_mixer_value(%p)",
                u->session, u->subsession, AUDIO_DEVICE_IN_MIC, AUDIO_DEVICE_OUT_SPEAKER,
                volume_linear, u->device_flags, audio_intf->get_route, audio_intf->set_mixer_value);
        }
    }
    return ret;
#else
...
static pa_hook_result_t sink_input_unlink_cb(pa_core *core, pa_sink_input *i, struct userdata *u) {
    ...
    if (i != NULL && role != NULL) {
        ...
        if(pa_streq(role, "phone") && (gain_type == AUDIO_GAIN_TYPE_SHUTTER1 ||
            gain_type == AUDIO_GAIN_TYPE_SHUTTER2 || gain_type == AUDIO_GAIN_TYPE_CAMCODING)) {
            if (audio_intf->set_route && u->device_out != AUDIO_DEVICE_OUT_SPEAKER)
                audio_intf->set_route(audio_data, u->session, u->subsession, u->device_in, u->device_out, u->device_flags);

            if (audio_intf->set_route_info)
                audio_intf->set_route_info(audio_data, "fm_mute", "0", NULL);

            pa_namereg_set_default_sink (core, u->def);

            pa_log_info("route was reverted because of shutter. session(%d:%d), in(%d), out(%d), flags(%x), get_route(%p), set_mixer_value(%p)",
                u->session, u->subsession, u->device_in, u->device_out, u->device_flags, audio_intf->get_route, audio_intf->set_mixer_value);
        }
    }
