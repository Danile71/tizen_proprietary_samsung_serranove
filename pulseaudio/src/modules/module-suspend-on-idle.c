...
{
    const char *value = pa_proplist_gets(p, "need_audio_pm");
    if (value) {
        return atoi (value);
    }
    return 0;
...
static pa_hook_result_t device_unlink_hook_cb(pa_core *c, pa_object *o, struct userdata *u) {
    ...
    if (pa_sink_isinstance(o)) {
        sink = PA_SINK(o);
        pa_log_info ("sink [%p][%d] is unlinked, now update pm", sink, (sink) ? sink->index : (uint32_t)-1);
        update_pm_status(u, PM_SINK, sink->index, PM_SUSPEND);
    } else if (pa_source_isinstance(o)) {
        source = PA_SOURCE(o);
        pa_log_info ("source [%p][%d] is unlinked, now update pm", source, (source) ? source->index : (uint32_t)-1);
