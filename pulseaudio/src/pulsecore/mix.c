...
void pa_smooth_volume_memchunk(
        pa_sink_input *i,
        pa_memchunk *c,
        const pa_sample_spec *spec,
        const pa_cvolume *volume) {
    ...
    if (pa_atomic_load(&i->thread_info.smooth_requested)) {
        pa_cvolume smooth_volume;
        uint32_t length = c->length;
        char *data = (char*)pa_memblock_acquire_chunk(c);

        // FIXME : i->sample_spec, after volume, before volume
        if (i->get_smooth_volume(i, i->thread_info.amount_of_current, &smooth_volume)) {
            pa_log_error("fail to get smooth volumes. smooth volume won't be adjusted.");
            calc_volume_table[spec->format] ((void *)linear, volume);
            do_volume(data, (void *)linear, spec->channels, c->length);
        } else {
            int size = 0;

#ifdef PA_SMOOTH_VOLUME_DETAIL
            pa_log_debug("Enter %d bytes, rate(%d), channels(%d)", c->length, spec->rate, spec->channels);
#endif
            while(length > 0) {
                size = PA_MIN(i->thread_info.amount_of_period, length);
                calc_volume_table[spec->format] ((void *)linear, &smooth_volume);
                do_volume(data, (void *)linear, spec->channels, size);
#ifdef PA_SMOOTH_VOLUME_DETAIL
                pa_log_debug("data(0x%x), cvolume(%d), linear(%d), size(%d), cur_pos(%d)", data, smooth_volume.values[0], linear[0].i, size, i->thread_info.amount_of_current);
#endif
                i->thread_info.amount_of_current += size;
                length -= size;
                data += size;

                if(i->thread_info.amount_of_current >= i->thread_info.amount_of_duration)
                    break;

                i->get_smooth_volume(i, i->thread_info.amount_of_current, &smooth_volume);
            }
        }

        if (i->thread_info.amount_of_current >= i->thread_info.amount_of_duration) {
            pa_atomic_dec(&i->thread_info.smooth_requested);
            i->thread_info.muted = i->muted;  // FIXME don't access thread_info
            i->thread_info.soft_volume = i->soft_volume; // FIXME
            if (length > 0) { /* remainder */
                pa_cvolume volume_mute;
                pa_cvolume_mute(&volume_mute, i->sample_spec.channels);

                calc_volume_table[spec->format] ((void *)linear, i->thread_info.muted ? &volume_mute : volume);
                do_volume(data, (void *)linear, spec->channels, length);
                pa_log_debug("fading end. data(%p), cvolume(%d), linear(%d), size(%d), cur_pos(%d). remainder",
                    data, volume->values[0], linear[0].i, length, i->thread_info.amount_of_current);
            }
        }
        pa_memblock_release(c->memblock);
    } else {
