...
static void pa_sink_input_write_pcm_dump(pa_sink_input *i, pa_memchunk *chunk)
{
    ...
    if (i->core->pcm_dump & PA_PCM_DUMP_PA_SINK_INPUT && !i->dump_fp && i->state == PA_SINK_INPUT_RUNNING) {
        pa_gettimeofday(&now);
...
static uint32_t get_smooth_volume(pa_sink_input *i, uint32_t usec, pa_cvolume* dest);
#endif
...
static void reset_callbacks(pa_sink_input *i) {
    ...
    i->mute_changed = NULL;
#if defined(__TIZEN__) && defined(PA_SMOOTH_VOLUME)
    i->get_smooth_volume = get_smooth_volume;
#endif
}
...
#if defined(__TIZEN__) && defined(PA_SMOOTH_VOLUME)
    pa_atomic_store(&i->thread_info.smooth_requested, 0);
    i->thread_info.amount_of_current = 0;
    i->thread_info.amount_of_duration = 0;
    i->thread_info.amount_of_period = 0;
    pa_atomic_store(&i->thread_info.fade_onff, true);
#endif
...
#if defined (__TIZEN__) && defined (ENABLE_PCM_DUMP)
    if (i->state == PA_SINK_INPUT_RUNNING && i->dump_fp && (i->core->pcm_dump_option & PA_PCM_DUMP_OPTION_SEPARATED)) {
...
#if defined(__TIZEN__) && defined(PA_SMOOTH_VOLUME)
    /* FIXME: have to consider resampling */
    do_volume_adj_here = TRUE;
        ...
        while (tchunk.length > 0) {
...
#if defined(__TIZEN__) && defined(PA_SMOOTH_VOLUME)
                    pa_smooth_volume_memchunk(i, &wchunk, &i->thread_info.sample_spec, &v);
#else
...
#if defined(__TIZEN__) && defined(PA_SMOOTH_VOLUME)
                    pa_smooth_volume_memchunk(i, &wchunk, &i->thread_info.sample_spec, &i->thread_info.soft_volume);
#else
            ...
            if (!i->thread_info.resampler) {
                ...
                if (nvfs) {
                    pa_memchunk_make_writable(&wchunk, 0);
#if defined(__TIZEN__) && defined(PA_SMOOTH_VOLUME)
                    pa_smooth_volume_memchunk(i, &wchunk, &i->sink->sample_spec, &i->volume_factor_sink);
#else
...
#if defined(__TIZEN__) && defined(PA_SMOOTH_VOLUME)
                        pa_smooth_volume_memchunk(i, &rchunk, &i->sink->sample_spec, &i->volume_factor_sink);
#else
...
/* Called from main context */
void pa_sink_input_set_smooth_volume(pa_sink_input *i, const pa_cvolume *volume, pa_bool_t save, pa_bool_t absolute) {
    pa_cvolume v;
...
#if defined(__TIZEN__) && defined(PA_SMOOTH_VOLUME)
    pa_atomic_store(&i->thread_info.fade_onff, false);
#endif
...
#ifdef __TIZEN__
/* Called from main context */
void pa_sink_input_set_smooth_mute(pa_sink_input *i, pa_bool_t mute, pa_bool_t save) {
    pa_sink_input_assert_ref(i);
    pa_assert_ctl_context();
    pa_assert(PA_SINK_INPUT_IS_LINKED(i->state));
...
#if defined(__TIZEN__) && defined(PA_SMOOTH_VOLUME)
static uint32_t get_smooth_volume(pa_sink_input *i, uint32_t bytes, pa_cvolume* dest)
{
    uint8_t k = 0;
    pa_volume_t volume = 0;

    pa_cvolume_set(dest, i->sample_spec.channels, volume);

    for(k=0; k<i->sample_spec.channels; k++) {
        volume = i->thread_info.angle[k] * (double)bytes;
        dest->values[k] = lrint(volume);

        if(i->thread_info.decrease[k])
            dest->values[k] *= -1;

        dest->values[k] += i->thread_info.delta_volume[k];
    }

#ifdef PA_SMOOTH_VOLUME_DETAIL
    pa_log_debug("volume(%d), bytes(%d), vol(%f), keep(%d)", volume, bytes, vol, i->thread_info.delta_volume.values[0]);
#endif
    return 0;
}

static int __calc_smooth_volume(pa_sink_input* i, pa_cvolume* src, pa_cvolume* dest)
{
    int k = 0;
    uint32_t frame_size = pa_frame_size(&i->sample_spec);

    pa_atomic_store(&i->thread_info.smooth_requested, 0);
    i->thread_info.amount_of_current = 0;
    i->thread_info.amount_of_duration = (pa_usec_to_bytes(SMOOTH_VOLUME_DURATION, &i->sample_spec) / frame_size) * frame_size;
    i->thread_info.amount_of_period = i->thread_info.amount_of_duration / pa_usec_to_bytes(SMOOTH_VOLUME_FREQUENCY, &i->sample_spec);
    i->thread_info.amount_of_period = PA_MAX(frame_size, (i->thread_info.amount_of_period / frame_size) * frame_size);

    for(k=0; k<i->sample_spec.channels; k++) {
        if(dest->values[k] > src->values[k]) { /* increase */
            i->thread_info.angle[k] = (double)(dest->values[k]-src->values[k]) / (double)(i->thread_info.amount_of_duration);
            i->thread_info.decrease[k] = 0;
        } else { /* decrease */
            i->thread_info.angle[k] = (double)(src->values[k]-dest->values[k]) / (double)(i->thread_info.amount_of_duration);
            i->thread_info.decrease[k] = 1;
        }
        i->thread_info.delta_volume[k] = src->values[k];
    }

    pa_atomic_inc(&i->thread_info.smooth_requested);

    pa_log_info("sink-input[%d] start_vol(%d) -> end_vol(%d), ch(%d), diff(%d) dur(%d), freq(%d), angle(%f), keep(%d)",
        i->index, src->values[0], dest->values[0], i->sample_spec.channels, dest->values[0]-src->values[0],
        i->thread_info.amount_of_duration, i->thread_info.amount_of_period, i->thread_info.angle[0], i->thread_info.delta_volume[0]);
    ...
    switch (code) {
        ...
        case PA_SINK_INPUT_MESSAGE_SET_SOFT_VOLUME:
            ...
            if (!pa_cvolume_equal(&i->thread_info.soft_volume, &i->soft_volume)) {
                pa_log_info("set smooth volume. sink-input[%d], fade_onoff(%d)", i->index, i->thread_info.fade_onff);

                if (pa_atomic_load(&i->thread_info.fade_onff) == false) {
                    i->thread_info.soft_volume = i->soft_volume;
                } else if (pa_atomic_load(&i->thread_info.smooth_requested) == 0) {
                    /* fade_onoff : this is one time value whenever mute function is called */
                    pa_atomic_store(&i->thread_info.fade_onff, false);
                    __calc_smooth_volume(i, &i->thread_info.soft_volume, &i->soft_volume);
                } else {
                    pa_log_info("skip set_volume(already requested). sink-input[%d] start_vol(%d) -> end_vol(%d)",
                        i->index, i->thread_info.soft_volume.values[0], i->soft_volume.values[0]);
                }
        ...
        {
            pa_cvolume volume;
            pa_cvolume_mute(&volume, i->sample_spec.channels);

            pa_log_info("set %s. sink-input[%d], fade_onoff(%d)", i->muted ? "mute" : "unmute", i->index, i->thread_info.fade_onff);
            ...
            if (i->thread_info.muted != i->muted) {
                if (pa_atomic_load(&i->thread_info.fade_onff) == false) {
                    ...
                    pa_atomic_store(&i->thread_info.fade_onff, false);
                    if (i->muted)
                        __calc_smooth_volume(i, &i->thread_info.soft_volume, &volume);
                    else {
                        i->thread_info.muted = i->muted;
                        __calc_smooth_volume(i, &volume, &i->thread_info.soft_volume);
                    }
                }
                pa_sink_input_request_rewind(i, 0, TRUE, FALSE, FALSE);
            }
