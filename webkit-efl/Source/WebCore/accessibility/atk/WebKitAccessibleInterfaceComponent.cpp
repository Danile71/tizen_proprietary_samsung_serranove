...
static gboolean webkitAccessibleComponentGrabHighlight(AtkComponent* component)
{
    g_return_val_if_fail(ATK_IS_COMPONENT(component), false);

    if(core(component) && core(component)->page() && core(component)->page()->chrome()->client())
    {
        core(component)->page()->chrome()->client()->highlightNode(core(component)->node());
        return true;
    }
    return false;
}

static gboolean webkitAccessibleComponentClearHighlight(AtkComponent* component)
{
    g_return_val_if_fail(ATK_IS_COMPONENT(component), false);

    if(core(component) && core(component)->page() && core(component)->page()->chrome()->client())
    {
        core(component)->page()->chrome()->client()->clearHighlightedNode(core(component)->node());
        return true;
    }
    return false;
}

static gboolean webkitAccessibleComponentGetHighlightIndex(AtkComponent* component)
{
    g_return_val_if_fail(ATK_IS_COMPONENT(component), false);

    // This is simple implementation of HighlightIndex property, which is used by
    // screen reader to determine reading order.
    // The ATK component can return:
    // 0         - default value, the reading order depends on the object's coordinates;
    // 1, 2, ... - determines the custom order;
    // -1        - excluding the object from the reading order

    // TODO - implement interface, that will allow the web developer to set the property 
    //        for the DOM element
...
void webkitAccessibleComponentInterfaceInit(AtkComponentIface* iface)
{
    ...
    iface->grab_highlight = webkitAccessibleComponentGrabHighlight;
    iface->clear_highlight = webkitAccessibleComponentClearHighlight;
    iface->get_highlight_index = webkitAccessibleComponentGetHighlightIndex;
}
