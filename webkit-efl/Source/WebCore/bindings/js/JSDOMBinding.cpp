...
PassRefPtr<DOMStringList> jsValueToWebCoreDOMStringList(ExecState* exec, JSValue value)
{
    if (value.inherits(&JSArray::s_info)) {
        ...
        for (unsigned i = 0; i < jsArray->length(); i++) {
            String string = valueToStringWithUndefinedOrNullCheck(exec, jsArray->pop(exec));
            strings->append(string);
        }
        return strings.release().leakRef();
    }
