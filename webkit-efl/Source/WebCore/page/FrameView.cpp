...
#if ENABLE(TIZEN_BLOCK_SENDING_RESIZE_EVENT_WHILE_LOADING)
...
#if ENABLE(TIZEN_ELEMENTS_NESTED_IN_FLATTENED_FRAME_FIX)
bool FrameView::inFlattenFrame()
{
    return (m_frame && m_frame->ownerElement() && (m_frame->ownerElement()->hasTagName(iframeTag) || m_frame->ownerElement()->hasTagName(frameTag) ) && m_frame->settings() && m_frame->settings()->frameFlatteningEnabled());
}
#endif

#if ENABLE(TIZEN_WEBKIT2_TILED_SCROLLBAR)
void FrameView::updateScrollbars(const IntSize& desiredOffset)
{
    ScrollView::updateScrollbars(desiredOffset);

    Page* page = frame() ? frame()->page() : 0;
    if (!page)
        return;

    page->chrome()->client()->updateScrollbarModes(frame());
}
#endif
...
#if ENABLE(TIZEN_TEXT_AUTOSIZING) // Add null check routine
                if (m_frame->document() && m_frame->document()->textAutosizer())
#endif
...
{
#if ENABLE(TIZEN_DISABLE_CUSTOM_SCROLLBAR)
...
void FrameView::adjustViewSize()
{
    ...
    // If ScrollingArea::constrainsScrollingToContentEdge is true, the scroll position can be adjusted
    // before painting tiles. So, we prevent scrollingToContentEdge in here.
    bool constrainsScrollingToContentEdge = ScrollView::constrainsScrollingToContentEdge();
    ScrollView::setConstrainsScrollingToContentEdge(false);
#endif
#if ENABLE(TIZEN_ADJUST_CONTENTS_SIZE_FOR_MINUS_X_WORKAROUND)
    // FIXME: In the case <html> starts at non-zero position (for example, the contents overflow to left of the <html> by dir=RTL (right-to-left)),
    // the contents size should not include the left area of <html> because the leftmost paint position is (0,0) of the <html>.
    // So, in here, I adjusted the contents size not to include the left area of <html>.
    // It's not reasonable patch for Desktop browsers because it works wrong when the viewport size is changed.
    // But now the TIZEN is only for Mobile, it works for now. (But need to be fixed ASAP)
    if (rect.x() < 0)
        setContentsSize(IntSize(size.width() + rect.x(), size.height()));
    else
#endif
    setContentsSize(size);
#if ENABLE(TIZEN_WEBKIT2_TILED_BACKING_STORE)
    // Recover constrainsScrollingToContentEdge
    ScrollView::setConstrainsScrollingToContentEdge(constrainsScrollingToContentEdge);
#endif
}
...
#if ENABLE(TIZEN_WEBKIT2_TILED_BACKING_STORE)
    // When showing IME, the fixed position element can be scrolled over maxOffset.
    int width = contentsSize.width();
    int height = contentsSize.height();
#else
    IntRect visibleContentRect = this->visibleContentRect();
    int width = contentsSize.width() - visibleContentRect.width();
    int height = contentsSize.height() - visibleContentRect.height();
#endif
    IntSize maxOffset(width, height);

    FloatSize dragFactor = fixedElementsLayoutRelativeToFrame() ? FloatSize(1, 1) : FloatSize(
        (width * frameScaleFactor) / maxOffset.width(),
        (height * frameScaleFactor) / maxOffset.height());
...
void FrameView::setFixedVisibleContentRect(const IntRect& visibleContentRect, bool allowFixedElementUpdate)
#else
...
void FrameView::requestVisibleContentRectRestore(const IntPoint& position, float scale)
{
    Page* page = m_frame->page();
    if (!page)
        return;
    if (page->mainFrame() != m_frame)
        return;
    page->chrome()->client()->requestVisibleContentRectRestore(position, scale);
}
#endif
...
#if ENABLE(TIZEN_IFRAME_FLATTENING_WORKAROUND)
static void setAncestorNeedsLayoutIfNeeded(RenderObject* ancestor, RenderObject* descendant)
{
    for (RenderObject* r = descendant; r; r = r->container()) {
        if (!r->selfNeedsLayout())
...
#endif // ENABLE(TIZEN_IFRAME_FLATTENING_WORKAROUND)
...
#if ENABLE(TIZEN_IFRAME_FLATTENING_WORKAROUND)
                setAncestorNeedsLayoutIfNeeded(m_layoutRoot, relayoutRoot);
#endif // ENABLE(TIZEN_IFRAME_FLATTENING_WORKAROUND)
...
#if ENABLE(TIZEN_FRAMEVIEW_NULL_CHECK_WORKAROUND)
    // If it is called from resetScrollbars during FrameView destruction, the frame() can be 0.
    // In this case, the below codes that access the frame() shouldn't be executed.
    if (!frame())
        return;
#endif
    if (offset != scrollOffset())
