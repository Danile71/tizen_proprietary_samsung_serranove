...
public:
    ...
    virtual bool supportsSelectionForegroundColors() const { return m_supportsSelectionForegroundColors; }

    virtual void themeChanged();

    // Set platform colors and notify they changed
    void setSelectionForegroundColor(int foreR, int foreG, int foreB, int foreA, int backR, int backG, int backB, int backA);
    void setSelectionBackgroundColor(int foreR, int foreG, int foreB, int foreA, int backR, int backG, int backB, int backA);
...
#if PLATFORM(TIZEN)
    void adjustSizeConstraints(RenderStyle*, FormType, StyleResolver*) const;
#else
    ...
    virtual String extraFullScreenHorizontalStyleSheet();
    virtual String extraFullScreenVerticalStyleSheet();
#else
...
#if ENABLE(TIZEN_GSTREAMER_VIDEO)
    virtual bool paintMediaTimeRemaining(RenderObject* object, const PaintInfo& info, const IntRect& rect);
    virtual bool paintMediaDefaultImage(RenderObject*, const PaintInfo&, const IntRect&);
    virtual bool paintMediaOverlayLoadingSpinner(RenderObject*, const PaintInfo&, const IntRect&);
    virtual bool paintMediaRotateButton(RenderObject* object, const PaintInfo& info, const IntRect& rect);
    virtual bool paintMediaControlPanelBackGroundImage(RenderObject*, const PaintInfo&, const IntRect&);
    virtual double mediaControlsFadeInDuration() { return 0.0; }
    virtual double mediaControlsFadeOutDuration() { return 0.0; }
#endif
#endif
    ...
    {
#if ENABLE(TIZEN_WEBKIT2_CHANGEABLE_UI_FOR_FOCUS_RING)
        return Color();
#else
...
#if ENABLE(TIZEN_CARET_INTERVAL)
    virtual double caretBlinkInterval() const { return 1.0; }
#endif
...
private:
...
#if ENABLE(TIZEN_GSTREAMER_VIDEO)
    Color m_mediaBufferingColor;
    Color m_mediaPlayingColor;
#endif
#endif
    ...
    bool m_supportsSelectionForegroundColors;
...
#endif
#if ENABLE(TIZEN_THEME_SCALE_SUPPORT)
    IntRect convertToScaleRect(const PaintInfo& info, const IntRect& rect, FormType type, RenderObject* object);
#endif
