...
#if ENABLE(TIZEN_TEXTURE_MAPPER_CULLER)
    TextureMapperCuller* culler;
#endif
#if ENABLE(TIZEN_CORRECT_CLIPPING_WITH_INTERMEDIATE_SURFACE)
    bool bBindingIntermediateSurface;
#endif
...
#endif
#if ENABLE(TIZEN_CORRECT_CLIPPING_WITH_INTERMEDIATE_SURFACE)
        , bBindingIntermediateSurface(false)
#endif
    { }
};
...
public:
...
#if ENABLE(TIZEN_TEXTURE_MAPPER_CULLER)
    void paintWithCuller(const FloatRect&);
#endif
    void paint();
    ...
    bool childOfFullScreenLayerForVideoIncludingSelf();
    bool hasFullScreenLayerForVideoIncludingSelf();
#endif
...
private:
...
#if ENABLE(TIZEN_TEXTURE_MAPPER_CULLER)
    void computeOpaqueRectRecursive(TextureMapperCuller*);
#endif
#if ENABLE(TIZEN_UI_SIDE_ANIMATION_SYNC)
    bool hasStoppedAnimation(AnimatedPropertyID type);
#endif
...
#if ENABLE(TIZEN_INHERIT_REF_COUNT_TO_TEXTURE_MAPPER_LAYER)
    RefPtr<TextureMapperPlatformLayer> m_contentsLayer;
#else
    ...
    {
#if ENABLE(CSS_FILTERS)
        return !m_state.filters.isEmpty();
#else
        return false;
#endif
    }
