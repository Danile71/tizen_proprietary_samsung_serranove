...
bool ImageFrame::copyBitmapData(const ImageFrame& other)
{
    ...
    m_strideWidth = other.m_strideWidth;
...
void ImageFrame::setStride()
{
    ...
    m_strideWidth = m_stride / sizeof(PixelData);
    return;
}

bool ImageFrame::setSize(int newWidth, int newHeight, bool isGLTargetSurface, bool decodeInCpuMemoryForcely)
{
    ASSERT(!width() && !height());
    WebCore::SharedPlatformSurfaceManagement& sharedPlatformSurfaceManagement = WebCore::SharedPlatformSurfaceManagement::getInstance();

    // Decode the image in sharedPlatformSurface when
    //   (1) the target surface is GLsurface AND
    //   (2) the number of sharedPlatformSurface that already have been created is less than 150 AND
    //   (3) decodeInCpuMemoryForcely is false AND
    ...
    if (isGLTargetSurface && sharedPlatformSurfaceManagement.canCreateSharedPlatformSurface(150) && !decodeInCpuMemoryForcely
        && newWidth <= CAIRO_GL_SURFACE_MAX_WIDTH && newHeight <= CAIRO_GL_SURFACE_MAX_HEIGHT) {
