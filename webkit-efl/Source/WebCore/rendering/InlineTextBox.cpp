...
void InlineTextBox::paint(PaintInfo& paintInfo, const LayoutPoint& paintOffset, LayoutUnit /*lineTop*/, LayoutUnit /*lineBottom*/)
{
    ...
    const QualifiedName& attrName = HTMLNames::colorAttr;
    if (renderer()->node() && renderer()->node()->parentElement() && (renderer()->node()->parentElement()->hasTagName(HTMLNames::fontTag) && renderer()->node()->parentElement()->hasAttribute(attrName)
            || renderer()->node()->parentElement()->style() && !renderer()->node()->parentElement()->style()->getPropertyValue(attrName.toString()).isEmpty()))
        context->useFontColor(true);
    else
        context->useFontColor(false);
#endif
...
void InlineTextBox::paintSelection(GraphicsContext* context, const FloatPoint& boxOrigin, RenderStyle* style, const Font& font, Color textColor)
{
    ...
    Color c = renderer()->selectionBackgroundColor().blendWithWhite();
#else
...
void InlineTextBox::paintTextMatchMarker(GraphicsContext* context, const FloatPoint& boxOrigin, DocumentMarker* marker, RenderStyle* style, const Font& font)
{
    LayoutUnit selectionHeight = this->selectionHeight();
    ...
    markerRect = renderer()->localToAbsoluteQuad(FloatQuad(markerRect), SnapOffsetForTransforms).enclosingBoundingBox();
    toRenderedDocumentMarker(marker)->setRenderedRect(markerRect);
    ...
    if (renderer()->frame()->editor()->markedTextMatchesAreHighlighted()) {
        ...
        GraphicsContextStateSaver stateSaver(*context);
        updateGraphicsContext(context, color, color, 0, style->colorSpace());  // Don't draw text at all!
...
float InlineTextBox::positionForOffset(int offset) const
{
    ...
    TextRun run = constructTextRun(styleToUse, font);
