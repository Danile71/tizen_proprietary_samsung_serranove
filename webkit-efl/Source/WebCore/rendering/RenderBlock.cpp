...
#if ENABLE(TIZEN_INFLATE_REPAINT_FOR_NO_OUTLINE_WORKAROUND)
        if (style() && style()->userModify() != READ_ONLY && maximalOutlineSize(PaintPhaseOutline) == 0)
            repaintRect.inflate(5); // FIXME : The value for this should be calculated correctly, but apply default outline value to fix the issue for now.
#endif
        if (hasOverflowClip()) {
...
{
#if ENABLE(TIZEN_BLOCK_SELECTION_GAP)
    LayoutUnit logicalTop = lastLogicalTop + paddingTop();
    LayoutUnit logicalHeight = blockDirectionOffset(rootBlock, offsetFromRootBlock) + logicalBottom - logicalTop -paddingBottom();
#else
...
#if ENABLE(TIZEN_SELECTION_BACKGROUND_COLOR)
    Color selectionBackgroundColor = this->selectionBackgroundColor().blendWithWhite();
#else
    Color selectionBackgroundColor = this->selectionBackgroundColor();
#endif

#if ENABLE(TIZEN_PAINT_SELECTION_ANTIALIAS_NONE)
    if (paintInfo) {
        bool antialias = paintInfo->context->shouldAntialias();
        ...
        paintInfo->context->setShouldAntialias(antialias);
    }
#else
    if (paintInfo)
        paintInfo->context->fillRect(pixelSnappedIntRect(gapRect), selectionBackgroundColor, style()->colorSpace());
                                                ...
                                                RenderObject* selObj, LayoutUnit logicalLeft, LayoutUnit logicalTop, LayoutUnit logicalHeight, const PaintInfo* paintInfo)
{
...
#if ENABLE(TIZEN_SELECTION_BACKGROUND_COLOR)
    Color selectionBackgroundColor = selObj->selectionBackgroundColor().blendWithWhite();
#else
    Color selectionBackgroundColor = selObj->selectionBackgroundColor();
#endif
    ...
    if (paintInfo) {
        bool antialias = paintInfo->context->shouldAntialias();
        paintInfo->context->setShouldAntialias(false);
        paintInfo->context->fillRect(pixelSnappedIntRect(gapRect), selectionBackgroundColor, selObj->style()->colorSpace());
        paintInfo->context->setShouldAntialias(antialias);
    }
#else
    if (paintInfo)
...
static bool rangesIntersect(LayoutUnit floatTop, LayoutUnit floatBottom, LayoutUnit objectTop, LayoutUnit objectBottom)
{
...
#if ENABLE(TIZEN_ADJUST_IMG_WIDTH_WITH_PERCENT)
                if (isHTMLMarquee() && child->isRenderImage()) {
                    float childWidth = child->maxPreferredLogicalWidth();
                    float childMinWidth = child->minPreferredLogicalWidth();
                    float childMaxWidth = child->maxPreferredLogicalWidth();

                    if (child->style()->width().isPercent()) {
                        childWidth = clientWidth() * child->style()->width().percent() / 100;
                        childMaxWidth = childWidth;
                    }

                    if (child->style()->minWidth().isPercent())
                        childMinWidth = max(childWidth, clientWidth() * child->style()->minWidth().percent() / 100);

                    if (child->style()->maxWidth().isPercent())
                        childMaxWidth = min(childWidth, clientWidth() * child->style()->maxWidth().percent() / 100);

                    childMin += childMinWidth;
                    childMax += childMaxWidth;
                } else {
...
#if ENABLE(TIZEN_BASELINE_POSITION_FIX)
...
void RenderBlock::addFocusRingRects(Vector<IntRect>& rects, const LayoutPoint& additionalOffset)
{
    ...
    if (inlineElementContinuation())
        inlineElementContinuation()->addFocusRingRects(rects, flooredLayoutPoint(LayoutPoint(additionalOffset + inlineElementContinuation()->containingBlock()->location() - location())));
}
