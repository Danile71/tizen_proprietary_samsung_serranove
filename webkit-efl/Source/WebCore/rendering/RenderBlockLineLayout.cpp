...
#if ENABLE(TIZEN_CSS3_TEXT)
    ETextAlignLast alignmentLast = style()->textAlignLast();
                                                         ...
                                                         GlyphOverflowAndFallbackFontsMap& textBoxDataMap, VerticalPositionCache& verticalPositionCache)
{
    ...
    float logicalLeft = logicalLeftOffsetForLine(logicalHeight(), lineInfo.isFirstLine(), lineLogicalHeight);
    float availableLogicalWidth = logicalRightOffsetForLine(logicalHeight(), lineInfo.isFirstLine(), lineLogicalHeight) - logicalLeft;
...
bool RenderBlock::positionNewFloatOnLine(FloatingObject* newFloat, FloatingObject* lastFloatFromPreviousLine, LineInfo& lineInfo, LineWidth& width)
{
    ...
    LayoutUnit paginationStrut = newFloat->m_paginationStrut;
