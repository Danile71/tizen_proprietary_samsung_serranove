...
float RenderBox::splitScrollOverflow(ScrollDirection direction, ScrollGranularity granularity, float offset)
{
    RenderLayer* l = layer();
    if (l)
        return l->splitScrollOverflow(direction, granularity, offset);

    RenderBlock* b = containingBlock();
    if (b && !b->isRenderView())
        return b->splitScrollOverflow(direction, granularity, offset);
...
static bool isCandidateForOpaquenessTest(RenderBox* childBox)
{
    ...
    if (childStyle->position() != StaticPosition && childBox->containingBlock() != childBox->parent())
        return false;
    if (childStyle->visibility() != VISIBLE || childStyle->wrapShapeOutside())
        return false;
...
#if ENABLE(TIZEN_PAINT_MASK_IMAGE_ANTIALIAS_NONE)
...
#if ENABLE(TIZEN_FIRST_LINE_IMAGE_PAINTING)
    if (RenderStyle* firstLineStyle = style()->getCachedPseudoStyle(FIRST_LINE)) {
        for (const FillLayer* backgroundLayer = firstLineStyle->backgroundLayers(); backgroundLayer; backgroundLayer = backgroundLayer->next()) {
            if (backgroundLayer->image() && image == backgroundLayer->image()->data()) {
                repaint();
                break;
            }
        }
...
void RenderBox::mapLocalToContainer(RenderBoxModelObject* repaintContainer, TransformState& transformState, MapCoordinatesFlags mode, bool* wasFixed) const
{
...
void RenderBox::computeRectForRepaint(RenderBoxModelObject* repaintContainer, LayoutRect& rect, bool fixed) const
{
    ...
    if (o->hasOverflowClip()) {
        ...
        if (containerBox->hasOverflowClip() && !containerBox->layer()->usesCompositedScrolling()) {
#endif
        LayoutRect repaintRect(topLeft, rect.size());
    ...
    const RenderBlock* cb, RenderRegion* region, LayoutUnit offsetFromLogicalTopOfFirstPage) const
{
    ...
    if (!logicalWidth.isIntrinsicOrAuto()) {
        RenderBlock* cb = containingBlock();
        if (cb && cb->view() && cb->view()->frameView() && cb->view()->frameView()->inFlattenFrame() && logicalWidth.isPercent() && (logicalWidthResult > availableLogicalWidth))
            logicalWidthResult = availableLogicalWidth;
    }
#endif
    ...
    switch (logicalHeight.type()) {
        ...
        case Calculated:
        {
            ...
            if (isOutOfFlowPositioned())
                ...
                availableHeight =  toRenderBox(cb)->availableLogicalHeight(IncludeMarginBorderPadding);
                // It is necessary to use the border-box to match WinIE's broken
                // box model.  This is essential for sizing inside
                // table cells using percentage heights.
...
LayoutUnit RenderBox::availableLogicalHeightUsing(const Length& h, AvailableLogicalHeightType heightType) const
{
    ...
    // FIXME: This is wrong if the containingBlock has a perpendicular writing mode.
    LayoutUnit availableHeight = containingBlock()->availableLogicalHeight(heightType);
...
#if ENABLE(TIZEN_FULLSCREEN_VIDEO_WORKAROUD)
    if (isRenderFullScreen()) {
        Frame* mainFrame = view()->frameView()->frame()->page()->mainFrame();
        if (mainFrame != view()->frameView()->frame())
            computedValues.m_extent = mainFrame->view()->visibleHeight();
    }
#endif
