...
public:
...
#if !ENABLE(TIZEN_OPTIMIZE_CONTENT_WIDTH_HEIGHT_CALCULATION)
    LayoutUnit contentWidth() const { return clientWidth() - paddingLeft() - paddingRight(); }
#else
    LayoutUnit contentWidth() const { return (style()->paddingLeft() == style()->paddingRight()) ? clientWidth() - (2 * paddingLeft()) : clientWidth() - paddingLeft() - paddingRight(); }
#endif
    ...
    virtual float splitScrollOverflow(ScrollDirection, ScrollGranularity, float offset);
#endif
...
protected:
    ...
    virtual void mapLocalToContainer(RenderBoxModelObject* repaintContainer, TransformState&, MapCoordinatesFlags mode = ApplyContainerFlip | SnapOffsetForTransforms, bool* wasFixed = 0) const OVERRIDE;
    virtual const RenderObject* pushMappingToContainer(const RenderBoxModelObject*, RenderGeometryMap&) const OVERRIDE;
