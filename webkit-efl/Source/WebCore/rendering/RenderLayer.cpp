...
#if ENABLE(TIZEN_BROWSER_DASH_MODE) && ENABLE(TIZEN_ACCELERATED_ROTATE_TRANSFORM)
...
RenderLayer::RenderLayer(RenderBoxModelObject* renderer)
    ...
    , m_hasAcceleratedRotate(0)
#endif
#if ENABLE(TIZEN_BROWSER_DASH_MODE) && ENABLE(TIZEN_ACCELERATED_ROTATE_TRANSFORM)
    , m_countUpdateTransform(0)
    , m_isAccelerated(0)
    , m_transformDashModeTimer(this, &RenderLayer::transformDashModeFired)
    , m_updateTransformTimer(this, &RenderLayer::updateTransformFired)
#endif
{
...
#if ENABLE(TIZEN_BROWSER_DASH_MODE) && ENABLE(TIZEN_ACCELERATED_ROTATE_TRANSFORM)
    if (m_transformDashModeTimer.isActive())
        m_transformDashModeTimer.stop();
    if (m_updateTransformTimer.isActive())
        m_updateTransformTimer.stop();
#endif
}
...
bool RenderLayer::acceleratedCompositingForOverflowScrollEnabled() const
{
    return renderer()->frame()
        && renderer()->frame()->page()
        && renderer()->frame()->page()->settings()->alwaysUseAcceleratedOverflowScroll();
}
...
#if ENABLE(TIZEN_ACCELERATED_ROTATE_TRANSFORM)
        if (m_isAccelerated && !m_hasAcceleratedRotate && renderer()->style()->transform().hasRotate())
            m_hasAcceleratedRotate = true;
#endif
#if ENABLE(TIZEN_BROWSER_DASH_MODE) && ENABLE(TIZEN_ACCELERATED_ROTATE_TRANSFORM)
        ++m_countUpdateTransform;
        if (!m_updateTransformTimer.isActive() && !m_isAccelerated) {
            m_updateTransformTimer.startOneShot(0.3);
        }

        if (m_isAccelerated && !m_transformDashModeTimer.isActive()) {
            WTF::setWebkitDashMode(WTF::DashModeBrowserJavaScript, 3000);
            m_transformDashModeTimer.startOneShot(2.9);
        }
#endif
    }
...
#if ENABLE(TIZEN_BROWSER_DASH_MODE) && ENABLE(TIZEN_ACCELERATED_ROTATE_TRANSFORM)
void RenderLayer::updateTransformFired(Timer<RenderLayer>*)
{
    const int kUpdateTransformRepeatThreshold = 10;
    if (m_countUpdateTransform > kUpdateTransformRepeatThreshold && !m_isAccelerated)
...
#if ENABLE(TIZEN_FIX_REPAINTING_BUG_OF_COMPOSITED_LAYER)
...
void RenderLayer::convertToLayerCoords(const RenderLayer* ancestorLayer, LayoutPoint& location) const
{
    ...
    location += toLayoutSize(m_topLeft);
}
...
#if ENABLE(ACCELERATED_OVERFLOW_SCROLLING) || ENABLE(TIZEN_CSS_OVERFLOW_SCROLL_ACCELERATION)
...
void RenderLayer::scrollRectToVisible(const LayoutRect& rect, const ScrollAlignment& alignX, const ScrollAlignment& alignY)
{
    ...
    if (renderer()->hasOverflowClip() && !restrictedByLineClamp) {
        ...
        IntSize clampedScrollOffset = clampScrollOffset(scrollOffset() + toSize(roundedIntRect(r).location()));
...
PassRefPtr<Scrollbar> RenderLayer::createScrollbar(ScrollbarOrientation orientation)
{
    ...
    if (hasCustomScrollbarStyle)
        ...
        widget = Scrollbar::createNativeScrollbar(this, orientation, RegularScrollbar, false);
#else
...
#if ENABLE(TIZEN_CSS_OVERFLOW_SCROLL_ALLOW_SCROLLING)
...
float RenderLayer::splitScrollOverflow(ScrollDirection direction, ScrollGranularity granularity, float offset)
{
    return ScrollableArea::splitScrollOverflow(direction, granularity, offset);
}
#endif
                        ...
                        PaintLayerFlags paintFlags)
{
...
#if ENABLE(TIZEN_FILTER_SCALE_BUG_FIX)
        float contentsScale = 1;
        Frame* frame = renderer()->frame();
        if (frame)
            contentsScale = frame->contentsScaleFactor();
        if (filterPainter.prepareFilterEffect(this, calculateLayerBounds(this, rootLayer, 0), parentPaintDirtyRect, filterRepaintRect, contentsScale)) {
#else
    ...
    if (shouldPaintContent || shouldPaintOutline || isPaintingOverlayScrollbars) {
        ...
        paintOffset = toLayoutPoint(layerBounds.location() - renderBoxLocation());
    }
    ...
    if (localPaintFlags & PaintLayerPaintingCompositingForegroundPhase) {
        ...
        paintList(m_normalFlowList.get(), rootLayer, context, paintDirtyRect, paintBehavior, paintingRoot, region, overlapTestRequests, localPaintFlags);
                                       ...
                                       const HitTestingTransformState* transformState, double* zOffset)
{
    ...
    hitLayer = hitTestList(m_normalFlowList.get(), rootLayer, request, result, hitTestRect, hitTestPoint,
                           localTransformState.get(), zOffsetForDescendantsPtr, zOffset, unflattenedTransformState.get(), depthSortDescendants);
    if (hitLayer) {
        if (!depthSortDescendants)
            return hitLayer;
                                 ...
                                 ) const
{
    ...
    if (renderer()->hasClipOrOverflowClip()) {
        ...
        if (const ShadowData* boxShadow = renderer()->style()->boxShadow()) {
            IntRect overflow = pixelSnappedIntRect(layerBounds);
            do {
                if (boxShadow->style() == Normal) {
                    IntRect shadowRect = pixelSnappedIntRect(layerBounds);
...
#if ENABLE(TIZEN_CSS_OVERFLOW_SCROLL_FIX_WRONG_CLIPPING)
            if ((this != rootLayer || respectOverflowClip == RespectOverflowClip) && !renderer()->hasClip())
#else
...
#if ENABLE(TIZEN_WEBKIT2) && ENABLE(TIZEN_CSS_OVERFLOW_SCROLL_ACCELERATION)
...
IntRect RenderLayer::calculateLayerBounds(const RenderLayer* layer, const RenderLayer* ancestorLayer, CalculateLayerBoundsFlags flags)
{
    ...
    ASSERT(layer->isStackingContainer() || (!layer->posZOrderList() || !layer->posZOrderList()->size()));
...
RenderLayerBacking* RenderLayer::ensureBacking()
{
    if (!m_backing) {
        ...
        if (renderer()->isRenderFullScreen()) {
            if (m_backing->graphicsLayer() && renderer()->document() && renderer()->document()->webkitCurrentFullScreenElement())
                m_backing->graphicsLayer()->setFullScreenLayerForVideo(renderer()->document()->webkitCurrentFullScreenElement()->isMediaElement());
        }
#endif
...
#if ENABLE(TIZEN_CSS_OVERFLOW_SCROLL_ACCELERATION)
GraphicsLayer* RenderLayer::layerForScrollingContents() const
{
...
#if PLATFORM(TIZEN)
        Node* newNode = result.innerNode();
        if (newNode && !newNode->renderer())
            newNode = result.innerNonSharedNode();

        // Clear the :hover chain only if current node is not same as oldHoverNode.
        if (oldHoverNode && oldHoverNode != newNode) {
#else
        if (oldHoverNode) {
#endif
...
void RenderLayer::setBackingNeedsRepaint()
{
    ASSERT(isComposited());
#if ENABLE(TIZEN_CSS_OVERFLOW_SCROLL_ACCELERATION)
    if (backing() && backing()->paintsIntoWindow()) {
#else
...
void RenderLayer::setNeedsSyncOverflowScrolling(bool needsSync)
{
    if (m_backing)
        m_backing->setNeedsSyncOverflowScrolling(needsSync);
}
#endif
