...
void RenderListMarker::paint(PaintInfo& paintInfo, const LayoutPoint& paintOffset)
{
    ...
    if (type == Asterisks || type == Footnotes)
        ...
        if (style()->isLeftToRightDirection()) {
            ...
            context->drawText(font, RenderBlock::constructTextRun(this, font, suffixSpace, 2, style()), textOrigin + FloatSize(width, 0));
        } else {
            UChar spaceSuffix[2] = { ' ', suffix };
            TextRun spaceSuffixRun = RenderBlock::constructTextRun(this, font, spaceSuffix, 2, style());
            float width = font.width(spaceSuffixRun);
            context->drawText(font, spaceSuffixRun, textOrigin);
            context->drawText(font, textRun, textOrigin + FloatSize(width, 0));
        }
    }
}
...
FloatRect RenderListMarker::getRelativeMarkerRect()
{
    ...
    switch (type) {
        ...
        case Footnotes: {
            const Font& font = style()->font();
            relativeRect = FloatRect(0, 0, font.width(m_text), font.fontMetrics().height());
            break;
        }
        case Disc:
        case Circle:
        ...
        case Urdu:
            if (m_text.isEmpty())
                return FloatRect();
            const Font& font = style()->font();
            float itemWidth = font.width(m_text);
            UChar suffixSpace[2] = { listMarkerSuffix(type, m_listItem->value()), ' ' };
