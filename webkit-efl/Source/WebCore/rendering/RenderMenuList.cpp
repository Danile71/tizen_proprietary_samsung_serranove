    ...
    , m_popupIsVisible(false)
{
    ...
    TIZEN_LOGI(" RenderMenuList() Invoked ,this[%p]",this);
#endif
    ASSERT(element);
...
{
#if ENABLE(TIZEN_LIMIT_WEBPOPUPMENU_UPDATE)
    bool updateWebPopupMenu = false;
#endif
    if (m_optionsChanged) {
        updateOptionsWidth();
        m_optionsChanged = false;
#if ENABLE(TIZEN_LIMIT_WEBPOPUPMENU_UPDATE)
        updateWebPopupMenu = true;
#endif
    }
#if ENABLE(TIZEN_LIMIT_WEBPOPUPMENU_UPDATE)
    if (m_popupIsVisible && updateWebPopupMenu)
#else
    if (m_popupIsVisible)
#endif
...
void RenderMenuList::showPopup()
{
    ...
    if (!m_popup){
#if ENABLE(TIZEN_DLOG_SUPPORT)
    TIZEN_LOGI(" Creating popup ,this[%p]",this);
#endif
