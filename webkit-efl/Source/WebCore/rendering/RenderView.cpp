...
{
#if ENABLE(TIZEN_LAYOUTSTATE_NULL_CHECKING_WORKAROUND)
    if (m_layoutState)
        return;
#endif
...
void RenderView::mapLocalToContainer(RenderBoxModelObject* repaintContainer, TransformState& transformState, MapCoordinatesFlags mode, bool* wasFixed) const
{
...
{
    rects.append(snappedIntRect(accumulatedOffset, m_layer->size()));
}
