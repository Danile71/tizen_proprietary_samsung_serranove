...
#if ENABLE(TIZEN_TEXT_AUTOSIZING)
        FloatSize windowSize = screenRect(mainFrame->view()).size();
        windowSize.scale(1.0 / m_document->page()->deviceScaleFactor());
        windowInfo.windowSize = roundedIntSize(windowSize);
#else
...
#if ENABLE(TIZEN_TEXT_AUTOSIZING)
    // 0.5f has been calculated by testing to make a balance between the boosted-text and non boosted-text.
    float scaleValue = 0.5f;
    float mul = m_document->settings()->textAutosizingFontScaleFactor();
    if (mul >= multiplier * scaleValue)
        multiplier = mul;
    else
        multiplier = multiplier * scaleValue;
#endif
...
bool TextAutosizer::containerIsRowOfLinks(const RenderObject* container)
{
    ...
    //  2. it should contain min. 2 inline links and all links should
    ...
    // Example:  http://findmymobile.samsung.com (Russia site)
    int linkCount = 0;
...
#if ENABLE(TIZEN_TEXT_AUTOSIZING_NUMBER_OF_LINKS_FIX)
    return (linkCount >= 2);
#else
    return (linkCount >= 3);
#endif
